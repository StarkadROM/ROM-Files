...
function GemPageData:ForEachFitTypeNeighborOfGemPageSkillCell(skillCellId, action, args)
  if not skillCellId or not action or not self.dataMap or not self.skillCellNeighborDataMap or not self.skillCellFitTypeNeighborMap then
    return
  end
  ...
  if not neighborDatas or not next(neighborDatas) or not fitTypeNeighborDatas or not next(fitTypeNeighborDatas) then
    return
  end
...
function GemPageData:GetHeroFeatureLv()
  local addHeroLv = 1
  for _, item in pairs(self.dataMap) do
    if item.gemSkillData then
