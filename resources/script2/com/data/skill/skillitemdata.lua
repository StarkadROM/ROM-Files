...
function SkillItemData:ctor(id, pos, autopos, profession, sourceId, extendpos, shortcuts, extramaxlv, ignoreCondition)
  self.shortcuts = {}
  self:Reset(id, pos, autopos, 0, sourceId, extendpos, shortcuts, extramaxlv)
...
function SkillItemData:Reset(id, pos, autopos, cd, sourceId, extendpos, shortcuts, extramaxlv)
  if pos ~= nil then
  ...
  if maxCDTimes and maxCDTimes > 0 then
    SkillProxy.Instance:SetSkillLeftCD(self.sortID, maxCDTimes)
  end
end
function SkillItemData:_GetMaxlevel(paramName, staticData)
  ...
  if staticData and staticData[paramName] then
    ...
    while tempData[paramName] ~= nil and num < 99 do
      ...
      if not nextData.ExtraMaxLevel or not (nextData.ExtraMaxLevel > self.extraMaxLevel) then
        tempData = nextData
        num = num + 1
      end
    end
...
function SkillItemData:GetNextID(breakTop, staticData, newBreakTop, skipCheck)
  ...
  if staticData and staticData.ExtraMaxLevel then
    if staticData.ExtraMaxLevel > (self.extraMaxLevel or 0) and not skipCheck then
      nextID = nil
    end
  end
  return nextID
...
function SkillItemData:CheckFuncOpen(funcType)
  ...
  if funcType == self.FuncType.Normal then
    if self:GetID() == Game.Myself.data:GetAttackSkillIDAndLevel() then
      return true
    end
    ...
    if self.staticData.SkillType == "Switch" then
      return true
    end
    return false
...
function SkillItemData:GetMaxCDTimes()
  return self.staticData and self.staticData.Logic_Param.CdTimes
end
...
function SkillItemData:GetAltIcon(creature)
  ...
  if func == "mountform" then
    ...
    if mountForm and mountForm > 1 then
