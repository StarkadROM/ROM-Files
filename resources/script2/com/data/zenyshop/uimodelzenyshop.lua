...
UIModelZenyShop.DailyOneZenyRedTipID = 10600
function UIModelZenyShop:UpdateOneZenyGoodsBuyInfo()
  local shopConf = self:GetItemShopConf()
  if not shopConf[self.DailyOneZenyGoodsID] then
    RedTipProxy.Instance:RemoveWholeTip(self.DailyOneZenyRedTipID)
    return
  end
  local pre_buyCount = HappyShopProxy.Instance:GetCachedHaveBoughtItemCount(self.DailyOneZenyGoodsID) or 0
