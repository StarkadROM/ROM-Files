...
function SpyInfo.Detect()
  if SpyInfo.IsRunOnAndroid() then
    ...
    if #apksMap > 0 and SpyInfo.CacheCheat(apksMap, "apk") then
      isChange = true
    end
    local processesMap = {}
    for spy in pairs(SpyProcessMap) do
      for process in pairs(ProcessesMap) do
        if SpyInfo.Levenshtein(spy, process) then
          table.insert(processesMap, process)
        end
      end
    end
    if #processesMap > 0 and SpyInfo.CacheCheat(processesMap, "process") then
      isChange = true
    end
    if isChange then
...
function SpyInfo.ConvertReportLog()
  ...
  local serverId = tostring(server ~= nil and server.serverid or 1)
  local tmpTB = {}
