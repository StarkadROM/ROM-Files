...
function ApplicationInfo.GetAppleDeviceVersion(deviceName)
  ...
  if startPos then
    if not endPos or not (endPos - 1) then
    end
  else
  end
  return tonumber(string.sub(deviceModel, startPos + string.len(deviceName), (string.len(deviceModel)))) or -1
end
...
function ApplicationInfo.IsIphone7P_or_Worse()
  ...
  return diviceVersion >= 0 and diviceVersion < 10
end
