  ...
  if table.ContainsKey(gCachedDownloadTaskRecordID, key) then
    ...
    if self:IsDownloading(photo_id, o_or_t) then
      ...
      if BranchMgr.IsChina() then
        taskRecord.URL = fileServerURL .. "/" .. self:GetPathOfServer(scenic_spot_id, extension) .. (o_or_t and "" or "!100") .. (timestamp > 0 and "?t=" .. timestamp or "")
      else
    ...
    local url
    if BranchMgr.IsChina() then
      url = fileServerURL .. "/" .. self:GetPathOfServerWithRoleID(role_id, scenic_spot_id, extension) .. (o_or_t and "" or "!100") .. (timestamp > 0 and "?t=" .. timestamp or "")
    else
...
function NetIngScenicSpotPhotoNew:DownloadNew(scenic_spot_id, photo_id, timestamp, progress_callback, success_callback, error_callback, o_or_t)
  ...
  if table.ContainsKey(gCachedDownloadTaskRecordID, key) then
    ...
    if self:IsDownloading(photo_id, o_or_t) then
      ...
      local newURL
      if BranchMgr.IsChina() then
        newURL = fileServerURL .. "/" .. self:GetPathOfServerNew(scenic_spot_id) .. (o_or_t and "" or "!100") .. (timestamp > 0 and "?t=" .. timestamp or "")
      else
        newURL = fileServerURL .. "/" .. self:GetPathOfServerNew(scenic_spot_id)
        if BranchMgr.IsSEA() or BranchMgr.IsNA() or BranchMgr.IsEU() then
          if not o_or_t or not newURL then
            newURL = newURL .. "!33"
          end
    ...
    local url
    if BranchMgr.IsChina() then
      url = fileServerURL .. "/" .. self:GetPathOfServerNew(scenic_spot_id) .. (o_or_t and "" or "!100") .. (timestamp > 0 and "?t=" .. timestamp or "")
    else
      url = fileServerURL .. "/" .. self:GetPathOfServerNew(scenic_spot_id)
      if BranchMgr.IsSEA() or BranchMgr.IsNA() or BranchMgr.IsEU() then
        if not o_or_t or not url then
          url = url .. "!33"
        end
