  ...
  if table.ContainsKey(gCachedDownloadTaskRecordID, key) then
    ...
    if self:IsDownloading(photo_id, o_or_t) then
      ...
      if not BranchMgr.IsTW() and not BranchMgr.IsJapan() then
        ...
        taskRecord.URL = fileServerURL .. "/" .. self:GetPathOfServer(union_id, pos_index, extension) .. ("!" .. thumbnailVersionName) .. "?t=" .. timestamp
      else
    ...
    if not BranchMgr.IsTW() and not BranchMgr.IsJapan() then
      ...
      url = fileServerURL .. "/" .. self:GetPathOfServer(union_id, pos_index, extension) .. ("!" .. thumbnailVersionName) .. "?t=" .. timestamp
    else
