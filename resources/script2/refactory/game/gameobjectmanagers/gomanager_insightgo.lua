...
function GOManager_InsightGO:GetObjByLv(lv)
  ...
  if not self.insightBlockInfo[mapid] or not self.insightBlockInfo[mapid].OBJ or not self.insightBlockInfo[mapid].OBJ[lv] then
  ...
  if hide then
    ...
    if objects and #objects > 0 then
      ...
      for i = 1, #objects do
        ...
        xdlog("添加场景物件屏蔽", single, lv, objId)
        self.insightBlockInfo[mapID].OBJ[lv][objId] = 1
      end
    end
  else
    local npcids = data.npcid
    if npcids and #npcids > 0 and self.insightBlockInfo[mapID].NPC then
      local tempNpcs = {}
...
function GOManager_InsightGO:CheckNpcOutlineBlock(npcid)
  ...
  if TableUtility.ArrayFindIndex(blockNpcList, npcid) > 0 then
    return true
  end
  return false
end
