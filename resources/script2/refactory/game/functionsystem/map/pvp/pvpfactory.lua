  ...
  for i = 1, #roles do
    ...
    if myself ~= role then
      ...
      if inOB then
        ...
        role.data:Camp_SetIsInMyTeam(teamProxy:IsInMyTeam(playerid))
      end
    end
  end
end
...
local HandleNpcsGVG = function(roles, isGVGStart, includeMercenary)
  ...
  for k, role in pairs(roles) do
    ...
    if GvgProxy.Instance:CheckMetalNpcBornHide(role.data.id) then
      role:Hide()
    end
  end
end
local HandleAddPets = function(pets, isGVGStart, isTeamsPVP, isGroupPVP)
  if not pets then
    return
  end
  ...
  for i = 1, #pets do
    local nPet = pets[i]
    if nPet.data:IsCopyNpc_Detail() or nPet.data:IsFollowMaster_Detail() then
      if not isGVGStart and not isTeamsPVP and not isGroupPVP then
...
function TeamPws:Shutdown()
  ...
  Game.GameHealthProtector:SetForceMinPlayerCount(5)
  self.isFire = nil
  PvpProxy.Instance:UpdateFreeFire(false)
end
...
function TeamTwelve:HandleAddPets(pets)
  HandleAddPets(pets, false, false, true)
end
