...
function InteractMount:GetOn(cpid, charid)
  ...
  if not self.cpInited then
    ...
    InteractMount.super.GetOn(self, cpid, charid)
  end
...
function InteractMount:GetOff(charid)
  local lastCount = self.cpCount
  if not self.cpInited then
...
function InteractMount:GetRandomCP()
  ...
  if not (#cps > 0) or not cps[math.random(#cps)] then
    local value
  end
