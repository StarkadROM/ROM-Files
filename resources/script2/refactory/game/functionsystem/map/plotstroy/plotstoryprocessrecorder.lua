...
function PlotStoryProcessRecorder:Record_MoveSpd(...)
  ...
  for i = 1, #targets do
    ...
    local uid, roleMoveSpdProp = target.data.props and target.data.id, target.data.props:GetPropByName("MoveSpd")
...
function PlotStoryProcessRecorder:Restore_Sound(info)
  for k = 1, #info do
    local params = info[k]
    local oneShotAudioManageList = params[2]
    local loopAudioId = params[3]
    local oneShot2DAudioPlay = params[4]
    if oneShotAudioManageList and #oneShotAudioManageList > 0 then
