...
function PlotStoryProcess._setCreatureAnimatorCullingMode(creature, mode)
  if not creature or not creature.assetRole or Slua.IsNull(creature.assetRole.completeTransform) then
    return
  end
...
local tempArgs = {}
local tempFindMapNpcParams = {mapnpcid = nil, mapnpcuid = nil}
function PlotStoryProcess:place_to(params)
  ...
  if params.anchor_mapnpcid then
    tempFindMapNpcParams.mapnpcid = params.anchor_mapnpcid
    tempFindMapNpcParams.mapnpcuid = params.anchor_mapnpcuid
    anchorTarget = self:_getTargetByParams(tempFindMapNpcParams)
    if #anchorTarget > 0 then
      anchorTarget = anchorTarget[1]
    end
  end
  local targets = self:_getTargetByParams(params)
  for i = 1, #targets do
    local target = targets[i]
    if anchorTarget and anchor then
      local npcguid
      ...
      Game.InteractNpcManager:AddMountInter(npcguid, anchor, target.data.id)
  ...
  for i = 1, #targets do
    ...
    if actionData then
      ...
      local action = function(target)
        if target == Game.Myself then
          target:Client_PlayAction(actionName, startPct, loop, nil, focusDuration, freezeAtEnd)
        elseif params.pos then
          ...
          target:Server_PlayActionCmd(actionName, startPct, loop, nil, focusDuration, freezeAtEnd)
        end
      end
      action(target)
...
function PlotStoryProcess:ChangeTargetRolePartByParam(target, params)
  if target and target.assetRole then
    if not params.keepChange then
      Game.PlotStoryManager:RecordOriRolePart(target, params)
    end
...
function PlotStoryProcess:fullScreenEffect(params)
  if params and params.on == true then
    if not Game.Myself.data.attrEffect:BlindnessState() then
      local data = {}
...
function PlotStoryProcess:play_sound(params)
  ...
  local oneShotAudioManageList = {}
  local loopAudioId, oneShot2DAudioPlay
  if not path then
    self:ErrorLog_Plot()
  else
    local targets = self:_getTargetByParams(params)
    if #targets > 0 then
      for i = 1, #targets do
        ...
        TableUtility.ArrayPushBack(oneShotAudioManageList, AudioUtility.PlayOneShotAt_Path(path, tempPos[1], tempPos[2], tempPos[3], AudioSourceType.CUTSCENE_Sfx))
      end
    elseif loop then
      ...
      oneShot2DAudioPlay = AudioUtility.PlayOneShot2D_Path(path, AudioSourceType.CUTSCENE_Sfx)
    end
  end
  return true, oneShotAudioManageList, loopAudioId, oneShot2DAudioPlay
end
...
function PlotStoryProcess:play_video(params)
  ...
  local fadeIn, fadeOut = params.fade_out or params.fade_in or 0, 0
  local startVideoHide, endVideoHide = params.end_video_hide or params.start_video_hide or 0, 0
  if path then
    local hasAlphaCtrl = fadeIn + fadeOut + startVideoHide + endVideoHide > 0
    ...
    if VideoPanel.Instance and hasAlphaCtrl then
      VideoPanel.Instance:SetTextureAlpha(startVideoHide + fadeIn > 0 and 0 or 1)
...
function PlotStoryProcess:chapter(params)
  if not params.inTime or not params.outTime or not params.time then
    return true
  end
