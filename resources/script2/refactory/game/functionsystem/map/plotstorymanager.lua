...
function PlotStoryManager:ShutDownProgress(plotProgress)
  ...
  if self.pstlp_list then
    for i = #self.pstlp_list, 1, -1 do
      local plot = self.pstlp_list[i]
      if plot == plotProgress then
        table.remove(self.pstlp_list, i)
      end
    end
  end
...
function PlotStoryManager:ShowPlotStoryView(plotStoryShowSkip, skipDelay)
  ...
  self:AddUIView(PlotStoryManager.PlotStoryUIId, plotStoryShowSkip, {skipDelay = skipDelay})
end
...
function PlotStoryManager:Start_SEQ_PQTLP(pqtl_seq, plotEndCall, plotEndCallParam, customNpclist, plotStoryShowSkip, fixedDuration, extraParams, isLoopPlay, startFromPct, plotStartCall, plotStartCallParam, skipDelay)
  if type(pqtl_seq) == "table" and #pqtl_seq == 1 then
    return self:Start_PQTLP(pqtl_seq[1], plotEndCall, plotEndCallParam, customNpclist, plotStoryShowSkip, fixedDuration, extraParams, isLoopPlay, startFromPct, plotStartCall, plotStartCallParam, nil, skipDelay)
  elseif type(pqtl_seq) == "string" then
    return self:Start_PQTLP(pqtl_seq, plotEndCall, plotEndCallParam, customNpclist, plotStoryShowSkip, fixedDuration, extraParams, isLoopPlay, startFromPct, plotStartCall, plotStartCallParam, nil, skipDelay)
  end
  if not self.running then
    return false
  end
...
function PlotStoryManager:Start_PQTLP(pqtl_name, plotEndCall, plotEndCallParam, customNpclist, plotStoryShowSkip, fixedDuration, extraParams, isLoopPlay, startFromPct, plotStartCall, plotStartCallParam, needRestore, skipDelay)
  if not self.running then
    return false
  end
  local pqtl_id = PQTL_Manager.Instance:Create_PQTL(pqtl_name)
...
function PlotStoryManager:Create_PSTLP(plot_id, pqtl_name, plotEndCall, plotEndCallParam, customNpclist, plotStoryShowSkip, fixedDuration, extraParams, isLoopPlay, startFromPct, plotStartCall, plotStartCallParam, needRestore, skipDelay)
  ...
  if self.running then
    pstlp:Launch()
    if startFromPct and startFromPct > 0 and startFromPct < 1 then
...
function PlotStoryManager:CSNotify_PQTLP_End(plot_id)
...
function PlotStoryManager:CSNotify_PQTLP_Start(plot_id, is_freeview, isBranch)
  self.curPendingPQTLisFreePlot = is_freeview
  self.curPQTLisBranch = isBranch == 1
end
...
function PlotStoryManager:SetRoleMoveActionSpd(role, actionSpd)
  ...
  if id then
    self.roleActionSpdCache[id] = actionSpd
  end
...
function PlotStoryManager:ResetMoveActionSpd(role)
  ...
  if role then
    ...
    local actionSpd = self.roleActionSpdCache[id]
    if actionSpd then
      role:Logic_SetMoveActionSpeed(actionSpd)
    end
  else
    for id, actionSpd in pairs(self.roleActionSpdCache) do
