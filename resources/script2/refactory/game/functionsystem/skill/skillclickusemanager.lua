...
function SkillClickUseManager:_ChangeWaitForUse(skillItemData, pointPos, clampInLaunchRange, quickuse)
  ...
  if id ~= 0 then
    ...
    if FunctionSkill.Me().isCasting then
      ...
      if info and info:GetSkillID() // 1000 == id // 1000 and info:CanClientInterrupt() then
        return Game.Myself:Client_BreakSkillLead(id)
      end
      if info:CanUseOtherSkill(id) then
...
function SkillClickUseManager:TryLaunchPointTargetTypeSkill(skillIDAndLevel, point)
  if self._preWaitForUse:IsNull() then
    local ret, msg = Game.Myself:Client_UseSkill(skillIDAndLevel, nil, point)
    if msg ~= nil then
...
function SkillClickUseManager:Update(time, deltaTime)
  ...
  elseif self._waitForUse:IsNull() == false and self._waitForUse:CanUse() then
  ...
  if rootID then
    ...
    if skilldata then
      ...
      if skilldata then
        ...
        if not SpeicalComboMap[rollbackID] then
          local newRollBack
        end
        if newRollBack then
