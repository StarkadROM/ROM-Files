...
local SceneTopFuncWord_Class = SceneUIManager.UseUGUI and NSceneTopFuncWord or SceneTopFuncWord
function Creature_SceneTopUI._CreateSceneTopFuncWord(asset, args)
  ...
  cellData[6] = creature
  self.topFuncCell = SceneTopFuncWord_Class.CreateAsArray(cellData)
...
function Creature_SceneTopUI:SetTopFuncFrame(text, icon, clickFunc, clickArgs, creature, sceneUIType)
  ...
  Game.CreatureUIManager:AsyncCreateUIAsset(self.creatureId, SceneTopFuncWord_Class.ResID, follow, Creature_SceneTopUI._CreateSceneTopFuncWord, args, Creature_SceneTopUI._CreateSceneTopFunc_ArgsDeleter)
end
function Creature_SceneTopUI:RemoveTopFuncFrame()
  if self.asyncCreateTopFunc then
    Game.CreatureUIManager:RemoveCreatureWaitUI(self.creatureId, SceneTopFuncWord_Class.ResID)
  end
  if self.topFuncCell then
...
function Creature_SceneTopUI:SetTopGiftSymbol(symbolName, clickEvent, clickEventParam)
  ...
  LuaVector3.Better_Set(tempV3, -20, 20)
...
function Creature_SceneTopUI:createOrGetTopSingUI()
  ...
  if creature then
    ...
    if not creature:IsUIMask(MaskPlayerUIType.ChatSkillWord) then
      local maskSingIndex
    end
...
function Creature_SceneTopUI:UpdateComodoBuildingProduce()
  if self.sceneComodoBuildingProduce then
    ...
    local args = ReusableTable.CreateArray()
    args[1] = follow
    args[2] = self.creature and self.creature.data.staticData.id
...
function Creature_SceneTopUI:SetGvGCookingInfo(creature)
  ...
  if not self.sceneNpcGvGCooking then
    ...
    args[1] = follow
    args[3] = self
    self.sceneNpcGvGCooking = SceneTopGvGCooking.CreateAsArray(args)
    ReusableTable.DestroyAndClearArray(args)
  end
end
