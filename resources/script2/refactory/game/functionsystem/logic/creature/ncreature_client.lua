...
function NCreature:Client_PlayAction2(name, normalizedTime, loop, fakeDead, forceDuration)
  if not self:HasAllPartLoaded() then
    return
  end
  self.ai:PushCommand(FactoryAICMD.GetPlayActionCmd(name, normalizedTime, loop, fakeDead, forceDuration), self)
end
function NCreature:Client_PlayActionMove(name, normalizedTime, loop, fakeDead, forceDuration)
  if not self:IsDressed() then
    return
  end
  ...
  self.ai:PushCommand(FactoryAICMD.GetPlayActionCmd(name, normalizedTime, loop, fakeDead, forceDuration, actionSpeed), self)
end
function NCreature:Client_PlayActionIdle(name, normalizedTime, loop, fakeDead, forceDuration)
  if not self:IsDressed() then
    return
  end
...
local HugNpcGUID = 0
function NCreature:Client_AddHugRole(npcId)
...
function NCreature:Client_PlayUseSkill(phaseData)
  ...
  if not staticData.NameZh or not (OverSea.LangManager.Instance():GetLangByKey(staticData.NameZh) .. "!!") then
    local speakName
  end
  if speakName then
