...
function NPlayer:UpdateRegisterMultiMount(enable)
  if enable then
    Game.InteractNpcManager:UpdateRegisterInteractMount(self.data.id, self.assetRole:GetPartID(Asset_Role.PartIndex.Mount))
  else
...
function NPlayer:TryUpdateSpecialBuff(buffInfo, active, fromID, layer)
  NPlayer.super.TryUpdateSpecialBuff(self, buffInfo, active, fromID, layer)
