...
function NMyselfPlayer:Server_PlayActionCmd(name, normalizedTime, loop, fakeDead, forceDuration, freezeAtEnd)
...
local IsTargetMultiMountCanRide = function(player)
  ...
  if opt & option_team > 0 and player:IsInMyTeam() then
    return true
  end
  return false
end
...
function NMyselfPlayer:Server_SyncShiftPoint(skillID, pos)
  ...
  if self.skill:GetSkillID() == skillID and self.skill:IsCastingShiftPointSkill() then
    ...
    local angleY = VectorHelper.GetAngleByAxisY(self:GetPosition(), pos)
    self.skill.phaseData:SetAngleY(angleY)
  end
end
...
local curTime, curgrid, info = 0, 0, nil
