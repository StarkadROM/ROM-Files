...
function NCreature:_UpdateTrackEffect(time, deltaTime)
  ...
  if effects then
    selfVector3[1], selfVector3[2], selfVector3[3] = self.assetRole:GetEPOrRootPosition(RoleDefines_EP.Chest)
...
function NCreature:PlayPickUpTrackEffect(path, pos, speed, audioPath)
...
function NCreature:UpdateContractEffect(time, deltaTime)
  ...
  for sourceid, effect in pairs(effects) do
    ...
    if lookatPlayer then
      ...
      local tm = TeamProxy.Instance:GetMyTeamMemberPos(sourceid)
      if tm then
        endPosition = tm:GetPos()
      end
    end
    if endPosition then
