...
function NCreature:IsReadyToHold(holdCP)
  ...
  if self.assetRole:IsPlayingAction(self:GetIdleAction()) or self.assetRole:IsPlayingAction(self:GetMoveAction()) then
...
function NCreature:Server_SyncSkill(phaseData)
  ...
  if nowInfo and nowInfo:CanUseOtherSkill(phaseData:GetSkillID()) and nowPhaseData and nowPhaseData:GetSkillPhase() == SkillPhase.Cast then
...
function NCreature:InitLogicTransform(serverX, serverY, serverZ, dir, scale, moveSpeed, rotateSpeed, scaleSpeed)
  ...
  self:InitLogicPos(serverX / 1000, serverY / 1000, serverZ / 1000)
end
function NCreature:InitLogicPos(x, y, z)
...
function NCreature:Server_PlayActionCmd(name, normalizedTime, loop, fakeDead, forceDuration, freezeAtEnd)
  self.ai:PushCommand(FactoryAICMD.GetPlayActionCmd(name, normalizedTime, loop, fakeDead, forceDuration, freezeAtEnd), self)
end
...
function NCreature:SetNeedMaskUI(needMaskUI)
  self.needMaskUI = needMaskUI
end
function NCreature:SetMaskUI(isMaskUI)
  if self.isMaskUI == isMaskUI then
    return
  end
  if isMaskUI == nil and self.cellPriority >= Game.MapManager:GetCreatureMaskRange() - 1 then
    return
  end
  self.isMaskUI = isMaskUI
...
function NCreature:RefreshUIMaskStatus()
  self.needMaskUI = not self.culling_visible or self.maskUIBecauseCellCreatureCount or self.cellPriority > Game.MapManager:GetCreatureMaskRange()
end
...
function NCreature:CheckMaskUI(time, deltaTime)
  if time > self.checkUINextTime then
    ...
    if self.isMaskUI ~= self.needMaskUI then
      self:SetMaskUI(self.needMaskUI)
    end
  end
end
...
function NCreature:OnPartCreated(part)
  ...
  if self:HasAllPartLoaded() then
    EventDispatcherRobust.Me():PassEvent(CreatureEvent.OnAllPartLoaded, self.data.id)
  end
end
function NCreature:OnBodyCreated()
end
function NCreature:HasAllPartLoaded()
  return self.assetRole and not self.assetRole:_IsLoading() and true or false
end
...
function NCreature:SetAllowConcurrent(value, skillids)
  self.allowConcurrent = value
  if self.buffConcurrentMap then
    TableUtility.TableClear(self.buffConcurrentMap)
  elseif skillids then
    self.buffConcurrentMap = {}
  end
  if skillids then
    for i = 1, #skillids do
      self.buffConcurrentMap[skillids[i]] = true
    end
  end
...
function NCreature:GetAllowConcurrent(skillid)
  if not self.buffConcurrentMap then
    ...
    return self.allowConcurrent and self.buffConcurrentMap[skillid // 1000] == true
  end
end
...
function NCreature:DoConstruct(asArray, data)
  ...
  self.data = data
  self.maskDistance = self.data.staticData and self.data.staticData.Distance and self.data.staticData.Distance * self.data.staticData.Distance or nil
  ...
  self.mapCellIndex = -1
  self.isMaskUI = false
  self.needMaskUI = false
  self.forceMaskUI = false
  self.cellPriority = 0
