...
function SkillPhaseData:ParseFromServer(msg, force)
  ...
  self:ClearGoPos()
  if msg.gopos and msg.gopos.x ~= 0 and msg.gopos.y ~= 0 and msg.gopos.z ~= 0 then
    local atkEffect = self:GetSpecialAttackEffect()
    if atkEffect then
...
function SkillPhaseData:GetSpecialAttackEffect()
  if not self.data[1] then
    return
  end
  local sData = Table_Skill[self.data[1]]
  if not sData then
    return
  end
  local attackEffects = sData.AttackEffects
  if not attackEffects then
    return
  end
  return attackEffects[1]
end
