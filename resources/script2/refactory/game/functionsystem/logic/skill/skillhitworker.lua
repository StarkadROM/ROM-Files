...
function SkillHitWorker:_Work(creature, targetGUID, damageType, damage, shareDamageInfos, comboDamageLabel, forceSingleDamage, targetIndex, targetCount, damageCount)
  ...
  if DamageType.None ~= damageType then
    ...
    self:_Hit(creature, targetCreature, damageType, damage, comboDamageLabel, forceSingleDamage, hitEP, targetPosition, allowEffect, allowHurtNum, damageCount)
  end
...
function SkillHitWorker:_Hit(creature, targetCreature, damageType, damage, comboDamageLabel, forceSingleDamage, hitEP, targetPosition, allowEffect, allowHurtNum, damageCount)
  local args = self.args
  local skillInfo = args[1]
  ...
  local sePath = skillInfo:GetHitSEPath(creature, fromWeaponID, damageType)
  ...
  if damageCount > 1 then
    ...
    if nil ~= sePath then
      targetCreature.assetRole:PlaySEOneShotOn(sePath, AudioSourceType.SKILL_Hit)
    end
  end
end
