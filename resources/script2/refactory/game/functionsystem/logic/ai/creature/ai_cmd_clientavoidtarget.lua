...
local Logic_MoveToAvoidPos = function(myself, avoid_target, avoidDistance, targetPos)
  ...
  local ret, no_move = Logic_GetAvoidPosition(endPos, srcPos, avoidDistance, targetPos)
  if ret then
    if no_move then
...
function AI_CMD_ClientAvoidTarget:Update(time, deltaTime, creature)
  if self.args[6] >= self.args[2] then
  ...
  if self.args[7] >= checkPosInterval then
    ...
    if Logic_MoveToAvoidPos(creature, self.args[1], self.args[3], self.args[8]) == -1 then
      self:End(time, deltaTime, creature)
      return true
    end
  end
