...
function AI_CMD_PlayAction:Start(time, deltaTime, creature)
  ...
  if (nil == self.args[2] or 1 > self.args[2]) and not self.args[3] then
    ...
    if hasAction then
      ...
      self.args[8] = creature:Logic_PlayAction(params)
      if self.args[8] then
  ...
  return hasAction or self.args[3] or self.args[4] or self.args[9]
end
function AI_CMD_PlayAction:End(time, deltaTime, creature)
  ...
  if nil ~= self.args[10] then
    removeSpEffectData.guid = self.args[10]
...
function AI_CMD_PlayAction:Update(time, deltaTime, creature)
  ...
  if nil ~= args[5] and args[5] > 0 then
    args[11] = args[11] + deltaTime
    if args[11] > args[5] then
      self:End(time, deltaTime, creature)
    end
  elseif (nil == args[2] or args[2] < 1) and not args[3] and not args[8] then
    self:End(time, deltaTime, creature)
  elseif args[4] and not creature:IsFakeDead() then
    self:End(time, deltaTime, creature)
  elseif args[9] and not creature:IsOnSceneSeat() then
    self:End(time, deltaTime, creature)
  end
end
