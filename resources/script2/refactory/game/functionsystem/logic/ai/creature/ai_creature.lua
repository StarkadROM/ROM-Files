  ...
  while index <= #cmdQueue do
    ...
    if currentCmds[cmdIndex] == nil then
      if cmd.AIClass.IsMoveCmd then
        if currentCmds[Index_CurCmd] == nil or currentCmds[Index_CurCmd].concurrent then
          else
          end
          if (cmd.concurrent or not currentCmds[Index_MoveCmd]) and AllowStart(cmd.AIClass, cmd, self, creature) then
...
function AI_Creature:BeHolded(masterGUID, creature, noBreak)
  ...
  if nil ~= creature.data then
    self.idleAI_BeHolded:Request_Set(masterGUID, creature.data:GetHoldScale(), creature.data:GetHoldDir(), creature.data:GetHoldOffset(), noBreak)
  else
...
function AI_Creature:DOPatrol(creature)
  if self.idleAI_Patrol == nil then
    ...
    if npcStaticData ~= nil and #npcStaticData > 0 then
