...
function GvgQuestScoreTip:Init()
  ...
  self:AddButtonEvent("HelpButton", function()
    TipsView.Me():ShowGeneralHelpByHelpId(525)
  end)
  self:FindQuestObj()
  self:FindScoreObj()
  self:FindTogs()
...
function GvgQuestScoreTip:UpdateByGVGSeasonState()
  ...
  if GvgProxy.Instance:IsLeisureSeason() then
    self:Hide(self.toggleRoot)
  else
    self:Show(self.toggleRoot)
  end
  self:Show(self.questRoot)
  self:Hide(self.scoreRoot)
end
...
function GvgQuestScoreTip:_UpdateServerScore()
  local data = GvgProxy.Instance:GetBattleInfo()
  self.scoreMidList:ResetDatas(data)
end
function GvgQuestScoreTip:_UpdateFixedScore()
  local data = GvgProxy.Instance:GetBattleEndInfo()
