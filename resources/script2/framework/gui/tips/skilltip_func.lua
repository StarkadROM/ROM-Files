...
local GetPioneerSkillListCount = function(tip)
  return #tip.data:GetPioneerSkill() + #tip.data:GetUnlearnedPioneerSkill()
end
...
function SkillTip:_MultiSelectFunc(bgHeight)
  ...
  if SkillTip.IsTypeAvailable(SkillTip.FuncTipType.MultiSelect) and self.data:CheckFuncOpen(SkillItemData.FuncType.Normal) then
    ...
    local _SkillOptionManager = Game.SkillOptionManager
    if self.data:GetID() == Game.Myself.data:GetAttackSkillIDAndLevel() then
      self.funcCheck_opt = normalAtkCheck_opt
      local data = {}
      data.text = ZhString.SkillTip_NormalAtkLabel
...
function SkillTip:_ItemCostFunc(bgHeight)
  ...
  if isShowTip then
    ...
    isShowTip = nextStaticData and nextStaticData.ItemCost and 0 < #nextStaticData.ItemCost
  end
  if isShowTip then
    if tip == nil then
...
function SkillTip:_SubSkillFunc()
  ...
  elseif data:CanSelectMount() then
    self.funcSub_opt = _OptionEnum.SelectMount
