...
function FloatBuffTip:SetData(data, forceOpen)
  ...
  local count = num > 4 and 4 or num
  ...
  if num > 4 and totalHeight < MaxBuffCellHeight then
...
function FloatBuffTip:CheckFood()
  ...
  self.foodBuffContainer:SetActive(effectiveFoodCount > 0)
  if effectiveFoodCount > 0 then
    self:GetFoodBuff()
  end
  return effectiveFoodCount > 0
end
