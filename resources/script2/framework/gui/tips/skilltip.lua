...
function SkillTip:GetCondition(skillData, nextID)
  ...
  return ConditionUtil.GetSkillConditionStr(skillData or Table_Skill[nextID], true, nil, nil, self.data.profession)
end
...
function SkillTip:UpdateCurrentInfo(skillData)
  ...
  redlog("markType", skillData.id, markType)
...
function SkillTip:GetSkillMarks(marktype)
  ...
  if marks then
    ...
    local n, mark, sname = #marks, nil, nil
    for i = 1, n do
      mark = marks[i]
      sname = nil
...
function SkillTip:GetHeroDesc(staticData, heroFeatureLv)
  ...
  if #descs == 3 then
    ...
    if heroFeatureLv < midLv then
      desc2 = string.format(_descUnlockColor, desc2) or desc2
    end
    desc3 = string.format(_descLv, maxLv, descs[3])
    if heroFeatureLv < maxLv then
      desc3 = string.format(_descUnlockColor, desc3) or desc3
    end
...
function SkillTip:CheckReplace(skillData)
  ...
  return skillData.SkillType == SkillType.Function or data:GetReplaceSkill() ~= nil
end
