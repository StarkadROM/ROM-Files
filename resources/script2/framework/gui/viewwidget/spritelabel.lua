...
local exclude_placeholder = ""
local iconpattern = "{%g-}"
local iconplaceholder = "     "
...
function SpriteLabel:ctor(label, width, iconWidth, iconHeight, iconCenterInLine)
  ...
  if label ~= nil then
    ...
    if SpriteLabel.useLuaVersion then
      if BranchMgr.IsJapan() then
        iconplaceholder = string.rep(" ", math.ceil(self.iconWidth / (math.floor(self.richLabel.defaultFontSize / 2) + self.richLabel.effectiveSpacingX)))
      else
        iconplaceholder = string.rep(" ", math.floor(self.iconWidth / (self.richLabel.defaultFontSize / 4)))
      end
    end
  end
...
function SpriteLabel:SetText(text, autoAddSps)
  ...
  if string.match(text, "icon") ~= nil then
    ...
    if not SpriteLabel.useLuaVersion then
      ...
      parsedText = self:SetRichLabel(text, self:CheckSpace())
    end
...
function SpriteLabel:AddSprite(data)
  ...
  for _, str in string.gmatch(data.info, SpriteLabel.PatternMap.equipupgrade) do
    ...
    if indicatorSp then
      ...
      if level then
        indicatorSp.color = level > 0 and activeEquipUpgradeColor or inactiveEquipUpgradeColor
  ...
  for _, str in string.gmatch(data.info, SpriteLabel.PatternMap.equipcard) do
    ...
    if indicatorBg then
      ...
      elseif number and number > 0 and number < 5 then
  ...
  for _, str in string.gmatch(data.info, SpriteLabel.PatternMap.price) do
    ...
    if indicatorSp then
      ...
      if arr and #arr > 1 then
        ...
        if itemId and Table_Item[itemId] and price and price > 0 then
...
function SpriteLabel:TryForceSpriteColor(sp)
  ...
  local colorStr = SpriteLabel.SpriteForceColorMap[sp.spriteName]
