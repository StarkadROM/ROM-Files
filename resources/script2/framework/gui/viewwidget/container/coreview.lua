...
function CoreView:PlayUIEffect(id, container, once, callback, callArgs)
  return self:PlayEffectByFullPath(ResourcePathHelper.UIEffect(id), container, once, callback, callArgs)
end
function CoreView:PlayEffectByFullPath(path, container, once, callback, callArgs)
  if once then
    ...
    end, {owner = self, params = callArgs})
  else
...
function CoreView:SetButtonEnable(button, enabled)
...
function CoreView:AdjustViewPort(oriViewPort, angleX)
  ...
  if self.temp_ViewPort == nil then
    self.temp_ViewPort = LuaVector3(vp_x, vp_y, vp_z)
  end
...
function CoreView:CameraFaceTo(transform, viewPort, rotation, duration, rotateOffset, listener, focusOffset)
  if not CoreView.RotateCamera then
    return
  end
  if nil == CameraController.singletonInstance then
...
function CoreView:CameraFocusAndRotateTo(transform, viewPort, rotation, duration, listener, focusOffset)
  if not CoreView.RotateCamera then
    return
  end
  if nil == CameraController.singletonInstance then
