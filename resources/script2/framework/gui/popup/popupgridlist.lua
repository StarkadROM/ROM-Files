...
function PopupGridList:ctor(gameObject, funcOnChange, owner, depth)
...
function PopupGridList:InitList(depth)
  self.objList = Game.AssetManager_UI:CreateAsset(ResourcePathHelper.UIPopup("PopupGridList"), self.objListContainer)
  if not self.objList then
    redlog("Load Failed: popup/PopupGridList")
    return
  end
  ...
  self.listCells = UIGridListCtrl.new(l_gridPopupItems, PopupGridListCell, "PopupGridListCell")
...
function PopupGridList:SetData(list, doNotCallback)
