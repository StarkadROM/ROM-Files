...
function StaticHurtNum:Show(num, pos, isFromMe, critType, isToMe)
  ...
  self.isToMe = isToMe
  if (not isFromMe or not isToMe) and Game.LogicManager_MapCell:MapCellLodLevelByPos(pos[1], pos[3]) ~= LODLevel.High then
    self:Hide()
    return
  end
...
function StaticHurtNum:FindObjects(effectGO)
  if effectGO and self.effectGO ~= effectGO then
    ...
    self.labelcomp = UIUtil.FindComponent("Label", Text, effectGO)
    ...
    self.crit = FindGO("Crit", effectGO):GetComponent(CanvasGroup)
    self.magicCrit = FindGO("MagicCrit", effectGO):GetComponent(CanvasGroup)
  end
end
