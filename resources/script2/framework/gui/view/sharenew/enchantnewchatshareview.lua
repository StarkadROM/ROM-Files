...
function EnchantNewChatShareView:UpdateCombine(combineEffs)
  ...
  if combineEffs then
    for i = 1, #combineEffs do
      ...
      if buffData then
        hasCombine = true
        self.combineAttrName.text = combineEffs[i].isWork and string.format("%s:%s", buffData.BuffName, buffData.BuffDesc) or string.format("%s:%s(%s)", buffData.BuffName, buffData.BuffDesc, combineEffs[i].WorkTip)
      end
    end
  end
