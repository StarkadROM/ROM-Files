...
function AllProfessionShareView:initView()
  ...
  self.attrLbls = {}
  self.attrLbls.str = self:FindGO("attr_str"):GetComponent(UILabel)
  self.attrLbls.agi = self:FindGO("attr_agi"):GetComponent(UILabel)
  self.attrLbls.vit = self:FindGO("attr_vit"):GetComponent(UILabel)
  self.attrLbls.int = self:FindGO("attr_int"):GetComponent(UILabel)
  self.attrLbls.dex = self:FindGO("attr_dex"):GetComponent(UILabel)
  self.attrLbls.luk = self:FindGO("attr_luk"):GetComponent(UILabel)
...
function AllProfessionShareView:showPropView()
  ...
  self.adventurePropGrid:ResetDatas(jobArray)
  local attrArray = {}
  for k, v in pairs(tempArray) do
    if ProfessionProxy.Instance:IsThisIdYiGouMai(v.id) then
      if Table_Class[v.id] and Table_Class[v.id].IsOpen ~= 0 then
        local attr = GameConfig.ProfessionAttrPlus[v.id] or ""
        local split = string.split(attr, "+")
        local splitLength = #split
        if splitLength == 2 then
          if attrArray[split[1]] then
            attrArray[split[1]] = attrArray[split[1]] + tonumber(split[2])
          else
            attrArray[split[1]] = tonumber(split[2])
          end
        end
      end
      if v and v.AdvanceClass then
        if #v.AdvanceClass == 3 then
          ...
          if #rightTable == 2 then
            for r = 1, #rightTable do
              if Table_Class[rightTable[r]] and Table_Class[rightTable[r]].IsOpen ~= 0 and ProfessionProxy.Instance:IsThisIdYiJiuZhi(rightTable[r]) then
                for i = jobDepth, 0, -1 do
                  local thisidClass = Table_Class[rightTable[r] + i]
                  if thisidClass and ProfessionProxy.Instance:IsThisIdYiJiuZhi(rightTable[r] + i) then
                    local attr = GameConfig.ProfessionAttrPlus[rightTable[r] + i] or ""
                    local split = string.split(attr, "+")
                    local splitLength = #split
                    if splitLength == 2 then
                      if attrArray[split[1]] then
          ...
          for k1, v1 in pairs(v.AdvanceClass) do
            if v1 and Table_Class[v1] and Table_Class[v1].IsOpen ~= 0 then
              for i = jobDepth, 0, -1 do
                local thisidClass = Table_Class[v1 + i]
                if thisidClass and ProfessionProxy.Instance:IsThisIdYiJiuZhi(v1 + i) then
                  local attr = GameConfig.ProfessionAttrPlus[v1 + i] or ""
                  local split = string.split(attr, "+")
                  local splitLength = #split
                  if splitLength == 2 then
                    if attrArray[split[1]] then
