...
function GuildInfoPage:InitGroupZone()
  ...
  self:AddClickEvent(self.lineGroupBtn, function(go)
    if GvgProxy.Instance:GetGroupCnt() == 0 then
...
function GuildInfoPage:GroupZoneShow()
  self.groupZoneObjRoot:SetActive(true)
  local groupZoneData = GvgProxy.Instance:GetGroupZoneList()
  self.groupZoneCtl:ResetDatas(groupZoneData)
end
...
function GuildInfoPage:UpdateGvgDroiyaLv()
  ...
  self.superGvg_Parent.gameObject:SetActive(FunctionUnLockFunc.checkFuncStateValid(6))
...
function GuildInfoPage:UpdateCurrentGvgZone()
  ...
  local gvgGroupID = GvgProxy.Instance:GetMyGuildGvgGroupID()
  if gvgGroupID then
    self.gvgGroupDesc = string.format(ZhString.NewGVG_GroupID, tostring(gvgGroupID))
