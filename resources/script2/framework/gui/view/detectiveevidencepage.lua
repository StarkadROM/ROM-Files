...
function DetectiveEvidencePage:RefreshTools()
  ...
  for k, v in pairs(staticToolList) do
    if self.existEvidenceData[k] and (not unlockedToolList or not unlockedToolList[k] or v <= unlockedToolList[k]) then
      xdlog("加入可用道具列表", k)
...
function DetectiveEvidencePage:RefreshPreviewPart(id)
  ...
  local actionName = ""
  local showList = {}
  if unlockMessages and #unlockMessages > 0 then
    for i = 1, #evidenceMessage do
    ...
    local previewModel = UIModelUtil.Instance:SetRoleModelTexture(self.evidenceTexture, self.parts, UIModelCameraTrans.Evidence, scale, nil, nil, nil, function(obj)
      ...
      for i = 1, 2 do
        ...
        if i == 1 then
          ...
          effectParent = effectParent or self:FindGO("effect_1", obj.complete.rendererNode.gameObject)
        else
...
function DetectiveEvidencePage:RefreshDetailPart(id)
  ...
  if totalEvidenceMessage - unlockMessageCount > 0 then
