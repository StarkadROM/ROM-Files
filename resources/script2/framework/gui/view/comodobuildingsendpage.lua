...
function ComodoBuildingSendPage:OnEnter()
  ...
  local containerDepth, panel = self.container.panel.depth, nil
...
function ComodoBuildingSendPage:UpdateRewards(resetPosition)
  ...
  if pId and eId then
    for _, data in pairs(Table_ManorSpecialReward) do
      if data.PetId == pId and data.EquipId == eId then
        local reachLimit, id = self.proxyIns:GetAreaSpecialRewardReceiveCount(data.id) >= data.Limit, nil
  ...
  local areaRewardData, list = Table_ManorAreaReward[self.showingAreaId], nil
  if pId then
...
local sortFunc = function(l, r)
  local lStar, rStar = r.hasStar and 1 or l.hasStar and 1 or 0, 0
  if lStar ~= rStar then
    return lStar > rStar
  end
  return l.id < r.id
