...
GVGSandTablePanel.ViewType = UIViewType.CheckLayer
...
function GVGSandTablePanel:AddViewEvts()
  self:AddClickEvent(self.helpBtn, function()
    local helpData = Table_Help[35251]
...
function GVGSandTablePanel:RefreshPage()
  ...
  if not curMapInfo then
    ...
    self.battleStatusRoot:SetActive(false)
  ...
  for i = 1, 8 do
    if pointConfig and pointConfig[i] then
      ...
      if pointInfos[i] then
        self.nodeCells[i]:SetData(pointInfos[i], defGuild, noMoreMetal)
      else
  ...
  if topGuilds and #topGuilds > 0 then
...
function GVGSandTablePanel:SetPerfectSpareStatus(bool)
  ...
  self.battleStatusRoot:SetActive(not bool)
