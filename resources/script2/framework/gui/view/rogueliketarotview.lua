...
function RoguelikeTarotView:FindObjs()
  ...
  local ctrlParentTrans, child, go = self:FindGO("Controls").transform, nil, nil
  self.stageCtrlGrids = {}
...
function RoguelikeTarotView:UpdateStage(stage)
  ...
  elseif stage == RoguelikeTarotViewStage.Draw then
    ...
    if stage == RoguelikeTarotViewStage.Confirm then
...
function RoguelikeTarotView:SetTarotTip(cardIndex, tarotId)
  ...
  if sData then
    ...
    if itemMaxLevel > 0 and itemMaxLevel == dungeonIns:GetRoguelikeItemNumByStaticId(sData.ItemID) then
...
function RoguelikeTarotView:Move(go, targetPosX, onFinished)
  ...
  t.method = arrayFindIndex(tarotInvisiblePosXes, targetPosX) > 0 and 1 or 2
  if onFinished then
...
function RoguelikeTarotView:CardsTurnOver(cardIndices, onFinished)
  ...
  local indexCount, card, cardBack, cardPosX, cardBackPosX = #cardIndices, nil, nil, nil, nil
  for i = 1, indexCount do
    local index, setOnFinished = i == indexCount and onFinished ~= nil, true
    ...
    if card and cardBack then
      ...
      if arrayFindIndex(tarotPosXes, cardPosX) > 0 and arrayFindIndex(tarotInvisiblePosXes, cardBackPosX) > 0 then
      ...
      elseif arrayFindIndex(tarotInvisiblePosXes, cardPosX) > 0 and arrayFindIndex(tarotPosXes, cardBackPosX) > 0 then
