...
HomeBuildingView.ViewType = UIViewType.BuildingLayer
...
function HomeBuildingView:OnClickBtnEditFurniture()
  ...
  self.objListSeriesType:SetActive(#datas > 1)
...
function HomeBuildingView:AdjustFurniturePlaceCell(deltaRow, deltaCol)
  ...
  if deltaCol < 0 and lastItemPos.x < self.cameraControl.cameraMinPos[1] or deltaCol > 0 and lastItemPos.x > self.cameraControl.cameraMaxPos[1] or deltaRow > 0 and lastItemPos.z < self.cameraControl.cameraMinPos[2] or deltaRow < 0 and lastItemPos.z > self.cameraControl.cameraMaxPos[2] then
    return
  end
  local tweener = self.curOperateItem.gameObject:GetComponent(TweenPosition)
  if tweener then
...
function HomeBuildingView:SetHomeScore()
  ...
  if not foundLv then
    ...
    self.tabScoreBoard.sliderHomeScore.value = needExp > 0 and curExp / needExp or 0
  end
end
...
function HomeBuildingView:HandleQuickUseItem(note)
  ...
  if not data or not data.data or data.type ~= QuickUseProxy.Type.Item then
    return
  end
