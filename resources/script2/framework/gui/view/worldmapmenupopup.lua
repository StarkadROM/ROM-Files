...
function WorldMapMenuPopUp:DoGoMap()
  ...
  if not activeMaps[self.chooseData.id] then
    return
  end
  if self.isFromFurniture then
    ServiceNUserProxy.Instance:CallGoToGearUserCmd(self.chooseData.id, SceneUser2_pb.EGoToGearType_Single, nil)
  else
...
function WorldMapMenuPopUp:UpdateGoBtn(data)
  ...
  local activeMaps = WorldMapProxy.Instance.activeMaps
  if activeMaps[data.id] then
    local mapInfo = Table_Map[data.id]
    canTransfer = mapInfo.MoneyType ~= 2 or NewRechargeProxy.Ins:AmIMonthlyVIP()
  end
  if canTransfer then
    self:SetTextureWhite(self.goBtn, ColorUtil.ButtonLabelOrange)
    self.goBtn:GetComponent(BoxCollider).enabled = true
  else
    self:SetTextureGrey(self.goBtn)
    self.goBtn:GetComponent(BoxCollider).enabled = false
  end
end
...
function WorldMapMenuPopUp:UpdateMap2D()
  ...
  local sceneInfo = autoImport("Scene_" .. self.mapdata.NameEn)
...
function WorldMapMenuPopUp:MakeQuestFatherTable()
  ...
  for k, v in pairs(self.questType) do
    ...
    for i = 1, #list do
      ...
      if questData.staticData.ColorFromServer == v.ColorFromServer and questData.map == mapid and questData.traceInfo ~= "" then
...
function WorldMapMenuPopUp:OnExit()
  ...
  self.minimapWindow:UpdateMapTexture(nil)
