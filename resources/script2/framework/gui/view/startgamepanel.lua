...
function StartGamePanel:OnEnter()
  ...
  self:initAndroidKey()
  self:requestSyncSavedID()
end
function StartGamePanel:requestSyncSavedID()
  FunctionLogin.Me():SyncLocalSavedDID()
end
...
function StartGamePanel:initView()
  ...
  do
    local wgPlatformEntrance = self.goPlatformEntrance:GetComponent(UIWidget)
    wgPlatformEntrance.leftAnchor.relative = 1
    wgPlatformEntrance.rightAnchor.relative = 1
    wgPlatformEntrance.bottomAnchor.relative = 1
    wgPlatformEntrance.topAnchor.relative = 1
    wgPlatformEntrance.leftAnchor.absolute = -140
    wgPlatformEntrance.rightAnchor.absolute = -44
    wgPlatformEntrance.bottomAnchor.absolute = -100
    wgPlatformEntrance.topAnchor.absolute = -6
    local lbPlatformEntrance = self:FindComponent("Lb", UILabel, self.goPlatformEntrance)
    lbPlatformEntrance.gameObject:SetActive(true)
    lbPlatformEntrance.text = ZhString.StartGamePanel_PlatformEntrance
  end
  self.FixResource = self:FindGO("FixResource")
  do
    local wgFixResource = self.FixResource:GetComponent(UIWidget)
    wgFixResource.leftAnchor.relative = 1
    wgFixResource.rightAnchor.relative = 1
    wgFixResource.bottomAnchor.relative = 1
    wgFixResource.topAnchor.relative = 1
    wgFixResource.leftAnchor.absolute = -140
    wgFixResource.rightAnchor.absolute = -44
    wgFixResource.bottomAnchor.absolute = -244
    wgFixResource.topAnchor.absolute = -150
    local lbFixResource = self:FindComponent("Lb", UILabel, self.FixResource)
    lbFixResource.gameObject:SetActive(true)
  ...
  self.startBtnTexName = StartGamePanel.StartBtnTexName
...
function StartGamePanel:InitBg()
  ...
  if self.bgVideoPlayer then
    ...
    function self.bgVideoPlayer.onError()
      self.bgVideoPlayer.gameObject:SetActive(false)
    end
    local path = VideoPanel.VideoPath .. StartGamePanel.BgVideoName
...
function StartGamePanel:ShowAgeDetailInfo()
  GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
    view = PanelConfig.AgeDetailInfoPanel
  })
end
...
function StartGamePanel:OnClickStartGame()
  if BranchMgr.IsChina() then
    ...
    if isGuest and UnionConfig and UnionConfig.bindingGuest and UnionConfig.bindingGuest == 1 then
...
function StartGamePanel:StartCheckLimitServer()
  local isNeedCheck = FunctionLogin.Me():checkIsNeedCheck()
  if not isNeedCheck or self.BlockRequest then
    return false
  end
  self:DisableLoginCollider()
  local xdsdkApplicationInfo = AppBundleConfig.GetXDSDKInfo()
  helplog("StartGamePanel:StartCheckLimitServer( )", xdsdkApplicationInfo.APP_ID)
  FunctionTrafficSdk.GetInstance():startCheck(xdsdkApplicationInfo.APP_ID)
  return true
end
function StartGamePanel:CallServer(acc)
  ...
  local block = self:StartCheckLimitServer()
  if block then
    return
  end
  local SDKEnable = FunctionLogin.Me():getSdkEnable()
...
function StartGamePanel:UpdateServerList(note)
  ...
  if SDKEnable then
    local serverDatas = FunctionLogin.Me():getServerDatas()
    if serverDatas and #serverDatas > 0 then
      self.serverData = FunctionLogin.Me():getDefaultServerData()
      if self.serverData.serverid ~= nil then
        PlayerPrefs.SetInt("PlayerPrefsMyServerId", tonumber(self.serverData.serverid))
      end
      if #serverDatas > 1 then
...
function StartGamePanel:HandlerSelectServer(note)
  if note ~= nil then
    ...
    if FunctionLogin.Me():getSdkEnable() and self.serverData.serverid ~= nil then
      helplog("HandlerSelectServer ==> ", self.serverData.serverid)
      PlayerPrefs.SetInt("PlayerPrefsMyServerId", tonumber(self.serverData.serverid))
    end
    self:UpdateServerShow()
  end
...
function StartGamePanel:OnExit()
  ...
  LogUtility.SetEnable(LogEnable)
  LogUtility.SetTraceEnable(LogEnable)
  selfIns = nil
end
