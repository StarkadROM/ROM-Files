...
function GemPage:InitBg()
  ...
  LuaVector3.Better_Set(bg2Angles, LuaGameObject.GetLocalEulerAngles(self.bg2Trans))
  tickManager:CreateTick(0, 16, self._OnBg2TickUpdate, self, 0)
  self:PlayUIEffect(EffectMap.UI.GemPageTurntable, self:FindGO("BgEffectContainer1"))
...
function GemPage:InitOthers()
  ...
  self:PlayUIEffect(EffectMap.UI.GemPageFloat, self.frontEffectContainer)
end
...
function GemPage:TryDelayedPlaySkillValidEffects(skillCell, delay)
  ...
  delay = delay or 800
  if not skillCell.data or not GemProxy.CheckContainsGemSkillData(skillCell.data) or not self.pageData:CheckIsSkillEffectValid(skillCell.data.id) then
...
function GemPage:_OnBg2TickUpdate(interval)
  bg2Angles[3] = bg2Angles[3] - 0.005 * interval
  if bg2Angles[3] <= -180 then
    bg2Angles[3] = bg2Angles[3] + 360
  end
...
function GemPage:_ForEachNeighborOfCell(cell, action, args)
  if not cell or not cell.neighborIds or not next(cell.neighborIds) then
    return
  end
...
function GemPage:_ForEachNeighborPathKeyOfCell(cell, action, args)
  ...
  local neighborIds, pathKey, attrCellId, skillCellId = cell.neighborIds, nil, nil, nil
...
function GemPage:GetCellById(id)
  ...
  return self.skillCellMap[id] or self.attrCellMap[id]
end
function GemPage:GetCenterScreenPositionXYZ()
  return LuaGameObject.WorldToScreenPointByTransform(self.uiCam, self.bg2Trans, Space.World)
end
function GemPage:GetCenterViewportPositionXYZ()
  return LuaGameObject.WorldToViewportPointByTransform(self.uiCam, self.bg2Trans, Space.World)
end
