...
function PackageFashionPage:ClickItemTab(cellCtl)
  ...
  if isNoneTipShow then
    self.noneTipLabel.text = cellCfg and cellCfg[2] == 1055 and not PackageEquip_ShowCar_Class[myPro] and ZhString.PackageView_FashionPageNoneTipForNonBarrowProfessions or ZhString.PackageView_FashionPageNoneTip
  end
...
function PackageFashionPage:AddEvts()
  ...
  self:AddButtonEvent("FashionHelpButton", function(go)
    self:ShowHelpTip()
  end)
    ...
    if items then
      for i = 1, #items do
        ...
        if savedItems and #savedItems > 0 then
          ...
          if not found and useableNum > 0 then
...
function PackageFashionPage.IsFashionItemEquiped(itemData)
...
function PackageFashionPage:ClickItem(cellCtl)
  ...
  if cellCtl.isEquiped and not cellCtl.isFashionHide then
    FunctionItemFunc.OffEquip_Fashion(data)
  elseif data and data.CanEquip and data:CanEquip() then
    FunctionItemFunc.DepositFashionEvt(data)
...
function PackageFashionPage:ShowHelpTip()
  local Desc = Table_Help[1000] and Table_Help[1000].Desc or ZhString.Help_RuleDes
...
function PackageFashionPage:RefreshMyselfRole()
  ...
  if self.role == nil then
    ...
    self.role:SetSuffixReplaceMap(Table_Class[MyselfProxy.Instance:GetMyProfession()].ActionSuffixMap)
...
function PackageFashionPage:DoPlayEmojiAct(actId)
  ...
  elseif actId == m_actionID_withBarrow then
    ...
    self:PlayRoleAction(sdata.Name)
  end
end
function PackageFashionPage:PlayRoleAction(name, delay)
  if delay == nil then
    self:_PlayRoleAction(name)
  else
...
function PackageFashionPage:_PlayRoleAction(name)
  if name == nil then
    return
  end
  if self.role == nil then
...
function PackageFashionPage:GetAdvFashionOfCurrentTab(ignoreFashionHide)
  ...
  local advFashion = tabData and tabData.tabConfig[2]
