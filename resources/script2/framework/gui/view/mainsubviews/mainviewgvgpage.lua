...
function MainViewGvgPage:_updatePerfectDefenseCD()
  ...
  self.perfectDefenseCDLab.text = string.format(ZhString.MainViewGvgPage_PerfectDefense, min, sec)
end
...
function MainViewGvgPage:updateHonorValue()
  ...
  if not data or not data:GetMultiple() then
    local reward_multiple
  end
  if reward_multiple and reward_multiple > 0 then
...
function MainViewGvgPage:AddViewEvts()
  ...
  self:AddListenEvt(GVGEvent.GVG_PerfectDefensePause, self.PausePerfectDefenseTick)
