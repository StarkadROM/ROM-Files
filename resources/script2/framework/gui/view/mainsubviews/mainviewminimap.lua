...
function MainViewMiniMap:InitData()
  ...
  self.minimapIconHide = PlayerPrefs.GetInt(PlayerPrefsMainViewMapIconHide, 0) or 0
...
function MainViewMiniMap:InitUI()
  ...
  self.minimapWindow:ActiveCenterMiniMapWindow()
...
function MainViewMiniMap:InitBigMap()
  ...
  self:AddClickEvent(self.bigMapButton, function(go)
    ...
    if inRaid then
      ...
      if MapManager:IsPVPMode_TeamPws() and not isObRunning then
  ...
  self:AddClickEvent(self.sceneHelp, function()
    local data = Table_Help[35238]
    if data then
...
function MainViewMiniMap:EnLargeBigMap(b, notCenterMyPos)
  ...
  if b then
    ...
    self.bigmapWindow:SetMapScale(MiniMapWindow.MAPSCALE_LARGE)
    ...
    self.bigmapWindow:SetMapScale(MiniMapWindow.MAPSCALE_NORMAL)
    self.bigmapWindow:ResetMapPos()
...
function MainViewMiniMap:ActiveSettingPage(isShow)
  if isShow then
    if not self.settingPageCtrl then
      self.settingPage = self:LoadPreferb_ByFullPath("GUI/v1/part/MiniMapSettingPage", self.mapBord)
      self.settingPageCtrl = MainViewMapSettingPage.new(self.settingPage)
      self.settingPageCtrl:SetPanelDepth(self.mPanel.depth + 10)
      self:InitSettingToggle()
      self.settingPageCtrl:RefreshGrid()
      self.container:RegisterChildPopObj(self.settingPage)
    end
  elseif self.settingPageCtrl then
    GameObject.DestroyImmediate(self.settingPage)
    self.settingPageCtrl = nil
  end
end
...
function MainViewMiniMap:OnShow()
  self:UpdateMyMapPos()
  self.minimapWindow:OnViewShow()
  self.bigmapWindow:OnViewShow()
...
function MainViewMiniMap:OnHide()
  ...
  self.minimapWindow:OnViewHide()
  self.bigmapWindow:OnViewHide()
end
  ...
  elseif tog == 2 then
    ...
    if npcList then
      for i = 1, #npcList do
        ...
        if npcData and npcData.MapIcon ~= "" and npcData.NoShowMapIcon ~= 1 and FunctionNpcFunc.checkNPCValid(npcData.id) then
          local isUnlock = true
    ...
    if gvgCookingNpc then
      ...
      if npcSData and npcSData.MapIcon ~= "" and npcSData.NoShowMapIcon ~= 1 and FunctionNpcFunc.checkNPCValid(npcData.id) then
...
function MainViewMiniMap:UpdateNpcWalk()
  ...
  self.minimapWindow:UpdateNpcWalkPointMap(cacheMap, true)
  self.bigmapWindow:UpdateNpcWalkPointMap(cacheMap, true)
end
function MainViewMiniMap:UpdateNearlyMonsters()
  ...
  for i = 1, #monsterList do
    ...
    if _roleDefunes_Camp.ENEMY == monster.data:GetCamp() and not self.thanatosSymbols[monster.data.id] and monster.data.userdata:Get(UDEnum.OPTION) ~= 1 then
      ...
      if monsterMapData then
        ...
        elseif sdata.MmapHide ~= 1 then
          if isRaid then
            if not isInGVGDetailedRaid or isInGVGDetailedRaid and staticMonsterId ~= GVGMetalID then
              symbolName = "map_dot"
            end
    ...
    elseif _roleDefunes_Camp.FRIEND == monster.data:GetCamp() and MapManager:IsPvPMode_TeamTwelve() then
      monsterMapData = self.tempMap[monster.data.id]
      if monsterMapData then
        self.tempMap[monster.data.id] = nil
      else
...
function MainViewMiniMap:UpdateNearlyTreasures()
  ...
  if self.minimapWindow:IsActive() then
    self.minimapWindow:UpdateFixedTreasurePoses(self.fixedTreasureMap, true)
  end
  if self.bigmapWindow:IsActive() then
    self.bigmapWindow:UpdateFixedTreasurePoses(self.fixedTreasureMap, true)
  end
end
...
function MainViewMiniMap:UpdateQuestMapSymbol()
  ...
  elseif inRaid and not MapManager:IsInGuildMap() then
  else
    questlst = {} or QuestProxy.Instance:getQuestListByMapAndSymbol(MapManager:GetMapID())
  end
  ...
  for _, q in pairs(questlst) do
    ...
    if params.ShowSymbol ~= 2 and params.ShowSymbol ~= 3 and self:CheckQuestIconAvailableInMap(q) then
  ...
  local npcList = MapManager:GetNPCPointArray()
  if npcList and self:CheckDailyQuestIconAvailableInMap() then
    for i = 1, #npcList do
      local npcPoint = npcList[i]
...
function MainViewMiniMap:UpdateTeamMemberPos(id, refreshSymbol)
  ...
  if not memData or memData.mapid ~= nowMapid and memData.raid ~= nowMapid or memData.offline == 1 or not memData:IsSameline() and not memData:IsInCommonlineMap() or mymData and mymData.sceneid ~= memData.sceneid then
    self:RemoveTeamMemberPos(id)
    return
  end
  ...
  self.minimapWindow:UpdateTeamMemberSymbol(self.teamMemberMapDatas, true)
  self.bigmapWindow:UpdateTeamMemberSymbol(self.teamMemberMapDatas, true)
...
function MainViewMiniMap:UpdateMapAllInfo(map2d)
  ...
  if map2d then
    self.bigmapWindow:UpdateMapTexture(self.mapdata, LuaGeometry.GetTempVector3(MiniMapWindow.DefaultMapTextureSize, MiniMapWindow.DefaultMapTextureSize), map2d, self:CheckBuildingIconAvailableInMap())
    ...
    if self.mapdata.MapScale then
      ...
      self.minimapWindow:UpdateMapTexture(self.mapdata, LuaGeometry.GetTempVector3(maxline, maxline), map2d, self:CheckBuildingIconAvailableInMap())
    end
...
function MainViewMiniMap:MapEvent()
  ...
  self:AddListenEvt(GVGEvent.GVGDungeonLaunch, self.UpdateGVGMetaInfo)
  self:AddListenEvt(GVGEvent.GVGDungeonShutDown, self.ClearGVGMetaInfo)
...
function MainViewMiniMap:UpdateMapChangeEvents()
...
function MainViewMiniMap:UpdateMapIconHide(note)
  local tempBit, tempShowAreaTips = note.body.bit, note.body.showAreaTips
  if tempBit ~= nil then
    helplog("隐藏小地图图标bit值 " .. tempBit)
    if self.minimapIconHide == tempBit then
      return
    end
    self.minimapIconHide = tempBit
    self:UpdateQuestMapSymbol()
    self:UpdateTransmitter()
    self.minimapWindow:UpdateNpcPoints(self:CheckBuildingIconAvailableInMap())
    self.bigmapWindow:UpdateNpcPoints(self:CheckBuildingIconAvailableInMap())
    self.minimapWindow:RefreshServerBuildingPoint(self:CheckBuildingIconAvailableInMap())
    self.bigmapWindow:RefreshServerBuildingPoint(self:CheckBuildingIconAvailableInMap())
  end
  if tempShowAreaTips ~= nil then
    helplog("小地图区域文字显示", tempShowAreaTips)
    if self.miniMapShowAreaTips == tempShowAreaTips then
      return
    end
    self.miniMapShowAreaTips = tempShowAreaTips
    self:UpdateShowAreaTips()
  end
end
...
function MainViewMiniMap:CheckQuestIconAvailableInMap(questData)
  local questType = questData.type
  if questType == QuestDataType.QuestDataType_MAIN then
    return self:GetBitByInt(self.minimapIconHide, 0)
  elseif questType == QuestDataType.QuestDataType_BRANCH then
    return self:GetBitByInt(self.minimapIconHide, 1)
  elseif questType == QuestDataType.QuestDataType_WORLD then
    return self:GetBitByInt(self.minimapIconHide, 5)
  elseif self:isDailyQuest(questType) then
    return self:GetBitByInt(self.minimapIconHide, 2)
  else
    return self:GetBitByInt(self.minimapIconHide, 7)
  end
  return true
end
...
function MainViewMiniMap:CheckDailyQuestIconAvailableInMap()
  return self:GetBitByInt(self.minimapIconHide, 2)
end
function MainViewMiniMap:CheckTransmitterIconAvailableInMap()
  ...
  return self:GetBitByInt(self.minimapIconHide, 4) and not inRaid
end
function MainViewMiniMap:CheckBuildingIconAvailableInMap()
  return self:GetBitByInt(self.minimapIconHide, 6)
end
...
function MainViewMiniMap:RemoveTreasureShow()
  ...
  self.minimapWindow:UpdateWorldQuestTreasure(self.minimapTreasureInfo, true)
  self.bigmapWindow:UpdateWorldQuestTreasure(self.minimapTreasureInfo, true)
...
function MainViewMiniMap:RunQuestSymbolAnimator(note)
  ...
  self.bigmapWindow:PlayChooseQuestSymbolAnimation(questData)
end
...
function MainViewMiniMap:HandleQuesUpdate(note)
  ...
  if self:IsObjActive(self.worldQuestProcess) then
    ...
    if curProcess and curProcess > 0 then
...
function MainViewMiniMap:ClearTreeList()
  ...
  _TableClearByDeleter(self.treeMapDatasMap, miniMapDataDeleteFunc)
  self.bigmapWindow:UpdateTreePoints(self.treeMapDatasMap, true)
  self.minimapWindow:UpdateTreePoints(self.treeMapDatasMap, true)
end
...
function MainViewMiniMap:UpdateTransmitter()
  if self:IsCurMapHasTransmitter() and self:CheckTransmitterIconAvailableInMap() then
    self.bigmapWindow:UpdateTransmitterPoints(self.bigmaplarge or false)
  else
    self.bigmapWindow:RemoveTransmitterPoints()
  end
end
...
function MainViewMiniMap:UpdateShowAreaTips()
  ...
  self.minimapWindow:UpdateAreaTips(self.areaTipsInfo)
  self.bigmapWindow:UpdateAreaTips(self.areaTipsInfo)
end
...
function MainViewMiniMap:_GetScenePlayerSymbolName(player, includeMyself)
  ...
  elseif MapManager:IsPVPMode_TeamPwsExcludeOthello() then
    playerSymbolName = MyselfProxy.Instance:GetPvpColor() == tag and Player_Symbol_Name_Map[2] or Player_Symbol_Name_Map[1]
  ...
  if hideValue > 0 and not Game.MapManager:IsPVPRaidMode() then
    playerSymbolName = nil
  end
  return playerSymbolName
end
function MainViewMiniMap:_GetOutNineScreenPlayerSymbolName(playerid)
...
function MainViewMiniMap:UpdateThanatosSymbol()
  ...
  elseif self.thanatosTick then
    ...
    self.minimapWindow:UpdateThanatosInfo(self.thanatosSymbols, true)
    self.bigmapWindow:UpdateThanatosInfo(self.thanatosSymbols, true)
  end
end
...
function MainViewMiniMap:UpdateOthelloMapInfo(note)
  ...
  self.minimapWindow:UpdateOthelloInfos(self.othelloMapInfo, true)
  self.bigmapWindow:UpdateOthelloInfos(self.othelloMapInfo, true)
  self:CheckLink()
end
local occupy
local updateindex = 0
...
function MainViewMiniMap:CheckLink()
  ...
  self.minimapWindow:UpdateLinkInfos(self.othelloLinkInfo, false)
  self.bigmapWindow:UpdateLinkInfos(self.othelloLinkInfo, false)
end
...
function MainViewMiniMap:ClearAllOthelloMapInfo()
  ...
  self.minimapWindow:UpdateOthelloInfos(self.othelloMapInfo, true)
  self.bigmapWindow:UpdateOthelloInfos(self.othelloMapInfo, true)
  self.minimapWindow:UpdateLinkInfos(self.othelloLinkInfo, true)
  self.bigmapWindow:UpdateLinkInfos(self.othelloLinkInfo, true)
  self:UpdateTeamMembersPos(true)
  self:ClearPlayerMap()
end
...
function MainViewMiniMap:UpdateRoguelikeMapInfo()
  ...
  self.minimapWindow:UpdateRoguelikeMap(self.roguelikeMapInfo, totalRoomCount, unlockRoomTypeMap)
  self.bigmapWindow:UpdateRoguelikeMap(self.roguelikeMapInfo, totalRoomCount, unlockRoomTypeMap, true)
end
function MainViewMiniMap:ClearAllRoguelikeMapInfo()
  _TableClearByDeleter(self.roguelikeMapInfo, miniMapDataDeleteFunc)
  self.minimapWindow:ClearRoguelikeMap()
  self.bigmapWindow:ClearRoguelikeMap()
  self:UpdateTeamMembersPos(true)
  self:ClearPlayerMap()
end
...
function MainViewMiniMap:UpdateStealthGameInfo()
  ...
  self.minimapWindow:UpdateStealthGame(self.stealthGameInfo)
  self.bigmapWindow:UpdateStealthGame(self.stealthGameInfo)
end
...
function MainViewMiniMap:ClearAllStealthGameInfo()
  ...
  self.minimapWindow:ClearStealthGame()
  self.bigmapWindow:ClearStealthGame()
  self:UpdateTeamMembersPos(true)
  self:ClearPlayerMap()
end
...
function MainViewMiniMap:HandlePosSyncFubenCmd(note)
  ...
  if posDatas then
    ...
    local mapDataMap, posData, guid, npcSID, serverPos, mapData, symbolName, staticData
    for i = 1, #posDatas do
      guid = posDatas[i].id
      ...
      if npcSID == 0 and guid ~= myselfId then
        ...
        mapData:SetParama("Symbol", self:_GetOutNineScreenPlayerSymbolName(guid))
      end
    end
  end
...
function MainViewMiniMap:OnRaidPuzzleLaunch()
  ...
  self.minimapWindow:CreatePuzzleMap(_Game.MapManager:GetRaidID())
  self.bigmapWindow:CreatePuzzleMap(_Game.MapManager:GetRaidID(), true)
...
function MainViewMiniMap:UpdatePuzzleShadow()
  ...
  self.minimapWindow:UpdatePuzzleMap(roomStrMap)
  self.bigmapWindow:UpdatePuzzleMap(roomStrMap)
...
function MainViewMiniMap:HandleRaidPuzzleRoomIconRaidCmd(note)
  ...
  self.minimapWindow:UpdateMapSymbolDatas(MiniMapWindow.Type.RaidPuzzleRoom, self.raidPuzzleRoomIconMap)
  self.bigmapWindow:UpdateMapSymbolDatas(MiniMapWindow.Type.RaidPuzzleRoom, self.raidPuzzleRoomIconMap)
end
...
function MainViewMiniMap:UpdateQuickTargetPos(note)
  ...
  self.minimapWindow:UpdateMapSymbolDatas(MiniMapWindow.Type.QuickTarget, self.quickTargetMap)
  self.bigmapWindow:UpdateMapSymbolDatas(MiniMapWindow.Type.QuickTarget, self.quickTargetMap)
end
...
function MainViewMiniMap:HandleAddCircleArea(note)
  ...
  self.bigmapWindow:UpdateMapCircleAreas(self.circleAreaMap)
  self.minimapWindow:UpdateMapCircleAreas(self.circleAreaMap)
end
...
function MainViewMiniMap:UpdateFlowerCarPos(note)
  ...
  if data and data.guid then
    ...
    self.minimapWindow:UpdateMapSymbolDatas(MiniMapWindow.Type.FlowerCar, self.flowerCarMap)
    self.bigmapWindow:UpdateMapSymbolDatas(MiniMapWindow.Type.FlowerCar, self.flowerCarMap)
  else
...
function MainViewMiniMap:UpdateGvgStrongHold()
  ...
  self.minimapWindow:UpdateGvgStrongHoldSymbols(true)
  self.bigmapWindow:UpdateGvgStrongHoldSymbols(true)
end
function MainViewMiniMap:UpdateLocalNpcPos()
  ...
  self.bigmapWindow:UpdateLocalNpcPos(self.npcPosMap)
  self.minimapWindow:UpdateLocalNpcPos(self.npcPosMap)
end
...
function MainViewMiniMap:UpdateTeamMateSymbol(id, miniMapData)
  ...
  elseif Game.MapManager:IsPvpMode_DesertWolf() then
    symbolName = self:_GetScenePlayerSymbolName(Game.Myself, true)
  else
...
function MainViewMiniMap:UpdateGVGMetaInfo(note)
  ...
  self.bigmapWindow:UpdateGvgMetalIcon(self.gvgMetalMap, true)
  self.minimapWindow:UpdateGvgMetalIcon(self.gvgMetalMap, true)
end
...
function MainViewMiniMap:UpdateWildMvpMonsters()
  self.minimapWindow:UpdateWildMvpSymbols(true)
  self.bigmapWindow:UpdateWildMvpSymbols(true)
end
