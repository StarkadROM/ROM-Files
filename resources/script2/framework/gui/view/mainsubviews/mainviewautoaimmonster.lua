...
local MaxAfkDuration = GameConfig.Afk and math.ceil(GameConfig.Afk.max_battle_time / 60) or 900
...
function MainViewAutoAimMonster:AddButtonEvt()
  ...
  self:AddClickEvent(self.selectTargetBtn, function()
    if self.selectTargetTime == nil or ServerTime.CurServerTime() - self.selectTargetTime > AutoBattle.ChangeTargetTime then
      ...
      if targetCreature and targetCreature:Alive() then
...
function MainViewAutoAimMonster:InitShow()
  ...
  self.isSweep = false
  local container = self:FindGO("Container", self.scrollGO)
  local wrapConfig = {
    wrapObj = container,
    pfbNum = 10,
...
function MainViewAutoAimMonster:IsChooseAll()
  local cellCtls = self.itemWrapHelper:GetCellCtls()
  if cellCtls and #cellCtls > 0 then
...
function MainViewAutoAimMonster:OnStartAfkClicked()
  ...
  if duration > 0 then
    do
      ...
      if not monsterIds or #monsterIds > 0 then
        MsgManager.ConfirmMsgByID(41147, function()
          ServiceUserAfkCmdProxy.Instance:CallReqAfkUserAfkCmd(duration * 60, inplace, protectTeam, monsterIds)
        end, nil, nil)
      else
...
function MainViewAutoAimMonster:OnAfkDurationChanged()
  local duration = (not self.afkDurationInput.value or not tonumber(self.afkDurationInput.value)) and 0
  local illegal = false
...
function MainViewAutoAimMonster:OnRecvBattleTime(data)
  data = data and data.body
  if not data then
    return
  end
  local timelen = data.timelen and math.floor(data.timelen / 60) or 0
  local totalTime = data.totaltime and math.floor(data.totaltime / 60) or 0
