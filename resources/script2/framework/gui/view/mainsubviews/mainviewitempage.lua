...
function MainViewItemPage:InitUI()
  ...
  if MAX_SHORTCUT / ROWCOUNT > 1 then
    for i = 6, 10 do
...
function MainViewItemPage:UpdateQuickItems()
  ...
  local _SetQuickItemData = function(i)
    ...
    if data and data.staticData and data.num > 0 then
      ...
      elseif data.staticData.id == MainViewItemPage.SignIn21ItemId then
        self:RegisterRedTipCheck(SceneTip_pb.EREDSYS_NOVICE_TARGET, self.quickItemCells[i].icon.gameObject, 42)
      end
    end
