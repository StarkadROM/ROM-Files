...
function MainViewMenuPage:InitUI()
  ...
  self:InitActivityCalendarBtn()
  self:InitBattlePassBtn()
  self:InitDailyDepositBtn()
  self:InitNoviceBattlePassBtn()
...
function MainViewMenuPage:UpdateServantIcon()
  ...
  if servantID ~= 0 then
    ...
    servantGender = not servantCfg or servantCfg.Gender or 1
  else
    servantGender = 1
  end
...
function MainViewMenuPage:InitActivityBtn()
  ...
  self:RegisterRedTipCheck(SceneTip_pb.EREDSYS_NOVICE_BP, self.DoujinshiButton, 17)
  ...
  self:RegisterRedTipCheck(SceneTip_pb.EREDSYS_SIGNACTIVITY_NOVICE, self.DoujinshiButton, 17)
...
function MainViewMenuPage:ActivitySignInBut()
  ...
  if sCfg and ins.signedCount >= sCfg.SignInMax then
    sCfg = nil
  end
...
function MainViewMenuPage:RefreshGiftTimeLimitBtn()
  if not TimeLimitShopProxy.Instance:CheckDojiBtnValid() then
    if self.giftTimeLimitBtn then
      self.giftTimeLimitBtn:SetActive(false)
      self.doujinshiGrid:Reposition()
    end
    return
  end
  if nil == self.giftTimeLimitBtn then
    local config = GameConfig.GiftTimeLimit
    local name = config and config.activityName or "限时商店"
    local icon = config and config.activityIcon or "tab_icon_107"
    self.giftTimeLimitBtn = self.container.activityPage:CreateDoujinshiButton(name, icon, function(go)
      self:_onClickGiftTimeLimitShop()
    end)
  else
    self.giftTimeLimitBtn:SetActive(true)
  end
  self.doujinshiGrid:Reposition()
end
function MainViewMenuPage:_onClickGiftTimeLimitShop()
  if TimeLimitShopProxy.Instance:CheckDojiBtnValid() then
    self:ToView(PanelConfig.TimeLimitShopView, {initPage = true})
  else
...
function MainViewMenuPage:UpdateTopic()
  ...
  if not self.topicInited then
    ...
    self:RegisterRedTipCheck(SceneTip_pb.EREDSYS_SERVANT_CHALLENGE, self.topic.gameObject, 42)
    ...
    self:AddClickEvent(self.topic.gameObject, function(go)
      self:ToView(PanelConfig.TopicView)
    end)
...
function MainViewMenuPage:UpdateLotteryActBtn()
  local actData = LotteryProxy.Instance:GetActivityLottery()
  ...
  for i = 1, #actData do
    ...
    if lotteryActConfig[t] then
      if actData[i].open then
        if nil == self.lotteryActObjs[t] then
          if BranchMgr.IsJapan() then
            ...
            self:AddClickEvent(self.lotteryActObjs[t], function(go)
              ...
              LotteryProxy.Instance:OpenNewLottery(t)
            end)
          else
        ...
        local isNew = redTipMgr:IsNew(redTip_LotteryActivity, t)
        if isNew then
          redTipMgr:RegisterUI(redTip_LotteryActivity, self.lotteryActObjs[t], 39)
        else
          redTipMgr:UnRegisterUI(redTip_LotteryActivity, self.lotteryActObjs[t])
        end
        local isFree = redTipMgr:IsNew(redTip_LotteryFree, t)
        if isFree then
          redTipMgr:RegisterUI(redTip_LotteryFree, self.lotteryActObjs[t], 39)
        else
          redTipMgr:UnRegisterUI(redTip_LotteryFree, self.lotteryActObjs[t])
        end
        local hasDailyReward = redTipMgr:IsNew(redTip_DailyReward, t)
        if hasDailyReward then
          redTipMgr:RegisterUI(redTip_DailyReward, self.lotteryActObjs[t], 39)
        else
          redTipMgr:UnRegisterUI(redTip_DailyReward, self.lotteryActObjs[t])
        end
...
function MainViewMenuPage:UpdateRewardButton()
  ...
  self.rewardBtn.gameObject:SetActive(#newpost > 0)
...
function MainViewMenuPage:UpdateBagNum()
  ...
  if uplimit > 0 and uplimit <= bagItemNum then
...
function MainViewMenuPage:MapViewInterests()
  ...
  self:AddListenEvt(MainViewEvent.InFiveClock, self.CheckAutoOpenPaysignView)
  ...
  self:AddListenEvt(ServiceEvent.UserEventGiftTimeLimitBuyUserEvent, self.RefreshGiftTimeLimitBtn)
  self:AddListenEvt(ServiceEvent.NUserVarUpdate, self.HandleVarUpdate)
  self:AddListenEvt(ServiceEvent.ActivityCmdDaySigninActivityCmd, self.RefreshDailyLoginBtn)
...
function MainViewMenuPage:UpdateMainViewRedTipRegister(redtipId)
  if redtipId == 707 then
    local active = self:CheckMainviewButtonRedTipValid(707)
    if not active then
      local questManual = self:FindGO("QuestManual", self.topRFuncGrid2.gameObject)
      if questManual then
        self:UnRegisterSingleRedTipCheck(707, questManual)
      end
    else
      local questManual = self:FindGO("QuestManual", self.topRFuncGrid2.gameObject)
      if questManual then
        self:RegisterRedTipCheck(707, questManual, 42)
      end
    end
  end
  if nil ~= _lotteryTip[redtipId] then
    ...
    for t, lotteryActObj in pairs(self.lotteryActObjs) do
      local isNew = redTipMgr:IsNew(redtipId, t)
      if isNew then
        redTipMgr:RegisterUI(redtipId, lotteryActObj, 39)
      else
        redTipMgr:UnRegisterUI(redtipId, lotteryActObj)
      end
    end
    if not redTipMgr:HasRedTipParam(SceneTip_pb.EREDSYS_LOTTERY_FREE) then
...
function MainViewMenuPage:CheckAutoOpenPaysignView()
  PaySignProxy.Instance:TryOpenRewardView()
end
    ...
    for actid, tab in pairs(cfg) do
      do
        ...
        if open then
              ...
              self.paySignObj[actid] = self.container.activityPage:CreateDoujinshiButton(tab.activityName, tab.activityIcon, function(go)
                if open then
                  if PaySignProxy.Instance:CheckPurchased(actid) then
                    self:ToView(PanelConfig.EnterPaySignRewardView, {id = actid})
                  else
          ...
          if self.paySignObj and self.paySignObj[actid] then
...
function MainViewMenuPage:TryCreateSignInHintAsset()
  if self.isSignInHintCreated or not BranchMgr.IsChina() then
    return
  end
  local hintResPath = ResourcePathHelper.EffectUI(EffectMap.UI.GlowHint4)
...
function MainViewMenuPage:CheckNoviceBattlePassValidation()
  return NoviceBattlePassProxy.Instance:IsNoviceBPAvailable()
end
...
function MainViewMenuPage:UpdateMidSummerAct()
  ...
  local map, canShow, obj, config = self.midSummerActBtnMap, nil, nil, nil
...
function MainViewMenuPage:UpdateCrowdfundingAct()
  ...
  local map, canShow, obj, config = self.crowdfundingActBtnMap, nil, nil, nil
  for id, _ in pairs(actMap) do
    canShow, obj = ins:CheckActivityValid(id), map and map[id]
    ...
    if canShow and config and not obj then
      ...
      obj = self.container.activityPage:CreateDoujinshiButton(config.activityName, config.activityIcon, function()
        ...
        CrowdfundingActProxy.Instance:Query(upId)
...
function MainViewMenuPage:InitSignIn21Button()
  ...
  elseif not canShow and self.signIn21Button then
    RedTipProxy.Instance:UnRegisterUI(SceneTip_pb.EREDSYS_NOVICE_TARGET, self.signIn21Button)
...
function MainViewMenuPage:InitManorInfoButton()
  ...
  elseif not canShow and self.manorBtn then
    RedTipProxy.Instance:UnRegisterUI(SceneTip_pb.EREDSYS_MANOR_PARTNER_STORY, self.manorBtn)
...
function MainViewMenuPage:InitSevenRoyalFamilyTreeButton()
  ...
  elseif not canShow and self.sevenRoyalFamilyTreeBtn then
    RedTipProxy.Instance:UnRegisterUI(SceneTip_pb.EREDSYS_FAMILY_REWARD, self.sevenRoyalFamilyTreeBtn)
...
function MainViewMenuPage:InitSevenRoyalFamiliesEvidenceBookButton()
  ...
  elseif not canShow and self.evidenceBookBtn then
    RedTipProxy.Instance:UnRegisterUI(SceneTip_pb.EREDSYS_SECRET_ENLIGHT, self.evidenceBookBtn)
...
function MainViewMenuPage:RefreshTechTreeBtn()
  ...
  if canShow and not self.techTreeBtn then
    ...
    self:RegisterRedTipCheck(SceneTip_pb.EREDSYS_TECHTREE_PRODUCE, self.techTreeBtn, 42)
  ...
  elseif not canShow and self.techTreeBtn then
    RedTipProxy.Instance:UnRegisterUI(SceneTip_pb.EREDSYS_TECHTREE_PRODUCE, self.techTreeBtn)
...
function MainViewMenuPage:RefreshNewbieTechTreeBtn()
  ...
  elseif self.newbieTechTreeBtn then
    RedTipProxy.Instance:UnRegisterUI(SceneTip_pb.EREDSYS_NEWBIETECHTREE_LEVEL_AWARD, self.newbieTechTreeBtn)
...
function MainViewMenuPage:HandleMountLotteryActivity()
  ...
  elseif self.mountLotteryBut and self.addCreditGrid then
    ...
    local row, column
    row = #activeChildren > 4 and 4 or #activeChildren
    self.addCreditGridBg.width = 62 + 105 * row
    column = math.ceil(#activeChildren / 4)
    self.addCreditGridBg.height = 62 + 100 * column
    self.addCreditGrid:Reposition()
...
function MainViewMenuPage:CheckMainviewButtonRedTipValid(redtipId)
  if redtipId == 707 then
    ...
    local redTip = RedTipProxy.Instance:GetRedTip(redtipId)
    if redTip then
      return redTip.enable
    else
      return false
    end
  end
...
function MainViewMenuPage:InitNoviceShop()
  local proxy = NoviceShopProxy.Instance
  local stype, sid = proxy:GetShopConfig()
  local endtime = proxy:GetEndDate() or 0
  if BranchMgr.IsJapan() then
    if not self.noviceShopBtn then
      self.noviceShopBtn = self:CreatePocketLotteryButton(GameConfig.FirstDeposit.ActivityName, GameConfig.FirstDeposit.ActivityIcon)
      self:AddClickEvent(self.noviceShopBtn, function(go)
        proxy:CallQueryShopConfig()
        self:SetAddCreditNodeActive(false)
      end)
      self:RegisterRedTipCheck(NoviceShopProxy.RedTipID, self.noviceShopBtn, 39)
      self.addCreditGrid:Reposition()
    end
  else
    if not self.noviceShopBtn then
      local name = GameConfig.FirstDeposit.ActivityName
      self.noviceShopBtn = self.container.activityPage:CreateDoujinshiButton(name, GameConfig.FirstDeposit.ActivityIcon, function(go)
        proxy:CallQueryShopConfig()
      end)
    end
    self.doujinshiGrid:Reposition()
  ...
  if self.noviceShopBtn then
    if stype and sid and endtime > 0 then
      self.noviceShopBtn:SetActive(true)
    else
      self.noviceShopBtn:SetActive(false)
      return
    end
    if proxy:UpdateaRewardTip() or proxy:UpdateRedTip() then
      RedTipProxy.Instance:UpdateRedTip(NoviceShopProxy.RedTipID)
    else
      RedTipProxy.Instance:RemoveWholeTip(NoviceShopProxy.RedTipID)
    end
  end
end
...
function MainViewMenuPage:UpdateShopHotBtn()
  ...
  local innerTab
  if hotTabs and hotTabs.Inner and type(showTimeLimitIcon) == "number" and showTimeLimitIcon > 0 then
...
function MainViewMenuPage:InitNoviceBattlePassBtn()
  local noviceBattlePass = GameConfig.NoviceBattlePass
  if noviceBattlePass then
    local event = function(go)
      GameFacade.Instance:sendNotification(UIEvent.ShowUI, {
        viewname = "NoviceBattlePassView"
      })
    end
    if BranchMgr.IsJapan() then
      self.noviceBPObj = self:CreatePocketLotteryButton(noviceBattlePass.Name, noviceBattlePass.Icon)
      self:AddClickEvent(self.noviceBPObj, event)
    else
      self.noviceBPObj = self.container.activityPage:CreateDoujinshiButton(noviceBattlePass.Name, noviceBattlePass.Icon, event)
    end
    self:RegisterRedTipCheck(SceneTip_pb.EREDSYS_NOVICE_BP, self.noviceBPObj, 39)
    self:RegisterRedTipCheck(10729, self.noviceBPObj, 39)
    self:UpdateNoviceBattlePassBtn()
  end
end
function MainViewMenuPage:UpdateNoviceBattlePassBtn()
  if self.noviceBPObj then
    self.noviceBPObj:SetActive(self:CheckNoviceBattlePassValidation())
  end
end
function MainViewMenuPage:InitQuestManual()
  ...
  if questManual then
    ...
    self:RegisterRedTipCheck(310, questManual, 42)
    self:RegisterRedTipCheck(10100, questManual, 42)
  end
end
