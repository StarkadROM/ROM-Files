...
function SupplyDepotView:InitObjEvents()
  ...
  self:AddClickEvent(self.helpBtnGO, function()
    self:OpenHelpView(Table_Help[35041])
  end)
...
function SupplyDepotView:UpdateBulkPrice()
  local sum, cost, discount = 0, nil, nil
  ...
  if not canBuy or not ColorUtil.NGUIWhite then
  end
  self.bulkBuyPriceLabel.color = LuaGeometry.GetTempColor(1, 0.3764705882352941, 0.12941176470588237)
  self.bulkBuyPriceTable:Reposition()
  canBuy = canBuy and sum > 0
  self.bulkBuyBtnSp.CurrentState = canBuy and 1 or 0
  if not canBuy or not LuaGeometry.GetTempColor(0.7686274509803922, 0.5254901960784314, 0) then
  end
  self.bulkBuyBtnLabel.effectColor = LuaGeometry.GetTempColor(0.39215686274509803, 0.40784313725490196, 0.4627450980392157)
end
