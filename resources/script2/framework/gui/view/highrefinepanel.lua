...
function HighRefinePanel:InitView()
  ...
  self:AddClickEvent(helpBtn, function()
    self:OpenHelpView(Table_Help[1100])
  end)
...
function HighRefinePanel:UpdateLeftBord()
  ...
  local refineSum, mRefineSum, selection, val = 0, 0, nil, nil
...
function HighRefinePanel:UpdateAllToggles()
  ...
  local isAllRefine, isAllMRefine, enabledCellCount, selection = true, true, 0, nil
  local cells = self.propCtl:GetCells()
  for i = 1, #cells do
    if cells[i].isEnabled then
  ...
  self.allToggle1.value = enabledCellCount > 0 and isAllRefine
  self.allToggle2.value = enabledCellCount > 0 and isAllMRefine
