...
function LotteryMixedView:AddEvts()
  ...
  self:AddClickEvent(self:FindGO("HelpBtn"), function(go)
    self:ClickHelp()
  end)
...
local HelpId = 35051
function LotteryMixedView:ClickHelp()
...
function LotteryMixedView:_initLotteryFilter(resetPosition)
  ...
  if resetPosition and #rangeList > 0 then
...
function LotteryMixedView:_updateLotteryHelper()
  ...
  if 0 == LotteryProxy.Instance:GetUngetCount(self.lotteryFilterData) then
    local rate = LotteryProxy.Instance:GetRateBuyGroup(self.lotteryFilterData)
