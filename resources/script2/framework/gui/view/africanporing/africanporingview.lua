...
function AfricanPoringView:FindObjs()
  ...
  self:AddClickEvent(helpGO, function()
...
function AfricanPoringView:UpdateCoins()
  ...
  self.coin1Num.text = coin1 > 999 and "999+" or coin1
  self.shopCoin2Num.text = StringUtil.NumThousandFormat(coin2)
  self.coin2Num.text = coin2 > 999 and "999+" or coin2
end
...
function AfricanPoringView:UpdateBidBtn()
  ...
  if actData then
    ...
    local hasFreeBid = actData:HasFreeBids()
    if hasFreeBid then
    ...
    elseif actData.nextHit and 0 < actData.nextHit then
      self.safeCostTip.text = string.format(ZhString.AfricanPoring_NextHit, actData.nextHit)
    end
  else
    self.normalCostNum.text = 0
...
function AfricanPoringView:ShowPoolContent(pos)
  ...
  local allOwned = poolData:HasAllRewardOwned()
  if allOwned then
    self.rewardPoolContentTitle.text = ZhString.AfricanPoring_ContentTipTitleAllOnwed
  else
    self.rewardPoolContentTitle.text = ZhString.AfricanPoring_ContentTipTitleReplace
  end
  self.rewardPoolExtraGO:SetActive(allOwned)
