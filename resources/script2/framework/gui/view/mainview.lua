...
function MainView:LaunchLoginViewSequence()
  ...
  local actId = PaySignProxy.Instance:CanLoginShow()
  if actId then
    self.loginViewSequence:Append("EnterPaySignRewardView", {id = actId})
  end
  ...
  local anniverLoginID = DayloginAnniversaryPanel.CanShow()
  if anniverLoginID then
    self.loginViewSequence:Append("DayloginAnniversaryPanel", {id = anniverLoginID})
  end
...
function MainView:HandleAddSubView(data)
  ...
  local t, key, viewName, initParama, subViewData = type(data), nil, nil, nil, nil
  if t == "string" then
...
function MainView:HandleDayLoginPopup(note)
  ...
  if infos and #infos > 0 then
    ...
    for i = 1, #infos do
      ...
      if DailyLoginProxy.Instance:GetDaySignInActivity(single.activityid) then
        if single.novicetype then
          if not self.loginViewSequence:IsHaveView("DayloginAnniversaryPanel") then
            newIncome = true
            self.loginViewSequence:Append("DayloginAnniversaryPanel", {
...
function MainView:HandleTimeLimitShopPopup(note)
  ...
  local shopInfo = Table_GiftTimeLimit[activeID]
  if not shopInfo then
    redlog("弹脸商品不存在", activeID)
    return
  end
  if not self.loginViewSequence then
    self.loginViewSequence = ViewSequenceManager.Me():CreateViewSequence(self)
  ...
  if goods and #goods >= 0 then
    ...
    for i = 1, #goods do
      ...
      if single.id == self.newInStock and single.state == 1 and not self.loginViewSequence:IsHaveView("TimeLimitShopView") then
        newIncome = true
