...
local addMaterialGuid = function(arr, items, targetNum)
  local finishedNum, item, num = 0, nil, nil
  for i = 1, #items do
    item = items[i]
    ...
    if not (targetNum <= finishedNum) then
    end
  end
end
function AdventureMakeFoodPopUp:AddEvents()
  self:AddButtonEvent("ActionBtn", function()
    ...
    for i = 1, #self.recipeData.Material do
      func = nil
      mat = self.recipeData.Material[i]
      if mat[1] == 1 then
...
function AdventureMakeFoodPopUp:UpdateInfo()
  local bagProxy, mats, totalCount, totalLv, matData, targetCount, bagNum, items, matExpInfo = BagProxy.Instance, self.recipeData.Material, 0, 0, nil, nil, nil, nil, nil
  for i = 1, 6 do
