...
function DeComposeView:GetDecomposeEquips()
  ...
  for i = 1, #bagEquips do
    ...
    if (equipData.DecomposeID ~= nil or next(equipData.NewEquipDecompose) ~= nil) and BagProxy.Instance:CheckIfFavoriteCanBeMaterial(equip) ~= false and equipInfo.refinelv <= GameConfig.Item.material_max_refine and not self.chooseEquips[equip.id] then
...
function DeComposeView:StartDeCompose()
  ...
  local valid = true
  for k, v in pairs(self.chooseEquips) do
    if not BagProxy.CheckEquipIsClean(v, true) then
      valid = false
      break
    end
