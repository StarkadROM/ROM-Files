...
function GunShootPanel:_AddPointSymbol(creature)
  if not creature or not creature.data or not creature.data.staticData then
    return
  end
  ...
  if self.shootRangeMonsterID then
    ...
    if self.monsters[mid] then
      ...
      if self.isQWS_BirdType then
        ...
        local pointColor = point > 0 and _pointColor[1] or _pointColor[2]
        local prefix = point > 0 and "+" or ""
