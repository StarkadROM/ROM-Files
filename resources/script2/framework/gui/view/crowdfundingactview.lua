...
function CrowdfundingActView:InitView()
  ...
  local iconName, icon = Table_Item[self.cfg.item].Icon, nil
  self.itemIcons = {}
  for i = 1, 10 do
...
function CrowdfundingActView:GetDestIndex()
  ...
  if donate > 0 then
    for i = 1, #cfg do
      if donate <= cfg[i].processnum then
        if donate > (cfg[i - 1] and cfg[i - 1].processnum or 0) then
...
function CrowdfundingActView:OpenHelpView()
  local data = Table_Help[35090]
  if data then
