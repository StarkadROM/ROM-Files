...
local addPersonalArtifact = function(datas, bagType)
  ...
  local bagItems, item = bag and bag:GetItems(), nil
  if bagItems then
    for i = 1, #bagItems do
      item = bagItems[i]
...
local equipSortFunc = function(l, r)
  ...
  local lRefreshed, rRefreshed = r.personalArtifactData:IsRefreshed() and 1 or l.personalArtifactData:IsRefreshed() and 1 or 0, 0
...
function PersonalArtifactFunctionView:_InitComposePreview()
  ...
  artifactIns:SetPreviewFlag(self.initializedState == PersonalArtifactFunctionState.Compose)
end
...
function PersonalArtifactFunctionView:InitRight()
  ...
  self:AddClickEvent(self.helpBtn, function()
    if not self.helpId then
      return
    end
    self:OpenHelpView(Table_Help[self.helpId])
  end)
...
function PersonalArtifactFunctionView:OnClickComposeMaterialCell(index)
  ...
  if BagItemCell.CheckData(data) then
    ...
    self:ShowComposeItemTip(#mats > 0 and mats[1] or data.staticData.id)
  end
end
...
function PersonalArtifactFunctionView:UpdatePageState(state)
  ...
  self.helpBtn:SetActive(self.helpId ~= nil)
...
function PersonalArtifactFunctionView:UpdateRefreshAttrList(paData, isSameTarget)
  ...
  if paData then
    local attrs, isToSave = next(paData.candidateAttrs) ~= nil, true
    if isToSave then
...
function PersonalArtifactFunctionView:UpdateDecompose()
  ...
  local totalCount, t, quality, count = 0, nil, nil, nil
  ...
  local hasChooseData = #chooseData > 0
