...
function QuestManualView:GetGameObjects()
  ...
  if self.tabObj[4] then
    RedTipProxy.Instance:RegisterUI(SceneTip_pb.EREDSYS_FUNCTION_OPENING, self.tabObj[4], 10, {-27, -19})
  end
...
function QuestManualView:InitView()
  ...
  self.mainProcessPage = self:AddSubView("MainProcessPage", MainProcessPage)
  self.bgList = {}
...
function QuestManualView:updateManualContent()
  if self.currentVersionCell then
    ...
    if currentTypeName == "QuestType1" then
      self.mainProcessPage:SetData(currentVersion)
    ...
    if funcState then
      if v.Race then
        ...
        if findFlag then
          ...
          elseif self.curTab == 2 and v.Tab and v.Tab == 2 then
            TableUtility.TableShallowCopy(copy, v)
            TableUtility.ArrayPushBack(questversion, copy)
          end
        end
