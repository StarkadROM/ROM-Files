...
local tempTable, gemProxyIns, tickManager = {}, nil, nil
...
function GemFunctionPage:InitData()
  ...
  local funcStateId, param, index = GemProxy.BannedQualityFuncStateId, nil, nil
...
function GemFunctionPage:InitRight()
  ...
  self:AddClickEvent(helpBtn, function()
    self:OpenHelpView(Table_Help[self.helpId or 1005])
  end)
...
function GemFunctionPage:OnClickNeedGem(index)
  ...
  self.chooseSymbol:SetActive(index ~= nil and index > 0)
...
function GemFunctionPage:TryUpdateComposeMainLabel(isReady)
  ...
  if self.pageState == GemFunctionState.SameName then
    if not isReady or not self.selectedMaterialData[1] or not self.selectedMaterialData[1]:GetName() then
    end
    self.mainLabel.text = GemFunctionMainLabel[self.pageState]
  elseif self.pageState == GemFunctionState.RandomProf then
    ...
    if not isReady or not quality or not weight or not string.format(ZhString.Gem_QualityComposeTip, quality, weight) then
    end
    self.mainLabel.text = GemFunctionMainLabel[self.pageState]
...
function GemFunctionPage:CheckFunctionReady()
  ...
  local isReady, rowCount = false, nil
  if self.curMultiFunctionGO.activeSelf then
...
function GemFunctionPage:SetItemToSculpt(data)
  ...
  local needGem, pos = data.gemSkillData.needAttributeGemTypes, nil
