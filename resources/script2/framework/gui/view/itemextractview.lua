...
function ItemExtractView:InitView()
  ...
  table.sort(self.selectedItems, function(l, r)
    ...
    local lCfg, rCfg = self.exchangeCfg[rId] or self.exchangeCfg[lId] or 0, 0
    if lCfg ~= rCfg then
      return lCfg < rCfg
    end
    return lId < rId
...
function ItemExtractView:DoInputAnim(from, to)
  ...
  if not (to >= 200) or not 2 then
  end
  self.inputAnimFillAmount, self.inputAnimFillTo = from / 100 - math.floor(from / 100), to / 100 - math.floor(to / 100)
...
function ItemExtractView:DoExtractAnim(from, to)
  ...
  if not (from >= 200) or not 2 then
  end
  self.extAnimFillAmount, self.extAnimFillTo = from / 100 - math.floor(from / 100), to / 100 - math.floor(to / 100)
...
function ItemExtractView:UpdateProgressLabel(value, delta)
  ...
  self.progressLabel.text = string.format(format, value) .. (delta and delta > 0 and " [c][149b81]+" .. string.format(format, delta) .. "[-][/c]" or "")
end
