...
function MiyinStrengthen:CalcMaxUpgradeLv(currentlv, maxlv)
  local zenyenough, miyinenough, needmiyin, needzeny, need = true, true, 0, 0, nil
  local updatelevel = currentlv
  BagProxy.Instance:GetItemNumByStaticID(5030)
  local myROB, myMiyin = MyselfProxy.Instance:GetROB(), BagProxy.Instance:GetItemNumByStaticID(5030)
  while true do
    ...
    if zenyenough and miyinenough and not (maxlv < updatelevel) and not (myMiyin < needmiyin) and not (myROB < needzeny) then
    end
  end
