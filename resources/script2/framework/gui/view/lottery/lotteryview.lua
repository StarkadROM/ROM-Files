...
function LotteryView:CallLottery(price, year, month, times)
  FunctionSecurity.Me():OpenLottery(function()
    self:_CallLottery(price, year, month, times)
  end, arg)
end
function LotteryView:_CallLottery(price, year, month, times)
  local _LotteryProxy = LotteryProxy.Instance
  local skipType = _LotteryProxy:GetSkipType(self.lotteryType)
  local skipValue = _LotteryProxy:IsSkipGetEffect(skipType)
  _LotteryProxy:CallLottery(self.lotteryType, year, month, self.npcId, price, self.costValue, skipValue, times)
end
