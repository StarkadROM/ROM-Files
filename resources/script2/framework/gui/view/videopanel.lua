...
function VideoPanel:_launchVideo()
  ...
  local res
  if FileHelper.ExistFile(self.filePath) then
    res = self.videoPlayer:OpenVideo(self.filePath, true)
  else
    local path = VideoPanel.VideoPath .. self.filePath
    if FileHelper.ExistFile(path) then
      res = self.videoPlayer:OpenVideo(path, true)
    else
      res = self.videoPlayer:OpenVideo("Videos/" .. self.filePath, false)
    end
  end
  if not res then
    self:CloseSelf()
