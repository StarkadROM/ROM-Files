...
local clickDisablePredicate = function(gemCell, self)
  return self.mode == GemUpgradePageMode.SetMaterial and not GemProxy.CheckIsFavorite(gemCell.data) and GemProxy.CheckContainsGemAttrData(gemCell.data) and GemProxy.CheckContainsGemAttrData(self.targetCell.data) and gemCell.data.gemAttrData.type ~= self.targetCell.data.gemAttrData.type
end
function GemUpgradePage:UpdateList(noResetPos)
  ...
  self.itemContainer:SetActive(#gems > 0)
...
function GemUpgradePage:UpdateExpSlider(beforeAttrData, afterAttrData)
  ...
  if not (expDelta > 0) or not string.format(ZhString.Gem_UpgradeExpLabelUpgradeFormat, expDeltaFromCurLv, expToNextLv) then
  end
  self.expLabel.text = string.format(ZhString.Gem_UpgradeExpLabelCommonFormat, beforeAttrData.exp, expToNextLv)
...
function GemUpgradePage:RemoveOneTargetCellItemFromItemDatas(gems)
  ...
  local id, gem = self.targetCell.data.id, nil
  for i = 1, #gems do
    gem = gems[i]
    if id == gem.id then
...
function GemUpgradePage:TryAutoAddMaterial()
  ...
  for i = #gems, 1, -1 do
    ...
    if data then
      for j = 1, data.num do
        if not (#self.selectedMaterialData >= 20) then
          afterAttrData = self:GetGemAttrDataOfAfterUpdate(beforeAttrData)
        end
        if not afterAttrData:IsMax() then
          TableUtility.ArrayPushBack(self.selectedMaterialData, self:GetAttributeGemCloneData(data, 1))
        end
      end
    end
