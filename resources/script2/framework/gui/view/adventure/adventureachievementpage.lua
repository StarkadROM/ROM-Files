...
function AdventureAchievementPage:UpdateUserData()
  ...
  local newGenTime = "2030-01-01 05:00:00"
  local compareTime
  if EnvChannel.IsReleaseBranch() and bornMapConfig and bornMapConfig.releaseTime then
    newGenTime = bornMapConfig.releaseTime
  elseif bornMapConfig and bornMapConfig.TFTime then
    newGenTime = bornMapConfig.TFTime
  end
  local st_year, st_month, st_day, st_hour, st_min, st_sec = StringUtil.GetDateData(newGenTime)
  compareTime = os.time({
    day = st_day,
    month = st_month,
    year = st_year,
  ...
  local str = ""
  if createtime >= compareTime then
    str = ZhString.AdventureAchievePage_NewGenBornAndToulInfo
  elseif destProfession ~= 152 then
...
function AdventureAchievementPage:updateProfessionData()
  ...
  for i = 1, AdventureAchieveProxy.Instance.currentCgPfNum do
    ...
    if value and value > 0 and professMap[i] then
      local data = {
        name = professMap[i],
        time = value
      }
