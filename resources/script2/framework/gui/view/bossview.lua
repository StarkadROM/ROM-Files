...
function BossView:InitView()
  ...
  self.helpBtn:SetActive(FunctionUnLockFunc.checkFuncStateValid(116))
  ...
  self.helpBtn:SetActive(deadBossOpen)
...
function BossView:InitFakeToggles()
  ...
  RedTipProxy.Instance:RegisterUI(SceneTip_pb.EREDSYS_DEAD_BOSS, self.deadTogSp.gameObject, 26, {25, 20})
...
function BossView:UpdateBossInfo(data)
  ...
  if bossStaticdata.Type ~= 3 then
    ...
    if deadbossCfg then
      deadreward = deadbossCfg.Dead_Reward
    else
      return
    end
  end
  local dropItems = {}
  if deadreward then
    local funcStateId, bannedItemIds, id = 115, nil, nil
    bannedItemIds = FunctionNpcFunc.Me():CheckSingleFuncForbidState(funcStateId) and Table_FuncState[funcStateId] and Table_FuncState[funcStateId].ItemID
