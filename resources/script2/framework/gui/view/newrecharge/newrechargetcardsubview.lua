...
function NewRechargeTCardSubView:RegisterClickEvent()
  self:AddClickEvent(self.m_uiBtnHelp, function()
    helplog("Click HelpButton")
    self:OpenHelpView()
    local helpData = Table_Help[1002]
    if helpData then
...
function NewRechargeTCardSubView:UpdateMonthlyVIPLeftCanBuy()
  if not self.monthlyVIPShopItemConf or not self.monthlyVIPShopItemChargedCount or not self.monthlyVIPShopItemConf.MonthLimit then
    return
  end
  local monthLimit = self.monthlyVIPShopItemConf.MonthLimit
  if not (monthLimit > 999) or self.monthlyVIPShopItemConf.LimitType ~= 1 then
    self.u_LeftCanBuyNumber.gameObject:SetActive(false)
...
function NewRechargeTCardSubView:ShowCard_EP(isCover, cardIndex)
  ...
  if not (monthLimit > 999) or self.epCardConf.LimitType ~= 1 then
