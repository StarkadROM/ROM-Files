        ...
        if self.shopdata.changeCost then
          if total then
            ...
            for i = 1, limitCount do
              if not (total < finalPrice + self.shopdata:GetChangeCost(curCount + i)) then
...
function NewHappyShopBuyItemCell:shopNormal(data)
  ...
  if limitType == HappyShopProxy.LimitType.AccWeek then
    ...
    self.todayCanBuy.gameObject:SetActive(canBuyCount >= 0 and max > 0)
    if max > 0 then
  ...
  if shopdata then
    if shopdata.isDeposit then
      self.contextDatas[ItemTipAttriType.Code] = {
        label = string.format("%s：%s%s", ZhString.HappyShop_PriceTitle, shopdata.productConf.CurrencyType, StringUtil.NumThousandFormat(self:getRealPrice(), nil, true))
      }
    else
...
function NewHappyShopBuyItemCell:RealConfirm()
  if self.shopdata.isDeposit then
    self.m_depositBuyFunc()
    self:Cancel()
  else
    local flag, func = true, nil
