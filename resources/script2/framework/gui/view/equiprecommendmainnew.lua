...
function EquipRecommendMainNew:RecvProfessionQueryUserCmd(data)
  local datas, maxDepth, depth = data.items, 0, nil
  if not datas then
...
function EquipRecommendMainNew:RecEquipUserCmd(param)
  ...
  local count, isFind = #self.ERDatsFromServer, nil
...
function EquipRecommendMainNew:GetFatherTable()
  ...
  if forbidBranch then
    for i = 1, #forbidBranch do
      branchTable[forbidBranch[i]] = nil
    end
  end
  local fatherTable = {}
  ...
  for m, n in pairs(fatherTable) do
    if 0 < TableUtility.ArrayFindIndex(newnewclass, n.fatherGoal.id) then
      local id2add = n.fatherGoal.id
      for k = 1, #n.childGoals do
        if n.childGoals[k].id == id2add then
          id2add = nil
          break
        end
      end
      if id2add then
        table.insert(n.childGoals, Table_Class[id2add])
      end
    end
  end
...
function EquipRecommendMainNew:GetClassDataToShow(branchId)
  local myDepth, isSpecialBranch, job, branch = ProfessionProxy.GetJobDepth()
  ...
  local targetDepth = math.clamp(myDepth, isSpecialBranch and 3 or 2, 9999)
  for k, v in pairs(Table_Class) do
...
function EquipRecommendMainNew:ClickGoal(parama, isNotPlayAnim)
  ...
  elseif parama.child and parama.child.data then
    ...
    self.curProfessionId = self.goal
    local goalCells = self.goalListCtl:GetCells()
    local count, childCtls = #goalCells, nil
    for i = 1, count do
