...
function HeadwearRaidTowerUpgradeView:OnEnter()
  ...
  if not self.npcdata:GetSceneUI() then
    local sceneUI
  end
  if sceneUI then
  ...
  self.title.text = self.npcdata.data.staticData.NameZh
  self:UpdateInfo()
end
...
function HeadwearRaidTowerUpgradeView:AddEvents()
  self:AddListenEvt(ServiceEvent.NUserHeadwearTowerUserCmd, self.UpdateInfo)
...
function HeadwearRaidTowerUpgradeView:SetLv(info)
  ...
  self.title.text = self.npcdata.data.staticData.NameZh .. " Lv." .. info.level
end
