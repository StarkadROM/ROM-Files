...
function PlayerBottomView:AddViewEvents()
  ...
  self:AddListenEvt(ServiceEvent.NUserHeadwearTowerUserCmd, self.HandleHeadwearRaidTowerInfoChange)
  self:AddListenEvt(ServiceEvent.NUserHeadwearRoundUserCmd, self.HandleHeadwearRaidRoundInfoChange)
end
...
function PlayerBottomView:HandleHeadwearRaidTowerInfoChange(note)
  if note and note.body and note.body.npcid then
    local npcs = NSceneNpcProxy.Instance:FindNpcs(note.body.npcid)
    if npcs and next(npcs) then
      if not npcs[1]:GetSceneUI() then
        local sceneUI
      end
      if sceneUI then
        sceneUI.roleBottomUI:UpdateHeadwearRaidTowerInfo(npcs[1])
      end
    end
  end
end
function PlayerBottomView:HandleHeadwearRaidRoundInfoChange(note)
  if not HeadwearRaidProxy.Instance.refreshCrystal then
    return
  end
  local towers = HeadwearRaidProxy.Instance.towers
  for k, _ in pairs(towers) do
