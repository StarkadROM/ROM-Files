...
function SetView:OnEnter()
  ...
  local setViewConfig = UnionConfig and UnionConfig.SetViewPages
...
function SetView:SetGameTime(data)
  local timeLen, timeTotal, musicTime, tutorTime, powerTime, color = 0, 0, 0, 0, 0, 1
  ...
  if data.powertime then
    powerTime = math.floor(data.powertime / 60)
  end
  self.gameTime.text = string.format(ZhString.Set_GameTime, string.format(BattleTimeStringColor[color], timeLen), timeTotal)
  ...
  self.playTime.text = string.format(ZhString.SetView_PlayTime, playTimeLen, playTotalTimeLen)
  self.playTimeSlider.value = 0
  if playTotalTimeLen > 0 then
...
function SetView:Save()
  ...
  ServiceSceneUser3Proxy.Instance:CallBattleTimeCostSelectCmd(self.setViewSystemState.gameTimeSetting)
end
...
function SetView:AddHelpButtonEvent()
  ...
  self:AddClickEvent(self.helpButton, function(go)
    local id1 = GameConfig.Setting.GametimeHelpId
    local id2 = GameConfig.Setting.PlaytimeHelpId
    if id1 and id2 then
      self:ShowHelpsView({
        Table_Help[id1],
        Table_Help[id2]
...
function SetView:ShowHelpsView(datas)
