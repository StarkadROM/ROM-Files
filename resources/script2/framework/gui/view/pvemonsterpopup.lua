...
function PveMonsterPopUp:OnEnter()
  ...
  if viewdata then
    self.monsterID = viewdata.monsterID
    self.previewData = Table_PveMonsterPreview[viewdata.monsterID]
    if not self.previewData then
      redlog("未找到 Table_PveMonsterPreview id ： ", viewdata.monsterID)
      return
    end
  end
...
function PveMonsterPopUp:TabChangeHandler(key)
  ...
  elseif PanelConfig.PveIntroductionPage.tab == key and not self.introdutionPage then
