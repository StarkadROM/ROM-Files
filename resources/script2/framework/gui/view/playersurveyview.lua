...
function PlayerSurveyView:InitShow()
  ...
  local descHelp = Table_Help[surveyConfig.DesID]
  if descHelp then
    self.mainTip1.text = descHelp.Desc
  end
  local tipsDesc = Table_Help[surveyConfig.HelpID]
  if tipsDesc then
    self.rule1Label.text = tipsDesc.Desc
  end
  local rewardList = {}
...
function PlayerSurveyView:UpdateBtns(index, canSubmit)
  ...
  elseif 1 < self.totalQuestionNum and index > 1 then
