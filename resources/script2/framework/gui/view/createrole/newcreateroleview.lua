...
function NewCreateRoleView:DoChangeHair(index_delta)
  ...
  if curHair < 1 then
    curHair = len + curHair or curHair
  end
  if len < curHair then
    curHair = curHair - len or curHair
  end
...
function NewCreateRoleView:DoChangeHairColor(index_delta)
  ...
  if curHairColor < 1 then
    curHairColor = len + curHairColor or curHairColor
  end
  if len < curHairColor then
    curHairColor = curHairColor - len or curHairColor
  end
...
function NewCreateRoleView:TweenToSelectRole()
  if self.maskFadeLeanTween or not self.mask or self.enter_tryed == true then
    return
  end
  local tweenStart = 0
  local tweenEnd = 1
...
function NewCreateRoleView:OnProfRouletteRotate(angle)
  ...
  if angle ~= self.lastRouletteAngle then
    ...
    if inPage > 0 and self.lastInPage ~= inPage then
