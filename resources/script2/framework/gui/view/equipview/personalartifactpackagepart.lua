...
function PersonalArtifactPackagePart:InitPart()
  ...
  self.listCtrl = WrapListCtrl.new(self.itemContainer, BagItemCell, "BagItemCell", WrapListCtrl_Dir.Vertical, 4, 102, true)
...
function PersonalArtifactPackagePart:UpdatePage(bagType, noResetPos)
  self.currentBagType = bagType or self.currentBagType or SceneItem_pb.EPACKTYPE_ARTIFACT_FLAGMENT
...
function PersonalArtifactPackagePart:UpdateInfo(noResetPos)
  self:UpdatePage(self.currentBagType or SceneItem_pb.EPACKTYPE_ARTIFACT_FLAGMENT, noResetPos)
end
...
function PersonalArtifactPackagePart:ShowItemTip(cellGO, data)
  ...
  tipOffset[1] = x > 0 and -650 or 190
