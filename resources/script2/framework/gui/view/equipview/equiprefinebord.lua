...
function EquipRefineBord:ClickRepair()
  ...
  if matdata.id == "None" then
    if self:TryOpenQuickBuyView(matSId, self.repair_matCell.needNum or 1) then
      return
    end
  end
  ...
  elseif EQUIP_REPAIR_MATERIAL_CFG[matSId] then
    local nowDataType, typeCfg = nowData.staticData.Type, nil
...
function EquipRefineBord:UpdateRepairMaterial(itemData)
  ...
  if itemData == nil then
    ...
    local sId, nowSData = nowData and itemData.staticData.id, nowData.staticData
  ...
  if self.isFashion then
    self.repair_matCell:SetShowRedTip(neednum ~= nil and materialEquipsCount ~= nil and materialEquipsCount > 0)
  end
  if self.repairTipBord then
...
function EquipRefineBord:UpdateNextGenRepairMaterial()
  ...
  if repairCfg and repairCfg[1] then
    ...
    self.repair_matdata = ItemData.new(curNum > 0 and MaterialItemCell.MaterialType.Material or "None", repairMatId)
