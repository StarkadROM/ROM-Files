...
function TransferProfessionView:SetPolygon(classid)
  ...
  if initialAttr and #initialAttr > 0 then
    local v = {}
...
function TransferProfessionView:SetRightTransferDesc(classid)
  ...
  if not config.TransferDesc then
    local transferDesc = {
      "Tag1-Tag2-Tag3-Tag4",
      "No Desc."
    }
...
function TransferProfessionView:UpdateSubClassByGroup(groupid)
  ...
  if _subClassList and #_subClassList > 0 then
...
function TransferProfessionView:TryRunQuestJump(jumpId)
  ...
  if questData then
    ...
    if jumpList and #jumpList > 0 and jumpList[jumpId] then
