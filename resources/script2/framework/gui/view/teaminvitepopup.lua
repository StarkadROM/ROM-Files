  ...
  if cellCtl then
    ...
    elseif cellCtl.eventType == "Invite" then
      ...
      if data.type == TeamInviteMemberType.MemberCat then
        ...
        if not teamProxy:IsInMyGroup(data.id) then
          ServiceSessionTeamProxy.Instance:CallInviteMember(data.data.masterid, data.data.cat, nil, nil, self.isOperateUniteGroupTeam)
        end
      elseif not teamProxy:IsInMyGroup(data.id) then
        ServiceSessionTeamProxy.Instance:CallInviteMember(data.id, nil, nil, nil, self.isOperateUniteGroupTeam)
        cellCtl:ActiveInviteButton(false)
      end
