...
function EquipComposeView:SelectFirstMat()
  ...
  if not cells or not (#cells > 1) or not cells[2] then
    local cell
  end
...
function EquipComposeView:ChooseItem(item)
  ...
  if #cardids > 0 then
    local recoverCost = EquipUtil.GetRecoverCost(item, true, false, false, false, false)
    MsgManager.ConfirmMsgByID(26101, function()
      ServiceItemProxy.Instance:CallRestoreEquipItemCmd(item.id, false, cardids, false, false, false)
