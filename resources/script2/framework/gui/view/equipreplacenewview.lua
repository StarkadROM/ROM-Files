...
function EquipReplaceNewView:InitView()
  ...
  self:AddButtonEvent("ActionBtn", function()
    ...
    if self:IsTargetNextGen() then
      ...
      local hasEnchant, data = false, nil
...
local updateLackItems_NextGen = function(lackItems, datas)
  local count, matData, num, neednum, lackData = 0, nil, nil, nil, nil
  for i = 1, #datas do
    matData = datas[i]
...
function EquipReplaceNewView:CheckZeny()
  local myZeny, robCfg = getComposeCfgFromItemByField(self.targetData, "ROB") or MyselfProxy.Instance:GetROB(), 0
...
function EquipReplaceNewView:IsTargetNextGen()
  return not self.targetData or not self.targetData.equipInfo or self.targetData.equipInfo:IsNextGen()
end
