...
function EquipRecoveryView:UpdateReward()
  ...
  local rewardCfg, item = self.recoverySData and self.recoverySData.RewardItem or _EmptyTable, nil
...
function EquipRecoveryView:CheckNeedRecover()
  ...
  local equipInfo, equipedCards, result, recoverCost = item.equipInfo, item.equipedCardInfo, false, nil
  if equipInfo then
...
function EquipRecoveryView:OpenHelpView()
  local data = Table_Help[self.isPlus and 35084 or 35085]
  if data then
