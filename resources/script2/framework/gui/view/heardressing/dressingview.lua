...
autoImport("HairPage")
autoImport("EyePage")
local _shopType, _shopID, _hairCost, _eyeCost, _ShopDressProxy
...
function DressingView:InitUIView()
  ...
  if not GameConfig.HairEye then
    local costConfig = {HairCost = 2000, EyeCost = 3000}
  end
...
function DressingView:RefreshSelectedROB(type, precost, hairID, eyeID)
  ...
  local enoughZeny, activeEnoughZeny, isSame
  if type == 1 then
    self.chargeNum.text = _hairCost
    ...
    hasTicket = _bHasTicket(precost)
    if hairID then
      activeEnoughZeny = enoughZeny and _ShopDressProxy:bActived(hairID, _DressType[1]) or false
      isSame = _ShopDressProxy:IsSame(_DressType[1])
    else
      activeEnoughZeny = enoughZeny
      isSame = _ShopDressProxy:IsSame(_DressType[2])
    end
    self:SetReplaceBtnState(not isSame and (hasTicket or activeEnoughZeny))
  elseif type == 2 then
    self.chargeNum.text = _eyeCost
    ...
    activeEnoughZeny = enoughZeny and _ShopDressProxy:bActived(eyeID, _DressType[3]) or false
    isSame = _ShopDressProxy:IsSame(_DressType[3])
    self:SetReplaceBtnState(not isSame and activeEnoughZeny)
  end
end
...
function DressingView:RefreshModel()
  ...
  if bodyType ~= nil then
    ...
    elseif t == "string" and string.find(bodyType, "hf_") then
      ...
      if eyeIndex and eyeIndex > 4 and tempGender ~= realGender then
...
function DressingView:ObserverEvent(role, param)
  ...
  local evt, partObj = param[1], param[2]
