...
function PersonalArtifactComposeView:OnEnter()
  ...
  if #self.formulaListCells > 0 then
    ...
    if childCells and #childCells > 0 then
...
function PersonalArtifactComposeView:OnClickMaterialCell(index)
  ...
  if BagItemCell.CheckData(data) then
    ...
    self:ShowItemTip(#mats > 0 and mats[1] or data.staticData.id)
  end
end
...
function PersonalArtifactComposeView:OpenHelpView()
  local data = Table_Help[35043]
  if data then
