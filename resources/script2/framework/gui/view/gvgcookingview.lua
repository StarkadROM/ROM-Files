...
function GVGCookingView:FindObjs()
  ...
  local star_point = GameConfig_GvgRimConfig.star_point
...
function GVGCookingView:InitEvents()
  ...
  self:AddClickEvent(self.m_uiImgBtnHelp, function(go)
    local helpData = Table_Help[35250]
  ...
  self:AddClickEvent(self.m_uiImgBtnIngredients, function(go)
    ...
    if self.costItemId then
      local num = BagProxy.Instance:GetItemNumByStaticID(self.costItemId, CookBagTypes)
...
function GVGCookingView:onClickEat(go)
  if self.lastEatenTime then
    ...
    local CONFIGCD = GameConfig_GvgRimConfig.eat_cd or 60
    if CONFIGCD > cd / 1000 then
      MsgManager.ShowMsgByID(GameConfig_GvgRimConfig.eat_msg)
      return
    end
  end
  ...
  MsgManager.ShowMsgByID(GameConfig_GvgRimConfig.eat_success_msg or 43185)
...
function GVGCookingView:UpdateOptNum()
  local heatTotalCount = GameConfig_GvgRimConfig.heat_count or 40
  ...
  local seasoningTotalCount = GameConfig_GvgRimConfig.seasoning_count or 40
  ...
  local ingredientsTotalCount = GameConfig_GvgRimConfig.ingredients_count or 40
...
function GVGCookingView:ShowData()
  ...
  self.m_uiTxtFire.text = info.m_heat .. "/" .. GameConfig_GvgRimConfig.HeatPoint
  self.m_uiTxtIngredients.text = info.m_ingredients .. "/" .. GameConfig_GvgRimConfig.IngredPoint
  self.m_uiTxtSeasoning.text = info.m_seasoning .. "/" .. GameConfig_GvgRimConfig.SeasoningPoint
  ...
  self.costItemNum = GameConfig_GvgRimConfig.IngredientsNum
...
function GVGCookingView:GetPointMsgId(point, opt)
  local msgTable
  if point > 1 then
    msgTable = GameConfig_GvgRimConfig.point_addmsg
  else
    msgTable = GameConfig_GvgRimConfig.point_failmsg
  end
  if msgTable then
    if opt == 1 then
...
function GVGCookingView:CanEaten()
  ...
  if gvgStartTime then
    local eat_begin_time = GameConfig_GvgRimConfig.eat_begin_time or 1800
    ...
    for i = 1, #gvgStartTime do
      if not gvgStartTime[i].super and curWDay == gvgStartTime[i].day then
        local h, m = gvgStartTime[i].min or gvgStartTime[i].hour or 0, 0
...
function GVGCookingView:OnEnter()
  ...
  if trans ~= nil then
    if not GameConfig_GvgRimConfig.CameraViewPort then
      local viewPort = {
        0.62,
        0.3,
        14
