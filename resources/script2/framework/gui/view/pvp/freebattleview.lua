...
function FreeBattleView:FindObjs()
  ...
  self.roomListBtnGO = self:FindGO("RoomListBtn")
end
...
function FreeBattleView:AddBtnEvts()
  ...
  EventDelegate.Add(self.free6v6ModelTog.onChange, function()
    if self.free6v6ModelTog.value then
      self.roomID, self.pwsConfig = next(pwsCfg)
      T_PVP_TYPE = PvpProxy.Type.FreeBattle
    else
  ...
  self:AddClickEvent(self.onlyMatchMyServerTip, function()
    local Desc = Table_Help[101] and Table_Help[101].Desc or ZhString.Help_RuleDes
    TipsView.Me():ShowGeneralHelp(Desc)
  end)
  if self.createRoomBtnGO then
    self.createRoomBtnGO:SetActive(true)
    self:AddClickEvent(self.createRoomBtnGO, function()
      local roomData = PvpCustomRoomProxy.Instance:GetCurrentRoomData()
      if roomData then
        if roomData.pvptype ~= PvpProxy.Type.FreeBattle then
  ...
  if self.roomListBtnGO then
    self.roomListBtnGO:SetActive(true)
...
function FreeBattleView:UpdateCustomRoomButton()
  ...
  if myRoomData and myRoomData.pvptype == PvpProxy.Type.FreeBattle then
