...
function NewRechargeRecommendTShopGoodsCell:SetCell_Deposit()
  ...
  if self.info.purchaseState == 0 then
    self.isForbidPurchase = true
    if self.info.batch_is_Batch and 1 < self.info.batch_DailyCount then
      ...
      self.u_itemPrice.text = ZhString.NewRechargeRecommendTShopGoodsCell_OnEffect
    elseif self.info.batch_is_Origin and self.info.purchaseLimitStr then
  ...
  elseif self.info.purchaseState == 1 and self.info.batch_is_Batch and self.info.batch_DailyCount == 1 then
    self:Set_BuyTimesMark(true, ZhString.NewRechargeRecommendTShopGoodsCell_FeedAgain)
    self.u_itemPrice.text = ZhString.NewRechargeRecommendTShopGoodsCell_OnEffect
  end
...
function NewRechargeRecommendTShopGoodsCell:SetCell_Shop()
  ...
  local hasPurchaseLimit = purchaseLimitTimes > 0 and purchaseLimitTimes < noLimitTime
  ...
  if canBuyCount then
    if limitType == HappyShopProxy.LimitType.AccWeek then
      if self.info.shopItemData.accMaxBuyLimitNum ~= 0 and self.info.shopItemData.accAreadyBuyCount <= self.info.shopItemData.accMaxBuyLimitNum then
        ...
        self:Set_BuyTimesMark(true, string.format(ZhString.NoviceShop_BuyLimit, canBuyCount, self.info.shopItemData.accMaxBuyLimitNum))
      end
    else
      local max = 0
      ...
      if canBuyCount >= 0 then
        self:Set_BuyTimesMark(true, string.format(ZhString.NoviceShop_BuyLimit, max - canBuyCount, max))
      end
    end
  else
    self:Set_BuyTimesMark(false, "")
...
function NewRechargeRecommendTShopGoodsCell:showRedTip()
  if self.u_fxxk_redtip then
    ...
    if GameConfig.ShopRed and GameConfig.ShopRed.Shopid and 0 ~= TableUtility.ArrayFindIndex(GameConfig.ShopRed.Shopid, shopId) then
      local pre_buyCount = HappyShopProxy.Instance:GetCachedHaveBoughtItemCount(shopId) or 0
