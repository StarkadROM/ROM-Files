...
function ItemTipModelCell:adjustCardViewLockView()
  ...
  self.getPathBtn.transform.localPosition = LuaGeometry.GetTempVector3(102, -61, 0)
...
function ItemTipModelCell:adjustModelViewLockView()
  ...
  self.getPathBtn.transform.localPosition = LuaGeometry.GetTempVector3(182, -45.9, 0)
  NGUITools.AddWidgetCollider(self.center, true)
...
function ItemTipModelCell:GetItemDetail()
  ...
  local refineStoreAttrLabelText = AdventureDataProxy.Instance:getIntoPackageRefineRewardStr(self.data.staticData, nil, self.data.store, self:GetRefineLv(), "[808080ff]")
  if refineStoreAttrLabelText then
    for i = 1, #refineStoreAttrLabelText do
      table.insert(temp.label, refineStoreAttrLabelText[i])
    end
  end
  local unlockCondition = AdventureDataProxy.getUnlockCondition(self.data)
  local rewardStr = AdventureDataProxy.Instance:getUnlockRewardStr(self.data.staticData)
...
function ItemTipModelCell:GetItemUnlock()
  ...
  if self.data.staticData.AdventureValue and self.data.staticData.AdventureValue > 0 then
    local temp = {
      label = AdventureDataProxy.getUnlockCondition(self.data) .. "，" .. "{uiicon=Adventure_icon_05}x" .. self.data.staticData.AdventureValue,
        ...
        if rewardItemsInfo then
          for _, singleRewardInfo in pairs(rewardItemsInfo) do
            if singleRewardInfo.type == 1 then
              ...
              if itemSData then
                temp.label = string.format("%s%s%sx%s", temp.label, ZhString.ItemTip_ChAnd, itemSData.NameZh, singleRewardInfo.num)
              end
            end
          end
        end
    ...
    temp.label = temp.label .. "[-][/c]"
    if not self.isUnlock then
      temp.label = "[c][808080ff]" .. temp.label
      temp.tiplabel = "[c][808080ff]" .. temp.tiplabel .. "[-][/c]"
    end
    temp.hideline = true
    return temp
  end
...
function ItemTipModelCell:UpdateSkillContext()
  local contextDatas, itemData, store = {}, nil, nil
  if self.curSelectGroupItemCell then
  ...
  if itemData:IsMount() then
    ...
    if equipSData and equipSData.UniqueEffect and equipSData.UniqueEffect.buff then
      local equipBuffs, buffEffect = equipSData.UniqueEffect.buff, nil
...
function ItemTipModelCell:UpdateTopInfo(data)
  ...
  if data then
    ...
    self.ItemName.text = (refinelv > 0 and "+" .. refinelv or "") .. data:GetName()
  end
end
...
function ItemTipModelCell:UpdateTopModel()
...
function ItemTipModelCell:Show2DCell()
  ...
  if cardInfo and self.cardCell then
    ...
    self.cardCell:SetChooseState(true)
...
function ItemTipModelCell:ShowFashionPreview()
  ...
  if (self.data:IsWeapon() or self.data:IsShield()) and not self.data.equipInfo:CanUseByProfess(prosfession) and self.data.equipInfo.professCanUse ~= nil then
    prosfession = next(self.data.equipInfo.professCanUse) or prosfession
  end
  if self.sfp then
    self.sfp:OnExit()
  end
