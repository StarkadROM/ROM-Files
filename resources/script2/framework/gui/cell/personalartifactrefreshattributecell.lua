...
local delimiter, sliderFullLength, ins, tickManager, strfmt, abs, round, epsilon = "＋", 352, nil, nil, nil, nil, nil, nil
...
function PersonalArtifactRefreshAttributeCell:SetData(data)
  ...
  if isUniqueEffect then
    ...
    for i = 1, #eff do
      sb:Append(Table_Buffer[eff[i]].Dsc)
      if i < #eff then
        sb:Append("\n")
      end
    end
...
function PersonalArtifactRefreshAttributeCell:UpdateForeSlider(v)
  ...
  local _, valueColor = ColorUtil.TryParseHexString(ratio + epsilon >= 0.8 and "A3F46D" or "F4D86D")
...
function PersonalArtifactRefreshAttributeCell:UpdateCompareResult(curValue, targetValue)
  ...
  local flag = curValue and targetValue and targetValue - curValue > 0
