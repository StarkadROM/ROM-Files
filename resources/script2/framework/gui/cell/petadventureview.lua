...
if not GameConfig.PackageMaterialCheck.pet_workspace then
  local PACKAGE_MATERIAL = {
    1,
    7,
    9
  }
...
function PetAdventureView:FindObjs()
  ...
  if UnionConfig and UnionConfig.PetAdventureView and UnionConfig.PetAdventureView.PetAdventureVolume and UnionConfig.PetAdventureView.PetAdventureVolume == 1 then
...
function PetAdventureView:AddEvts()
  ...
  self:AddClickEvent(rewardTipBtn, function(g)
...
function PetAdventureView:UpdatePetQuestInfo(data)
  ...
  if self:UseTicketValid() then
    self.consumeFightTimeLab.text = ZhString.PetAdventure_NoConsumeFightTime
  else
