...
function QuickUsePopupFuncCell:Init()
  ...
  self.interactiveIconGO = self:FindGO("InteractiveIcon", self.interactiveStyle)
  self.interactiveIcon = self.interactiveIconGO:GetComponent(UISprite)
...
function QuickUsePopupFuncCell:SkillEnableCheck()
  ...
  if newEnable then
    self:SetTextureWhite(self.quickUseBtn, LuaGeometry.GetTempColor(0.6196078431372549, 0.33725490196078434, 0, 1))
...
function QuickUsePopupFuncCell:TryClose()
  ...
  if self.type == QuickUseProxy.Type.Skill then
    if not self:CanUseSkillById(self.data and self.data.skillid) then
      return
    end
  end
