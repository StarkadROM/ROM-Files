...
function TaskQuestCell:SetNewTag()
  if not self.data then
    return
  end
  local questType = self.data.type
  if questType == "world" or questType == "worldboss" or questType == "acc_world" or questType == "acc_daily_world" then
    local isOver = LocalSaveProxy.Instance:GetWorldQuestTrace(self.data.id)
    if isOver ~= nil then
      self.newTag:SetActive(not isOver)
    end
  else
