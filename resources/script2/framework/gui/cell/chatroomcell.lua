...
function ChatRoomCell:FindObjs()
  ...
  self.bgDecorate = self:FindGO("bgDecorate")
  if self.bgDecorate then
    self.bgDecorate_Icon = self.bgDecorate:GetComponent(UISprite)
  end
end
  ...
  if data ~= nil then
    ...
    if hasPhoto then
      ...
      if self.voiceid ~= 0 and self.voicetime ~= 0 then
        ...
        if not BranchMgr.IsChina() and not BranchMgr.IsTW() or BackwardCompatibilityUtil.CompatibilityMode_Vspeech then
