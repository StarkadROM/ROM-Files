...
function TeamMemberCell:SetData(data)
  ...
  elseif data ~= nil then
    ...
    if self.data.id == Game.Myself.data.id then
      ...
      if self.data.role == 0 or not self.data.role then
      end
      self.roleid = MyselfProxy.Instance:GetMyDefaultTeamRole()
      ...
      self.roleid = ProfessionProxy.Instance:GetDefaultTeamRoleByPro(self.data.profession)
      self.roleCell:SetData(self.roleid)
      self.roleCell:SetScale(0.45, 0.4)
    end
...
local helpSet = function(parts, index, dataValue, usedebug)
  ...
  if nil ~= maskBit and bodyID and bodyID > 0 then
...
function TeamMemberCell:UpdateFollow()
  ...
  if self.data:IsHireMember() or self.data:IsRobotMember() then
    self.following:SetActive(false)
    self.inviteFollow:SetActive(false)
    return
  end
