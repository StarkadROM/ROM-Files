...
function ExchangeItemCell:SetFeedData()
  ...
  local own = ExchangeShopProxy.Instance:GetOwnCount(staticId)
  if 0 == own then
...
function ExchangeItemCell:UpdateOwn()
  ...
  if 0 == own then
    ...
    if previewNum == 0 then
      ...
      if not (own < previewNum) or not string.format(redColor, previewNum) then
      end
      self.choosenLab.text = string.format(greenColor, previewNum)
    end
