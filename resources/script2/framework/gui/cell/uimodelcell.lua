...
function UIModelCell:SetTexture(uiTexture)
  ...
  if self.uiTexture == uiTexture and not IsNull(self.renderTex) and uiTexture.mainTexture == self.renderTex and self.renderTex.width == uiTexture.width and self.renderTex.height == uiTexture.height then
    return
  end
  self.uiTexture = uiTexture
  local uiTextureGO = uiTexture.gameObject
  local luaMono = uiTextureGO:GetComponent(GameObjectForLua)
  if luaMono == nil then
    luaMono = uiTextureGO:AddComponent(GameObjectForLua)
  end
...
function UIModelCell:SetRolePartModelTexture(uiTexture, partIndex, id, cameraConfig, scale, callback, callbackArg)
  scale = scale or 1
  ...
  end, callbackArg, Asset_RolePart.SkinQuality.Bone4)
...
function UIModelCell:SetRoleModelTexture(uiTexture, parts, cameraConfig, scale, isPreviewMount, isStatic, suffixMap, callback, callbackArg)
...
function UIModelCell:SetFurnitureModelTexture(uiTexture, furnitureSID, cameraConfig, callback)
  ...
  Asset_Furniture.new(furnitureSID, self.container.transform, function(model)
    ...
    if self.model and self.model.gameObject then
      ...
      if posConfig and #posConfig > 2 then
