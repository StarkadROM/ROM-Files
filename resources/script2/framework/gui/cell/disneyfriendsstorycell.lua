  ...
  if data.lock then
    ...
    if data.read then
      ...
      if not menuId then
        ...
        if not menuUnlock or not LuaGeometry.GetTempVector4(0.17647058823529413, 0.17254901960784313, 0.2784313725490196, 1) then
        end
        self.storyLabel.color = LuaGeometry.GetTempVector4(0.5725490196078431, 0.596078431372549, 0.7333333333333333, 1)
      end
    else
      self.unlockPart:SetActive(false)
      self.statusIcon:SetActive(false)
