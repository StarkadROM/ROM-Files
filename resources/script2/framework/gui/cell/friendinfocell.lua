...
function FriendInfoCell:AddButtonEvt()
  ...
  self:AddClickEvent(teamBtn, function()
    if self:CheckInviteMember() then
      self:InviteMember()
    end
  end)
...
function FriendInfoCell:InviteMember()
  if Game.MapManager:IsPVPMode_TeamPws() then
    MsgManager.ShowMsgByIDTable(25930)
    return
  end
  local isInGroup = TeamProxy.Instance:IsInMyGroup(self.guid)
  if not isInGroup then
    ServiceSessionTeamProxy.Instance:CallInviteMember(self.guid)
  else
