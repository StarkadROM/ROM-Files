...
function TeamInfoPortrait:OnClickRankMember()
  ...
  if FriendProxy.Instance:IsFriend(self.data.id) then
    if self.data and not self.data.offline then
    else
      funckeys = playerTipFunc_Friend or playerTipFunc_Friend_Offline
    end
  elseif self.data and not self.data.offline then
  else
    funckeys = playerTipFunc or playerTip_Offline
  end
