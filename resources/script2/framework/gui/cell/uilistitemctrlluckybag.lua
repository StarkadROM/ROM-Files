...
function UIListItemCtrlLuckyBag:GetGameObjects()
  ...
  self:AddClickEvent(self:FindGO("hBtn", self.goDepositOriginBatchPurchase), function()
    local data = Table_Help[batchHelpTip]
    if data then
    ...
    if productID then
      ...
      if roleID then
        ...
        if serverID then
          ...
          isPayCondition1 = BranchMgr.IsChina() and runtimePlatform == RuntimePlatform.Android
          ...
          elseif not BranchMgr.IsChina() and FunctionSDK.Instance.CurrentType == FunctionSDK.E_SDKType.TXWY then
            TableUtility.TableClear(gReusableTable)
...
function UIListItemCtrlLuckyBag:SetOriginBatchPurchaseBtnGroup()
  ...
  local alreadyBought = batchDailyCount and batchDailyCount > 0 or self.purchaseTimes ~= nil and 0 < self.purchaseLimit_N and self.purchaseTimes >= self.purchaseLimit_N
  ...
  if batchDailyCount and batchDailyCount > 0 then
    if batchDailyCount > 1 then
