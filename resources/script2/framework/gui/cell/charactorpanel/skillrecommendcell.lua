...
function SkillRecommendCell:Init()
  ...
  self.basePoint = self:FindGO("basePoint"):GetComponent(UILabel)
  ...
  if GameConfig.SystemForbid.OpenServantEquipRecommend then
...
function SkillRecommendCell:SetData(data)
  if data then
    ...
    if solutionData then
      ...
      self.basePoint.text = solutionData.RecAddPoint
