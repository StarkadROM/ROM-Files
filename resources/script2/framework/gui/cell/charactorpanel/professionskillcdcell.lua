...
function ProfessionSkillCDCell:RefreshCD(f)
  ...
  if self.updateCdLb then
    if self.data and self.skillEnable then
      ...
      if not (curCd <= 0) or not ZhString.AdventurePanel_UseEffectSkill_CanUse then
      end
      self.updateCdLb.text = string.format(ZhString.SkillTip_CDTime.normal, math.ceil(curCd))
    else
