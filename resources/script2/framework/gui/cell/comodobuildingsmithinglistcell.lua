...
local countdownTickIdBase, shopIns, buildingIns, tickManager, config = 395, nil, nil, nil, nil
...
function ComodoBuildingSmithingListCell:SetData(data)
  ...
  if isUnlocked then
    ...
    local cost, c, item = config.ManorForgeItemCost[self.data], nil, nil
    for i = 1, #cost do
      c = cost[i]
...
function ComodoBuildingSmithingListCell:CheckMats()
  ...
  local cost, enough, id, neednum, num = config.ManorForgeItemCost[self.data], true, nil, nil, nil
  for i = 1, #cost do
...
function ComodoBuildingSmithingListCell:UpdateCountdown()
  ...
  self.working:SetActive(countdown > 0)
  if not (countdown >= 0) or not string.format(ZhString.ComodoBuilding_SmithingCountdownFormat, self:MakeTimeStr(countdown)) then
  end
  self.desc.text = config.ManorForgeDesc[self.data].Name
end
...
function ComodoBuildingSmithingListCell:OnDestroy()
