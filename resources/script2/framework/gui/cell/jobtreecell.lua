    ...
    do
      ...
      if #advClass == 1 then
        ...
        for k, v in pairs(Table_Class) do
          if v.OriginId == self.originid and not ProfessionProxy.Instance:CheckProfIsBanned(v.id) then
      ...
      elseif #advClass == 2 then
        ...
        for i = 1, #advClass do
          ...
          for k, v in pairs(Table_Class) do
            if v.OriginId == originID and not ProfessionProxy.Instance:CheckProfIsBanned(v.id) then
...
function JobTreeCell:UpdateThisCellLineState()
  ...
  elseif #advClass == 2 then
    ...
    if #advClass >= 3 and TableUtility.ArrayFindIndex(ProfessionProxy.MultiSexJob1st, data.id) > 0 then
      local rightTable = self:GetRightUpgradeTableForMultiSexJob(data.id)
