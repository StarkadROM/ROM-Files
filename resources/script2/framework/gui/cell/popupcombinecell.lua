local _SpriteIconMap = {
  [1] = {
    [0] = "",
    [1] = "{cardicon=mall_twistedegg_card_01}",
    [2] = "{cardicon=mall_twistedegg_card_02}",
    [3] = "{cardicon=mall_twistedegg_card_03}",
    [4] = "{cardicon=mall_twistedegg_card_04}",
    [5] = "{cardicon=mall_twistedegg_card_05}"
  }
}
...
local _GetFilter = function(config, firstKey, spriteIcon_type)
  ...
  for k, v in pairs(config) do
    local single = {}
    single.id = k
    if spriteIcon_type and _SpriteIconMap[spriteIcon_type] then
...
function PopupCombineCell:SetData(config, reset, spriteIcon_type)
  ...
  if data.fatherGoal then
    ...
    if data.childGoals and #data.childGoals > 0 then
      ...
      self.childBg.height = 20 + self.childSpace * #data.childGoals
    else
