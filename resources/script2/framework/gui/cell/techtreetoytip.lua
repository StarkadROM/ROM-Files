...
function TechTreeToyTip:InitCountCtrl()
  ...
  local longPress = minusBtn:GetComponent(UILongPress)
  if longPress then
  ...
  longPress = addBtn:GetComponent(UILongPress)
  if longPress then
    function longPress.pressEvent(obj, state)
...
function TechTreeToyTip:SetDesc()
  local desc = not OverSea.LangManager.Instance():GetLangByKey(self.staticData.Desc) and ""
