...
local myselfIns
function MyselfEquipItemShareCell:ctor(obj, index, isfashion)
...
function MyselfEquipItemShareCell:UpdateExtraInfo()
  ...
  if extra then
    ...
    if not self.data or not self.data.equipBuffUpSource then
      local equipBuffUpSource
    end
    if enchantInfo then
    ...
    if cardSlotNum and cardSlotNum > 0 then
...
function MyselfEquipItemShareCell:UpdateTwelveInfo()
