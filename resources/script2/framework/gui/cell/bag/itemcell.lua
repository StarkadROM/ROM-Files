...
function ItemCell:SetIcon(data)
  ...
  if itemType == 1200 then
    ...
    setFaceSuc = setFaceSuc or IconManager:SetFaceIcon("boli", self.icon)
  else
...
function ItemCell:SetLimitTimeCorner(data)
  ...
  self:SetShowCellPart("LimitTimeCorner", existTimeType == 1 or existTimeType == 2 or existTimeType == 3)
end
...
function ItemCell:UpdateStrengthLevel(strengthlv, withBuff)
  local isShow = strengthlv ~= nil and strengthlv > 0 and not self.forbidShowStrengthLv
  self.strenglv.gameObject:SetActive(isShow)
  if isShow then
...
function ItemCell:UpdateRefineLevel(refinelv, withBuff)
  local isShow = refinelv ~= nil and refinelv > 0 and not self.forbidShowRefineLv
...
function ItemCell:UpdateEquipLevel(equiplv, artifactlv)
  if not self.forbidShowEquipLv then
    ...
    elseif artifactlv and artifactlv > 0 then
      self.equiplv.gameObject:SetActive(true)
...
function ItemCell:UpdateCardSlot(data, isEquip)
  ...
  if isEquip and (slotNum > 0 or replaceCount > 0) then
  ...
  if itemtype == 58 then
    ...
    if mercenaryCats then
      for k, v in pairs(mercenaryCats) do
        ...
        if unlockEquips and #unlockEquips > 0 then
