...
function SkadaRankingCell:SetData(data, isSelf)
  ...
  if not data.totalTime then
    ...
    if sec > 0 or not haveContent then
      if haveContent then
        sb:Append(" ")
      end
      sb:Append(sec)
  ...
  elseif data.totalDamage < 10000 then
    ...
    self.labFightDamage.text = millionNum > 0 and millionNum .. "M" .. thousandStr or thousandStr
  end
  ...
  elseif 10000 > data.averageDamage then
    ...
    self.labDPS.text = millionNum > 0 and millionNum .. "M" .. thousandStr or thousandStr
  end
