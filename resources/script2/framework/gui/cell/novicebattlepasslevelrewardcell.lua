...
function NoviceBattlePassLevelRewardCell:CallBPTargetRewardCmd(is_all, lv, is_pro)
  if self.bPType == 2 then
...
function NoviceBattlePassLevelRewardCell:FindObjs()
  ...
  self:AddClickEvent(self.basicGetBtn, function()
    self:CallBPTargetRewardCmd(false, self.level, false)
  end)
  self:AddClickEvent(self.advGetBtn, function()
    self:CallBPTargetRewardCmd(false, self.level, true)
  end)
...
function NoviceBattlePassLevelRewardCell:SetData(data)
  if data then
    ...
    self:RefreshBasicState(self.level)
    self:RefreshAdvState(self.level)
  end
end
...
function NoviceBattlePassLevelRewardCell:RefreshBasicState(level)
  ...
  self.basicGetBtn:SetActive(not isBasicLocked and not isBasicReceived)
end
function NoviceBattlePassLevelRewardCell:RefreshAdvState(level)
  ...
  self.advGetBtn:SetActive(not isAdvLocked and not isAdvReceived)
end
