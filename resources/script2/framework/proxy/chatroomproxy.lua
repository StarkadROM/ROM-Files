  ...
  if not self:IsLoadedLocalFile(chatId) then
    ...
    if datas and #datas > 0 then
      ...
      for i = 1, #datas do
        ...
        if expressionId ~= nil and expressionId > 0 then
...
function ChatRoomProxy:AddSystemMessage(channelId, content, params, removeTime, id, targetid, isHideInSimplify)
...
function ChatRoomProxy:GetRandomExpressionTextId(t, id, hasTarget)
  ...
  if not self.expressionTexts[t] or not self.expressionTexts[t][id] or not self.expressionTexts[t][id][hasTarget] then
...
function ChatRoomProxy:CallUpdateFavoriteExpression(tempFavoriteExpressions)
  ...
  local i, origData, tempData = 1, nil, nil
...
function ChatRoomProxy.MakeExpressionText(text, senderName, targetName)
  ...
  if type(textData) ~= "string" or not OverSea.LangManager.Instance():GetLangByKey(textData) then
  end
  local result = string.gsub(OverSea.LangManager.Instance():GetLangByKey(textData.Msg), "$A", senderName or "")
