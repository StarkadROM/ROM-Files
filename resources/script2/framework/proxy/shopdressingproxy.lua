...
function ShopDressingProxy:SetOriginalEye(eyeId)
  ...
  local eyeSex
  if not self.originalBody or not Table_Body[self.originalBody] then
    local bodyData
  end
  if not bodyData or not bodyData.Feature then
    local bodyFeature
  end
  ...
  if eyeSex and eyeSex ~= userSex then
    if not Table_Eye[eyeId] or not Table_Eye[eyeId].PairItemID then
      local parsedEyeId
    end
    if parsedEyeId then
    ...
    if heroLevel >= tonumber(limitedLevel) then
      if v.goodsID and v.goodsID ~= 0 then
        if shoptype == dressShopType.Hair then
          ...
          if hairStyleData and hairStyleData.Sex and (userSex == hairStyleData.Sex or hairStyleData.Sex == 3) and hairStyleData.IsPro and hairStyleData.IsPro == 1 and hairStyleData.OnSale and hairStyleData.OnSale == 1 then
        ...
        elseif shoptype == dressShopType.EyeLenses then
          ...
          if eyeStaticData and eyeStaticData.Sex and (userSex == eyeStaticData.Sex or eyeStaticData.Sex == 3) and eyeStaticData.IsPro and eyeStaticData.IsPro == 1 and eyeStaticData.OnSale and eyeStaticData.OnSale == 1 then
