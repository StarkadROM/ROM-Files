...
function SkillProxy:_InnerStartCD(skillItemData, skillCD, skillDelayCD)
  if skillItemData then
    if skillCD then
      if skillItemData:GetMaxCDTimes() then
...
function SkillProxy:GetLearnedSkillBySortID(sortID, allskills)
  ...
  if bokiSkill then
    ...
    local skills = self.learnedSkills[sortID]
    return skills ~= nil and not allskills and skills[1] or skills or nil
  end
end
function SkillProxy:ForbitUse(skillItemData)
  ...
  if staticData then
    ...
    if skillItemData:GetID() == myselfData:GetAttackSkillIDAndLevel() and (myselfData:NoNormalAttack() or not self:CanMagicSkillUse(skillItemData) or not self:CanPhySkillUse(skillItemData)) then
      return true
    end
...
function SkillProxy:SkillCanBeUsed(skillItem)
  ...
  return not self:IsInCD(skillItem) and self:HasEnoughSp(skillItem) and self:HasEnoughHp(skillItem) and self:IsFitPreCondition(skillItem) and self:IsSubSkillFitPreCondition(skillItem) and self:HasFitSpecialCost(skillItem) and not self:IsExpire(skillItem) and self:HasUseCount(skillItem) or false
end
function SkillProxy:SkillCanBeUsedByID(skillID, allowNoLearned, isTrigger)
  ...
  if learnedSkill then
    ...
    local canBeUsed = not self:IsInCD(learnedSkill, isTrigger) and self:HasEnoughSp(learnedSkill, nil, isTrigger) and self:HasEnoughHp(learnedSkill) and self:IsFitPreCondition(learnedSkill) and self:IsSubSkillFitPreCondition(learnedSkill) and self:HasFitSpecialCost(learnedSkill) and not self:IsExpire(learnedSkill) and self:HasUseCount(learnedSkill) or false
...
function SkillProxy:IsInCD(skillItem, isTrigger)
  if skillItem then
    ...
    if skillItem:GetMaxCDTimes() then
      return skillItem:GetLeftCDTimes() <= 0 or CDProxy.Instance:SkillInCommualCD(skillItem:GetSortID())
    end
  ...
  if skillStaticData then
    ...
    if #skillCost > 0 then
      ...
      for i = 1, #skillCost do
        ...
        if specialCost.itemID and not isNoItem or specialCost.specialType == 4 then
          if dynamicSkillInfo ~= nil then
...
function SkillProxy:GetItemNumBySkillCostCfg(itemID, specialType)
  ...
  elseif specialType == ESPECIALSKILLCOSTTYPE.ESPSCOSTTYPE_HEADWEARSCENE or specialType == ESPECIALSKILLCOSTTYPE.ESPSCOSTTYPE_HEADWEARACTIVITYSCENE then
...
function SkillProxy:GetSkillCanBreak()
  ...
  if _MyselfProxy:HasJobBreak() then
    ...
    return point <= self:GetUsedPoints()
  end
  return false
end
...
function SkillProxy:SetSkillLeftCD(sortID, time)
...
function SkillProxy:RecvMarkSunMoonSkillCmd(datas)
  if datas then
    ...
    for i = 1, #datas do
      ...
      epro = datas[i].epro
      npcid = datas[i].npcid
      redlog("RecvMarkSunMoonSkill", sunmark, epro, npcid)
