...
function ServiceItemAutoProxy:CallItemUse(itemguid, targets, count, value, targetItemguids)
  if not NetConfig.PBC then
...
function ServiceItemAutoProxy:CallEnchantEquip(type, guid, is_improve, must_buff_item, count)
  if not NetConfig.PBC then
...
function ServiceItemAutoProxy:CallEquipExchangeItemCmd(guid, type, materials)
  if not NetConfig.PBC then
...
function ServiceItemAutoProxy:CallQueryLotteryInfo(infos, type, today_cnt, max_cnt, today_extra_cnt, max_extra_cnt, once_max_cnt, mixlotterycnts, safetyinfo, today_ten_cnt, max_ten_cnt, free_cnt)
  if not NetConfig.PBC then
...
function ServiceItemAutoProxy:CallMixLotteryArchiveCmd(type, price, once_max_cnt, discount, groups)
  if not NetConfig.PBC then
...
function ServiceItemAutoProxy:CallLotteryHeadwearExchange(items, success)
  if not NetConfig.PBC then
...
function ServiceItemAutoProxy:CallQueryAfricanPoringItemCmd(freenormalcount, lotterycount, status, items)
  if not NetConfig.PBC then
