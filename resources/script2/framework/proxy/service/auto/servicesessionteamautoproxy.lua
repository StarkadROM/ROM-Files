...
function ServiceSessionTeamAutoProxy:CallQueryUserTeamInfoTeamCmd(charid, teamid, groupteamid)
  if not NetConfig.PBC then
...
function ServiceSessionTeamAutoProxy:CallUpdateWantedQuestTeamCmd(quest)
  if not NetConfig.PBC then
    ...
    if quest ~= nil and quest.questdata.config.allrewardid ~= nil then
      ...
      if msg.quest.questdata.config.allrewardid == nil then
        msg.quest.questdata.config.allrewardid = {}
      end
    ...
    if quest ~= nil and quest.questdata.config.allrewardid ~= nil then
      ...
      if msgParam.quest.questdata.config.allrewardid == nil then
        msgParam.quest.questdata.config.allrewardid = {}
      end
      for i = 1, #quest.questdata.config.allrewardid do
        table.insert(msgParam.quest.questdata.config.allrewardid, quest.questdata.config.allrewardid[i])
      end
    end
    if quest ~= nil and quest.questdata.config.PreQuest ~= nil then
      if msgParam.quest.questdata.config == nil then
