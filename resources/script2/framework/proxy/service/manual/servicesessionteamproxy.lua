...
function ServiceSessionTeamProxy:CallInviteMember(userguid, catid, teamname, username, isgroup)
  local matchStatus, etype = PvpProxy.Instance:GetCurMatchStatus()
  if matchStatus and etype == PvpProxy.Type.TeamPws and (matchStatus.isprepare or matchStatus.ismatch) then
    MsgManager.ShowMsgByID(25984)
    return
  ...
  if isgroup == nil and TeamProxy.Instance:CheckIHaveGroupLeaderAuthority() and TeamProxy.Instance.myTeam:IsTeamFull() then
    isgroup = true
  end
  ServiceSessionTeamProxy.super.CallInviteMember(self, userguid, catid, teamname, username, isgroup)
end
...
function ServiceSessionTeamProxy:CallInviteGroupTeamCmd(charid, leadername, mycharid)
  if Game.MapManager:IsPveMode_Thanatos() then
    MsgManager.ShowMsgByID(25960)
    return
  end
  ServiceSessionTeamProxy.super.CallInviteGroupTeamCmd(self, charid, leadername, mycharid)
end
