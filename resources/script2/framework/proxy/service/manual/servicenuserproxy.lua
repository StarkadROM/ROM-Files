...
function ServiceNUserProxy:RecvCDTimeUserCmd(data)
  ...
  for i = 1, #data.list do
    ...
    if cdData.type == SceneUser2_pb.CD_TYPE_SKILL then
      needSendEvent = _CDProxy:Server_AddSkillCD(cdData.id, cdData.time, data.isall, cdData.coldtime, cdData.lefttimes, cdData.maxtimes)
...
function ServiceNUserProxy:RecvProfessionQueryUserCmd(data)
  ...
  if not self.manualCall then
    if UIManagerProxy.Instance:HasUINode(PanelConfig.MultiProfessionNewView) then
      return
    else
      GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
...
function ServiceNUserProxy:CallProfessionChangeUserCmd(branch, success)
  ...
  if etype == PvpProxy.Type.TeamPwsChampion and matchStatus and matchStatus.ismatch then
    MsgManager.ConfirmMsgByID(28108, function()
...
function ServiceNUserProxy:CallLoadRecordUserCmd(slotid)
  ...
  if etype == PvpProxy.Type.TeamPwsChampion and matchStatus and matchStatus.ismatch then
    MsgManager.ConfirmMsgByID(28108, function()
      ServiceNUserProxy.super:CallLoadRecordUserCmd(slotid)
    end)
    return
  end
...
function ServiceNUserProxy:RecvLoadRecordUserCmd(data)
  ...
  if not rid then
    local record = MultiProfessionSaveProxy.Instance:GetProfessionRecordSimpleDataById(data.slotid)
    rid = record and record.charid
  end
...
function ServiceNUserProxy:RecvHeadwearTowerUserCmd(data)
  HeadwearRaidProxy.Instance:RecvHeadwearTowerUserCmd(data)
  self:Notify(ServiceEvent.NUserHeadwearTowerUserCmd, data)
end
function ServiceNUserProxy:RecvHeadwearEndUserCmd(data)
  HeadwearRaidProxy.Instance:RecvHeadwearEndUserCmd(data)
  self:Notify(ServiceEvent.NUserHeadwearEndUserCmd, data)
end
function ServiceNUserProxy:RecvHeadwearRangeUserCmd(data)
...
function ServiceNUserProxy:RecvBattleTimelenUserCmd(data)
  ExpRaidProxy.Instance:CacheBattleTimelen(data)
