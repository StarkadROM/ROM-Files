...
function ServiceChatCmdProxy:CallChatCmd(channel, str, desID, voice, voicetime, msgid, msgover, photo, expression)
    ...
    if msgid ~= nil then
      ...
      for i = 1, splitLength do
        ...
        if i == splitLength then
          isOver = true
          ServiceChatCmdProxy.super.CallChatCmd(self, channel, str, desID, splitByteStr, voicetime, msgid, isOver, photo, expression, nil, items)
          ServiceChatCmdProxy.Instance:CallGetVoiceIDChatCmd()
        else
          ServiceChatCmdProxy.super.CallChatCmd(self, nil, "", nil, splitByteStr, nil, msgid, isOver, photo, expression, nil, items)
        end
      end
    end
  else
    ServiceChatCmdProxy.super.CallChatCmd(self, channel, str, desID, voice, voicetime, msgid, msgover, photo, expression, nil, items)
  end
end
