...
function ServicePlayerProxy:CallSkillBroadcast(random, data, creature, targetCreatureGUID, istrigger)
  if NetConfig.PBC then
...
function ServicePlayerProxy:SkillBroadcast(data)
  ...
  if data.data.number == SkillPhase.Hit then
    local isEmpty = true
  ...
  if creature ~= nil then
    if not skillInfo:IsMapSkill() or not Asset_Effect.EffectTypes.Map then
      local effectType
    end
