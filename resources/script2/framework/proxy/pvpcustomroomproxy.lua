...
function PvpCustomRoomProxy:GetRoomMenuList()
  if not self.raidModeList then
    ...
    local raidConfig = GameConfig.ReserveRoom.RaidMaps
    if raidConfig then
      for _, v in ipairs(raidConfig) do
        if v.etype == 9 then
          table.insert(self.raidModeList, {
            etype = v.etype,
            raidid = v.raidid,
            name = v.name
          })
...
function PvpCustomRoomProxy:OpenRoomInfoPopup()
  ...
  if curRoomData then
    if curRoomData.pvptype == PvpProxy.Type.DesertWolf then
      GameFacade.Instance:sendNotification(UIEvent.JumpPanel, {
