...
LocalSaveProxy.SAVE_KEY = {
  ...
  WorldQuestTraceNewTag = "WorldQuestTraceNewTag",
...
function LocalSaveProxy:GetCameraZoom()
  local value = FunctionPlayerPrefs.Me():GetFloat(LocalSaveProxy.SAVE_KEY.CameraZoom, 0)
  if value ~= 0 then
    return value
  end
end
...
function LocalSaveProxy:CheckPeddlerDailyDot()
  ...
  if str and string.len(str) > 0 then
    ...
    return delta >= 0 and delta < 86400
  end
end
...
function LocalSaveProxy:InitWorldQuestNewTag()
  if not FunctionPlayerPrefs.Me():IsInited() then
    return
  end
  if not ServerTime.CurServerTime() or not (ServerTime.CurServerTime() / 1000) then
    local serverTime
  end
  if not serverTime then
    return
  end
  local previousTimeStamp = FunctionPlayerPrefs.Me():GetString(LocalSaveProxy.SAVE_KEY.WorldQuestTraceTimeStamp, "")
  previousTimeStamp = tonumber(previousTimeStamp)
  if previousTimeStamp and serverTime > previousTimeStamp then
    local nextRefreshTime = ClientTimeUtil.GetNextDailyRefreshTime()
    xdlog("重置时间", nextRefreshTime)
    FunctionPlayerPrefs.Me():SetString(LocalSaveProxy.SAVE_KEY.WorldQuestTraceTimeStamp, nextRefreshTime)
  end
  self.worldQuestNewTagList = {}
  local str = FunctionPlayerPrefs.Me():GetString(LocalSaveProxy.SAVE_KEY.WorldQuestTraceNewTag, "")
  local ids = loadstring("return {" .. str .. "}")()
  for i = 1, #ids do
    local questId = ids[i]
    if questId and questId ~= "" then
      local isOver = FunctionPlayerPrefs.Me():GetBool(LocalSaveProxy.SAVE_KEY.WorldQuestTraceNewTag .. "_" .. questId, "")
      if isOver ~= nil then
        self.worldQuestNewTagList[questId] = isOver
      else
        FunctionPlayerPrefs.Me():DeleteKey(LocalSaveProxy.SAVE_KEY.DontShowAgain .. "_" .. questId)
      end
    end
  end
end
function LocalSaveProxy:AddWorldQuestTrace(id)
  local previousTimeStamp = FunctionPlayerPrefs.Me():GetString(LocalSaveProxy.SAVE_KEY.WorldQuestTraceTimeStamp, "")
  previousTimeStamp = tonumber(previousTimeStamp)
  local serverTime = ServerTime.CurServerTime() / 1000
  if not previousTimeStamp or previousTimeStamp < serverTime then
    local nextRefreshTime = ClientTimeUtil.GetNextDailyRefreshTime()
    redlog("下一次刷新时间", nextRefreshTime, serverTime)
    FunctionPlayerPrefs.Me():SetString(LocalSaveProxy.SAVE_KEY.WorldQuestTraceTimeStamp, nextRefreshTime)
  end
  local find = self:GetWorldQuestTrace(id)
  if find == nil then
    FunctionPlayerPrefs.Me():AppendString(LocalSaveProxy.SAVE_KEY.WorldQuestTraceNewTag, id, ",")
    FunctionPlayerPrefs.Me():SetBool(LocalSaveProxy.SAVE_KEY.WorldQuestTraceNewTag .. "_" .. id, false)
    FunctionPlayerPrefs.Me():Save()
    if not self.WorldQuestTraceNewTag then
      self.WorldQuestTraceNewTag = {}
    end
    self.worldQuestNewTagList[id] = false
  end
end
function LocalSaveProxy:RemoveWorldQuestTrace(id)
  local find = self:GetWorldQuestTrace(id)
  if find ~= nil then
    FunctionPlayerPrefs.Me():SetBool(LocalSaveProxy.SAVE_KEY.WorldQuestTraceNewTag .. "_" .. id, true)
    self.worldQuestNewTagList[id] = true
  end
end
function LocalSaveProxy:GetWorldQuestTrace(id)
  return self.worldQuestNewTagList and self.worldQuestNewTagList[id]
end
function LocalSaveProxy:GetWorldQuestTraceList()
  return self.worldQuestNewTagList
end
function LocalSaveProxy:IsGVGFramChecked()
  return FunctionPlayerPrefs.Me():GetInt(LocalSaveProxy.SAVE_KEY.GVG_FrameChecked, 0) == 1
end
