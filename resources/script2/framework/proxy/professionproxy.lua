...
ProfessionProxy.NoviceBranches = {1000, 2000}
...
function ProfessionProxy:ParseStaticInfo()
  ...
  if not ProfessionProxy.specialJobsRawType then
    ...
    for i = 1, #ProfessionProxy.specialJobs do
      ...
      table.insert(ProfessionProxy.specialJobsRawType, math.floor(sp / 10))
    end
  end
  if not ProfessionProxy.specialJobBranches then
    ...
    for i = 1, #ProfessionProxy.specialJobs do
      local sp = ProfessionProxy.specialJobs[i]
      if Table_Class[sp] and Table_Class[sp].TypeBranch then
        table.insert(ProfessionProxy.specialJobBranches, Table_Class[sp].TypeBranch)
      end
    end
  end
...
function ProfessionProxy:ParseNextProfession(tree, profession)
  local parentClass, class, p = profession.data, nil, nil
...
function ProfessionProxy:RecvProfessionQueryUserCmd(data)
  ...
  local needCheckHasInfo = data ~= nil and data.body ~= nil and data.body.items ~= nil and #data.body.items > 0
...
function ProfessionProxy:IsThisIdYiJiuZhi(id)
  ...
  if serverData then
    ...
    if self.GetJobDepth(id) == 1 then
      local advanceClassIdTable, sData = thisIdClassData.AdvanceClass, nil
...
function ProfessionProxy:IsThisIdYiGouMai(id)
  ...
  if curOcc and curOcc.professionData and curOcc.professionData.TypeBranch then
    ...
    if jobDepth == 1 then
      local thisIdClassData, SClassData = Table_Class[id], nil
...
function ProfessionProxy:CheckHeroCanBuy(profId)
  ...
  if profData and profData.HeroAdvanceClass then
    if self:IsThisIdYiJiuZhi(profData.HeroAdvanceClass) and self:CheckJobSwitchValid(profData.HeroAdvanceClass) then
...
function ProfessionProxy:GetMoneyNumberAndMoneyTypeForPurchaseThisId(id)
  if GameConfig.ProfessionExchangeTicket == nil then
    ...
    if ticketID and BagProxy.Instance:GetItemNumByStaticID(ticketID) > 0 then
  ...
  if ProfessionProxy.Instance:isOriginProfession(id) then
    ...
    local isCollabor = collaborHero and 0 < TableUtility.ArrayFindIndex(collaborHero, id) or false
    if isHero then
...
function ProfessionProxy:JudgeTicketAndShowOrHide(id, callback)
  if GameConfig.SystemForbid.OpenJapanMultiJobs then
    do
      ...
      if branchConfig and branchConfig.item and BagProxy.Instance:GetItemNumByStaticID(branchConfig.item) > 0 then
...
function ProfessionProxy.CanChangeProfession(failPrompt)
  ...
  if canChangeMaps and TableUtility.ArrayFindIndex(canChangeMaps, mapid) > 0 then
    return true
  end
  ...
  elseif mapType == 3 then
    ...
    if dungeonIns.roguelikeRaid then
      ...
      if subScenes and curRoomIndex and curRoomIndex > 0 then
...
function ProfessionProxy:PurchaseFunc(id, callback)
  if not self.CanChangeProfession(25693) then
    return
  end
  local isOriginProfession = self:isOriginProfession(id)
  local branch = Table_Class[id].TypeBranch
...
function ProfessionProxy:PurchaseLogic(id, callback)
  ...
  elseif self:IsThisIdFirstBoughtOtherBranchId(id) then
    ...
    if moneyType.id == allProfTicket then
      local allProfTicketItem = Table_Item[allProfTicket]
      if allProfTicketItem and BagProxy.Instance:GetItemNumByStaticID(allProfTicket) > 0 then
        local costStr = string.format(ZhString.Friend_RecallRewardItem, 1, allProfTicketItem.NameZh)
        MsgManager.ConfirmMsgByID(9630, function()
          ServiceNUserProxy.Instance:CallProfessionBuyUserCmd(branch, true)
        end, nil, nil, costStr, Table_Class[id].NameZh)
...
function ProfessionProxy:TryPurchaseHero(id)
  ...
  return BagProxy.Instance:GetItemNumByStaticID(ticketID) > 0 and true or false
end
...
function ProfessionProxy:GetTopScrollViewIconTable()
  ...
  local checkForbidProf = function(forbidProf)
    if forbidProf and next(forbidProf) then
      for i = #self.topScrollViewIconTable, 1, -1 do
        if self.topScrollViewIconTable[i] and TableUtility.ArrayFindIndex(forbidProf, self.topScrollViewIconTable[i].id) > 0 then
    ...
    if not FunctionUnLockFunc.Me():CheckCanOpen(menuId) then
      ...
      if menuData and menuData.event and menuData.event.type == "unlockprofession" then
        for _, professionId in ipairs(menuData.event.param or {}) do
          if TableUtility.ArrayFindIndex(forbidProf, professionId) == 0 then
            TableUtility.ArrayPushBack(forbidProf, professionId)
          end
        end
      end
    end
  end
  checkForbidProf(forbidProf)
  local curOcc = Game.Myself.data:GetCurOcc()
...
function ProfessionProxy:GetValidJobList()
  ...
  for k, v in pairs(S_data) do
    ...
    if classConfig and (not branchInfo or not branchInfo.gender or branchInfo.gender == self:GetCurSex()) then
      local data = {}
      data.id = prof
      data.isUnlock = true
  ...
  if humanDepth >= 1 and S_data[0] and S_data[0].profession == ProfessionProxy.humanNovice then
    self.multiProfessionList[0] = nil
  end
  if doramDepth >= 1 and S_data[0] and S_data[0].profession == ProfessionProxy.doramNovice then
    self.multiProfessionList[0] = nil
  end
  if hashuman then
    if humanDepth >= 1 then
  ...
  if hasdora then
    if S_data[0] and S_data.profession == ProfessionProxy.doramNovice then
      ...
      for k, v in pairs(Table_Class) do
        if ProfessionProxy.GetJobDepth(v.id) == 0 and self.IsDoramRace(v.id) and TableUtility.ArrayFindIndex(advClass, v.id) == 0 then
          local jobData = {}
          jobData.id = v.id
  ...
  local checkForbidProf = function(forbidProf)
    if forbidProf and next(forbidProf) then
      for i = 1, #forbidProf do
        local classConfig = Table_Class[forbidProf[i]]
        local jobBranch = classConfig and classConfig.TypeBranch
        if self.multiProfessionList[jobBranch] and not self:CheckBranchShowValid(forbidProf[i]) then
  ...
  checkForbidProf(self.forbidProfList)
  local curOcc = Game.Myself.data:GetCurOcc()
  local curProfessionId = curOcc.profession
  ...
  for k, v in pairs(self.multiProfessionList) do
    ...
    if not tempList[classType] then
      local profTypeList = {Type = classType}
    end
...
function ProfessionProxy:RefreshBannedProfList()
  ...
  self.forbidProfList = {}
  TableUtility.ArrayShallowCopy(self.forbidProfList, forbidProf)
end
function ProfessionProxy:CheckProfIsBanned(classid)
  if not self.forbidProfList then
    return false
  end
  if TableUtility.ArrayFindIndex(self.forbidProfList, classid) > 0 then
    return true
  else
    return false
  end
...
function ProfessionProxy.IsDoramForbidden()
  return not Table_Class or not Table_Class[ProfessionProxy.doramNovice] or UnionConfig and UnionConfig.roleDolanOpen and UnionConfig.roleDolanOpen == 0
end
...
function ProfessionProxy.GetSpecialJobDepth(profession)
  ...
  if TableUtility.ArrayFindIndex(ProfessionProxy.specialJobsRawType, rawType) > 0 then
    return curId % 10
  end
  return -1
end
...
function ProfessionProxy.GetBoughtProfessionIdThroughBranch(branch)
  if TableUtility.ArrayFindIndex(ProfessionProxy.NoviceBranches, branch) > 0 or 0 < TableUtility.ArrayFindIndex(ProfessionProxy.specialJobBranches, branch) then
...
function ProfessionProxy.GetAllBranchProfessionId()
  ...
  for k, _ in pairs(Table_Branch) do
    if k < 1000 and TableUtility.ArrayFindIndex(forbidBranch, k) == 0 and ProfessionProxy.GetBoughtProfessionIdThroughBranch(k) then
...
function ProfessionProxy.TypeBranchS2C(typeBranch)
  if TableUtility.ArrayFindIndex(ProfessionProxy.NoviceBranches, typeBranch) > 0 then
    return 0
  end
  return typeBranch
end
...
local bannedBranch = {}
local bannedProfMap = {}
local bannedBranchMap = {}
function ProfessionProxy.GetBannedProfessions()
  TableUtility.ArrayClear(bannedProf)
  TableUtility.TableClear(bannedProfMap)
  ...
  for pro, _ in pairs(bannedProfMap) do
    bannedProf[#bannedProf + 1] = pro
  end
  return bannedProf
end
function ProfessionProxy.GetBannedBranches()
  TableUtility.ArrayClear(bannedBranch)
  TableUtility.TableClear(bannedBranchMap)
  for branchid, funcStateId in pairs(Game.Config_BranchForbidMap) do
    if FunctionNpcFunc.Me():CheckSingleFuncForbidState(funcStateId) then
      bannedBranchMap[branchid] = 1
    end
  end
  for branchid, _ in pairs(bannedBranchMap) do
    bannedBranch[#bannedBranch + 1] = branchid
  end
  return bannedBranch
end
...
function ProfessionProxy:InitFirstTransferClass()
  ...
  if not staticData then
    ...
    self.firstSelectionViewIndex = TableUtility.ArrayFindIndex(self.groupClassList, staticData.GroupClassID)
...
function ProfessionProxy:GetClassState(id)
  ...
  xdlog(id, self:IsThisIdYiGouMai(id), self:IsThisIdKeGouMai(id))
  ...
  elseif not self.IsHero(id) and self:GetThisIdPreviousId(id) == curProfessionId and (typeBranch == curTypeBranch or self:isOriginProfession(id) and self.GetBrotherBranch(typeBranch) == curTypeBranch or TableUtility.ArrayFindIndex(ProfessionProxy.NoviceBranches, curTypeBranch) > 0) then
...
function ProfessionProxy:NeedShowNoviceJinjie()
  ...
  if jobLevel >= 10 and nowRoleLevel >= 10 then
    return true
  end
end
