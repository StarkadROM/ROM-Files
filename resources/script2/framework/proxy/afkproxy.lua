...
function AfkProxy:GetAfkTimeMsg(afkTime)
...
function AfkProxy:GetAfkBeKillMsg(killInfo)
  if killInfo then
    ...
    local config = Table_Monster[monsterId]
    if config then
...
function AfkProxy:GetAfkStatusMsg(status)
  if status and status > 0 and GameConfig.Afk.quit_reasons then
...
function AfkProxy:GetAfkMsgArray(tb, data)
  ...
  ArrayPushBack(tb, self:GetAfkTimeMsg(afkTime))
  local baseExp = statdata.baseexp or 0
  local jobExp = statdata.jobexp or 0
  if baseExp > 0 or jobExp > 0 then
  ...
  if beKillInfo then
    for _, info in pairs(beKillInfo) do
      if info.monsterid then
        local config = Table_Monster[info.monsterid]
        if config then
          local name = OverSea.LangManager.Instance():GetLangByKey(config.NameZh or "")
...
function AfkProxy:SyncAfkStatus(data)
  if not data or not data.charid or data.charid == 0 then
    self:SetAfk(false)
  else
    self:SetAfk(true)
