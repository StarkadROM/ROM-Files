...
function TeamProxy:InitTeamGoals()
  ...
  for k, v in pairs(Table_TeamGoals) do
    if (nil == v.FuncState or FunctionUnLockFunc.checkFuncStateValid(v.FuncState)) and v.GoalGroup then
...
function TeamProxy:ForbiddenByMatchRaidID(id)
  if not Table_MatchRaid[id] or not Table_MatchRaid[id].RaidConfigID then
    local raidCfg
  end
...
function TeamProxy:CheckRaidIdSupportDiffServer(id)
  if not id or not Table_MapRaid[id] then
    local mapRaidConfig
  end
...
function TeamProxy:CheckMatchRaidSupportDiffServer(id)
  if not id or not Table_MatchRaid[id] then
    local matchRaidConfig
  end
...
function TeamProxy:IsInMyGroup(playerid)
  local found = false
  found = self.myTeam and self.myTeam:GetMemberByGuid(playerid) ~= nil
  found = not found and self.uniteGroupTeam and self.uniteGroupTeam:GetMemberByGuid(playerid) ~= nil
  return found
end
