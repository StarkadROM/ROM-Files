...
function ComodoBuildingProxy:InitPartnerFavoredMap()
  ...
  for _, d in pairs(Table_ManorPartner) do
    ...
    if d.Favors then
      ...
      self.partnerMaxFavorMap[partnerId] = max > 0 and max or nil
    end
    if d.ItemFavor then
...
local getResourceProduceInfo = function(buildingId, produceParamArr)
  ...
  local produce, interval, capacity, param = basicCfg.produce, basicCfg.interval, basicCfg.capacity, nil
...
local updateBuildingFuncData = function(localData, serverDatas)
  local funcDataMap, t = localData.funcDataMap or {}, nil
...
local updateDispatchData = function(localData, serverDatas)
  local dispatch, data, element, areaId = localData.dispatch or {}, nil, nil, nil
  if serverDatas then
...
function ComodoBuildingProxy:CheckFavoredItemCanBuy(itemId)
  ...
  if partners then
    local manorIns, isAllFullFavorability, info = ManorPartnerProxy.Instance, true, nil
    for _, partner in pairs(partners) do
      info = manorIns:GetPartnerInfo(partner)
      if info then
      elseif info.favor < (self.partnerMaxFavorMap[info.id] or math.huge) then
...
function ComodoBuildingProxy:GetDisplayIconNameOfReservedProduce(buildingId)
  ...
  local flag = displayIconCfg and curProduce > 0
  if flag then
    local ratio = capacity and capacity > 0 and curProduce / capacity or 0
...
function ComodoBuildingProxy:GetDisplayIconNameOfSmithing(npcId)
  ...
  local displayIcon, countdown = globalCfg.ManorForgeComplete[npcId], nil
