...
function ExpRaidProxy:CacheBattleTimelen(body)
  if not body or not body.timelen or not body.totaltime then
    self.battleTimelen = nil
    return
  end
  self.battleTimelen = self.battleTimelen or {}
  self.battleTimelen.timelen = body.timelen
  self.battleTimelen.totaltime = body.totaltime
  self.battleTimelen.musictime = body.musictime
  self.battleTimelen.tutortime = body.tutortime
  self.battleTimelen.powertime = body.powertime
  self.battleTimelen.playtime = body.playtime
  self.battleTimelen.usedplaytime = body.usedplaytime
  ServiceFuBenCmdProxy.Instance:CallTeamExpQueryInfoFubenCmd()
end
function ExpRaidProxy:UpdateBattleTimelen(pvePassInfo)
  if not self.battleTimelen then
    self.battleTimelen = {}
  end
  if pvePassInfo.totalbattletime and pvePassInfo.battletime then
    self.battleTimelen.totaltime = pvePassInfo.totalbattletime
    self.battleTimelen.timelen = pvePassInfo.totalbattletime - pvePassInfo.battletime
  end
  if pvePassInfo.totalplaytime and pvePassInfo.playtime then
    self.battleTimelen.playtime = pvePassInfo.totalplaytime
    self.battleTimelen.usedplaytime = pvePassInfo.totalplaytime - pvePassInfo.playtime
  end
end
function ExpRaidProxy:SyncGameTimeSetting(serverData)
  self.gameTimeSetting = serverData and serverData.ecost or 1
end
function ExpRaidProxy:GetGameTimeSetting()
  return self.gameTimeSetting or 1
end
function ExpRaidProxy:GetPveTime(timeType)
  if timeType == nil then
    timeType = self.gameTimeSetting
  end
  if timeType == 1 then
    return self.battleTimelen.playtime - self.battleTimelen.usedplaytime
  else
    return self.battleTimelen.totaltime - self.battleTimelen.timelen
  end
end
function ExpRaidProxy:CheckBattleTimelen(containPlayTime)
  local body = self.battleTimelen
  if body then
    local threshold = GameConfig.TeamExpRaid.sysmsg_remind_battletime
    if threshold > self:GetPveTime(2) then
      if containPlayTime then
        return threshold < self:GetPveTime(1)
      end
      return false
    end
  end
...
function ExpRaidProxy:CheckBattleTimelenAndRemainingTimes()
  return self:CheckBattleTimelen() and self.remainingCount ~= nil and self.remainingCount > 0
end
