...
function TechTreeProxy.CheckMaterialsAdequate(materialDatas, packageCheck)
  ...
  local isAdequate, sId, needNum, num = true, nil, nil, nil
  ...
  if pLeafId and self:IsLeafUnlocked(pLeafId, treeId) then
    if #self:GetStaticTreeIdsOfLeaf(pLeafId) > 1 then
      while pLeafId do
        ...
        if not pLeafId or self:IsLeafUnlocked(pLeafId, treeId) then
          else
            pLeafId = nil
          end
        end
...
function TechTreeProxy:IsLeafUnlocked(leafId, treeId)
  ...
  local flag = false
  if #trees > 1 then
    flag = not self.unlockedLeaves[treeId][leafId] or true or flag
  elseif #trees == 1 then
    flag = self.unlockedLeaves[trees[1]][leafId] and true or flag
  end
  return flag
end
...
function TechTreeProxy:RecvSyncLeaf(data)
  ...
  if treeinfo and #treeinfo > 0 then
    for i = 1, #treeinfo do
...
function TechTreeProxy:RecvUnlockLeaf(leafId, level, nodeinfo)
  ...
  if nodeinfo and #nodeinfo > 0 then
    for i = 1, #nodeinfo do
