...
function ShortCutProxy:HandleServerShortCut(checkequip)
  ...
  for _, v in pairs(self.itemShortMap) do
    if v.type and nil ~= Table_Item[v.type] then
      if nil ~= Table_Equip[v.type] then
      ...
      elseif 0 >= _BagProxy:GetItemNumByStaticID(v.type, GameConfig.PackageMaterialCheck.default) then
        v.guid = ""
      end
    end
  end
