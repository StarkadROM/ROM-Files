...
function PvpProxy:NtfMatchInfo(etype, ismatch, isfighting, robot_rest_time, robot_match_time, configid, begintime)
  ...
  self.matchStateMap[etype].startMatchTime = begintime and begintime > 0 and begintime * 1000 or ismatch and ServerTime.CurServerTime() or nil
...
function PvpProxy:UpdateCountDown()
  if not self.teamPwsPreInfo or not self.teamPwsPreInfo.maxPrepareTime or not self.teamPwsPreInfo.startPrepareTime then
    self:RemoveTimeTick()
  end
...
function PvpProxy:HandleQueryTeamPwsTeamInfo(data)
  ...
  if beginweekday == 7 then
    beginweekday = 0 or beginweekday
  end
...
function PvpProxy:UpdateTeamPwsInfos(syncdata, sparetime, freefire)
  ...
  if freefire ~= nil then
    self:UpdateFreeFire(freefire)
  end
end
...
function PvpProxy:GetTypeName()
  ...
  elseif type == PvpProxy.Type.TeamPwsChampion then
    typeStr = ZhString.PvpTypeName_TeamPwsPVPChampion
  else
...
function PvpProxy:RecvOthelloInfoSyncFubenCmd(data)
  ...
  if data then
    ...
    if data.fullfire ~= nil then
      self:UpdateFreeFire(data.fullfire)
    end
  end
...
function PvpProxy:ShowPolly3v3v3Result()
  ...
  local creatureIdTopUiMap, creature, creatureData, colorId = pollyD and pollyD.creatureIdTopUiMap, nil, nil, nil
...
function PvpProxy:RecvDesertWolfRuleSync(data)
  ...
  self:UpdateFreeFire(data.full_fire)
