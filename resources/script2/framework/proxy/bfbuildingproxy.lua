...
function BFBuildingProxy:SetBuildData(data)
  ...
  if idata.time ~= 0 or not 0 then
  end
  idata.time = idata.sdata.RunTime + idata.time
  if idata.totalCount ~= 0 or not 1 then
  end
  idata.progress = idata.curCount / idata.totalCount
end
...
function BFBuildingProxy:GetDonateData(id)
  ...
  local donateList, ni, cnum = {}, nil, nil
  for i = 1, #idata.sdata.ItemNeed do
...
function BFBuildingProxy.IsBFBuilding(npcid)
  ...
  local npcfunc, nf = Table_Npc and Table_Npc[npcid] and Table_Npc[npcid].NpcFunction, nil
  if npcfunc then
    for i = 1, #npcfunc do
      nf = npcfunc[i].type
...
local pendingCheckTickId, tickManager = 1, nil
...
function BFBuildingProxy:_PlayBuildAction(nnpc, animParams, status, cbStatus)
  if not nnpc or not nnpc.assetRole or not nnpc.assetRole.PlayAction then
    return false
  end
