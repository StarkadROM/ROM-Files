...
function GuildProxy:UpdateMyGuildData(data)
  ...
  GvgProxy.Instance:ResetMyGuildUnionGroupID()
end
...
function GuildProxy:ExitGuild(guildid)
  ...
  GvgProxy.Instance:ClearMyGuildUnionGroupID()
end
...
function GuildProxy:CanIEditAuthority(job, authorityType)
  ...
  if mygm then
    ...
    if job > myjob then
      ...
      if authorityType > 32 then
        return edit2value & 1 << authorityType - 32 - 1 > 0
      else
...
function GuildProxy:GetJobEditAuthority(job)
  ...
  local editvalue, edit2value = config and config.EditAuthority2 or config and config.EditAuthority, 0
  if editvalue > 0 or edit2value > 0 then
    local result = {}
    for _, gtype in pairs(GuildAuthorityMap) do
      if gtype > 32 then
        local typevalue = 1 << gtype - 32 - 1
        if edit2value & typevalue > 0 then
...
function GuildProxy:_CheckGuildActivityData(data)
  ...
  if nil ~= funcStateId and not FunctionUnLockFunc.checkFuncStateValid(funcStateId) then
    return false
  end
...
function GuildProxy:GetMercenaryGuildName(creatureData)
  if not GvgProxy.Instance:GetGvgOpenFireState() then
    return nil
  end
