...
function MsgManager.ShowMsgTable(title, text, type, param, roleid, data, removeTime, lockreason, showInSimplify)
  local handler, index
  for i = 1, #BitIndexs do
    index = BitIndexs[i]
    if BitUtil.valid(type, index) then
      if BitUtil.band(type, index) > 0 then
        ...
        if handler ~= nil then
          handler(title, text, param, roleid, data, removeTime, lockreason, showInSimplify)
        end
      end
    else
      break
...
function MsgManager.FloatMsgTableParam(title, text, param, roleid, data)
  if data and data.id and Table_Sysmsg[data.id] and nil == Table_Sysmsg[data.id].showInPVP and Game.MapManager:IsPVPMode() then
...
function MsgManager.DontAgainConfirmMsgByID(id, confirmHandler, cancelHandler, source, ...)
  ...
  if data ~= nil and BitUtil.valid(data.Type, MsgManager.MsgType.DontShowAgain) then
    if BitUtil.band(data.Type, MsgManager.MsgType.DontShowAgain) > 0 then
      ...
      if dont == nil or dont > 0 and dont < curTime then
        ...
        local opt = data.DontShowAgainDefaultOpt
        if opt == 2 then
...
function MsgManager.RichConfirmMsgByID(id, confirmHandler, cancelHandler, source, btnText, btnfText, ...)
  ...
  if data ~= nil then
    local text = MsgParserProxy.Instance:TryParse(data.Text, ...)
    confirmHandler = MsgManager.AdaptConfirm(data.Confirm, confirmHandler)
    UIUtil.PopUpRichConfirmYesNoView(data.Title, text, confirmHandler, cancelHandler, source, btnText or data.button, btnfText or data.buttonF, id)
  end
end
...
function MsgManager.ChatMsgTableParam(text, param, channelID, removeTime, showInSimplify)
  local isHideInSimplify
  if showInSimplify == 1 then
    isHideInSimplify = false
  end
  ChatRoomProxy.Instance:AddSystemMessage(channelID, text, param, removeTime, nil, nil, isHideInSimplify)
end
...
function MsgManager.ChatGuildChannelMsgTableParam(title, text, param, roleid, data, removeTime, lockreason, showInSimplify)
...
function MsgManager.ChatGVGChannelMsgTableParam(title, text, param, roleid, data, removeTime, lockreason, showInSimplify)
  ...
  if amIMercenary or mercenaryNum and mercenaryNum > 0 then
...
function MsgManager.AndroidPerissonMsg(id, confirmHandler)
  ...
  if data ~= nil then
    ...
    if dont then
      ...
      local timeSatmp = tonumber(dont)
      isTimeEnd = serverTime > timeSatmp
    end
    if dont == nil or isTimeEnd == true then
      LocalSaveProxy.Instance:RemoveDontShowAgain(id)
      local viewData = {
