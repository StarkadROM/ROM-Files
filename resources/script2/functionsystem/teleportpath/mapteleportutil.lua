...
local MapOutterTeleport = MapOutterTeleport
MapTeleportUtil.EstimateNpcTransCost = 50
local ChangeMapCost = 50
...
function MapTeleportUtil.CanTargetTransferTo(targetMapID)
  ...
  if not ActivityEventProxy.Instance:IsFreeTransferMap(targetMapID, UIMapMapList.transmitType) then
    ...
    if tickNum == 0 then
      if not CostUtil.CheckROB(sConfig.Money or 0) then
        cache.State = false
        return false
      end
    end
  ...
  if not FunctionUnLockFunc.checkFuncStateValid(funcstateId) then
...
function MapTeleportUtil.GetMinCost(srcID, dstID)
  ...
  local retCost, retEp, retBp = math.huge, nil, nil
