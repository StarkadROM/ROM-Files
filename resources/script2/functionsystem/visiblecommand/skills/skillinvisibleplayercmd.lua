...
function SkillInVisiblePlayerCmd:HidePlayer(player)
  if player then
    ...
    if TeamProxy.Instance:IsInMyTeam(playerId) or PvpObserveProxy.Instance:IsRunning() or player.data.ownerID == Game.Myself.data.id and not isBoki or isPet and not isBoki and TeamProxy.Instance:IsInMyTeam(player.data.ownerID) then
