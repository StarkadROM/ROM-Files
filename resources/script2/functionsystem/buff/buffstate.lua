...
function BuffState:_PlayEffect(creature)
  ...
  if ep ~= nil then
    if self:IsEffectFollow() then
      self.effect = creature.assetRole:PlayEffectAt(effectPath, ep, nil, OnEffectCreated, self, self.config.EffectScale, nil, lodLevel, priority)
    else
      self.effect = creature.assetRole:PlayEffectOn(effectPath, ep, nil, OnEffectCreated, self, self.staticData.EffectScale, nil, lodLevel, priority)
    end
  else
...
function BuffState:_PlayDeActiveEffect(creature)
  ...
  self:_PlayOneShotEffectOn(creature, path, self.staticData.SE_end, self.staticData.Effect_endScale, nil, nil, lodLevel, priority, AudioSourceType.BUFF_end, true)
  if self.staticData.SE_start_Loop ~= "" and self.audioSource then
    self.audioSource:Stop()
    self.audioSource = nil
  end
