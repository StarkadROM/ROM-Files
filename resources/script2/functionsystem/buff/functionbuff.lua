...
function FunctionBuff:_ServerSyncAddBuff(creature, datas, isMine)
  ...
  for i = 1, #datas do
    buff = datas[i]
    creature:AddBuff(buff.id, false, nil, buff.fromid, buff.layer, buff.level, buff.active, buff.stateid, buff.maxlayer)
...
function FunctionBuff:UpdateOffingEquipBuff()
  ...
  if offPoses and #offPoses > 0 then
    ...
    GameFacade.Instance:sendNotification(MyselfEvent.AddBuffs)
  else
    EventManager.Me():PassEvent(MyselfEvent.RemoveBuffs)
    GameFacade.Instance:sendNotification(MyselfEvent.RemoveBuffs)
  end
...
function FunctionBuff:ParseBuffData(buff)
  ...
  if not self.myBuffs[id] then
    local buffData = {id = id, staticData = sData}
  end
