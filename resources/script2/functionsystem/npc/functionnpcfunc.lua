...
function FunctionNpcFunc.LotteryMixed(nnpc, params, npcFunctionData)
  LotteryProxy.Instance:OpenNewLottery(params)
end
function FunctionNpcFunc.LotteryHeadwear(nnpc, param, npcFunctionData)
  LotteryProxy.Instance:OpenNewLottery(1)
end
...
function FunctionNpcFunc.LotteryCard(nnpc, param, npcFunctionData)
  LotteryProxy.Instance:OpenNewLottery(3)
end
...
function FunctionNpcFunc.Refine(nnpc, params, funcConfig, isFromBag)
  ...
  if isfashion then
    ...
    FunctionNpcFunc.JumpPanel(PanelConfig.RefineCombineView, {
      npcdata = nnpc,
      index = 1,
      isFromBag = isFromBag,
...
function FunctionNpcFunc.SeniorEnchant(nnpc, params, funcConfig, isFromBag)
  ...
  FunctionNpcFunc.JumpPanel(PanelConfig.EnchantCombineView, {
...
local callSummonDeadBoss = function(difficulty)
  if getSummonDeadBossCount(difficulty) > 0 then
...
function FunctionNpcFunc.SewingRefine(npc, param)
  FunctionNpcFunc.JumpPanel(PanelConfig.NpcRefinePanel, {npcdata = npc, isfashion = true})
end
...
function FunctionNpcFunc.RefineTransfer(nnpc)
  FunctionNpcFunc.JumpPanel(PanelConfig.RefineCombineView, {npcdata = nnpc, index = 2})
end
...
function FunctionNpcFunc.CardLottery(npc, param, npcFunctionData)
  ...
  elseif LotteryProxy.IsNewPocketCardLottery(lotterytype) then
    LotteryProxy.Instance:OpenNewLottery(lotterytype)
  else
...
local checkSummonDeadBoss = function(difficulty)
  ...
  return getSummonDeadBossCount(difficulty) > 0 and NpcFuncState.Grey or NpcFuncState.Active
end
...
function FunctionNpcFunc.CheckRoguelike(npc, param, funcStaticId)
  ...
  local staticFunc, name = Table_NpcFunction[funcStaticId], nil
  if staticFunc then
...
function FunctionNpcFunc:CheckSingleFuncForbidState(funcStateID)
  ...
  if self:CheckServerID(singleCFG.ServerID) and serverTime > validtime.StartTimeStamp and serverTime < validtime.EndTimeStamp then
    return true
  end
  return false
end
...
function FunctionNpcFunc.CheckLv_strengthen()
  return Game.Myself.data.userdata:Get(UDEnum.ROLELEVEL) >= 15
end
function FunctionNpcFunc.CheckLv_Refine()
  return Game.Myself.data.userdata:Get(UDEnum.ROLELEVEL) >= 20
end
function FunctionNpcFunc.CheckLv_SeniorEnchant()
  return Game.Myself.data.userdata:Get(UDEnum.ROLELEVEL) >= 55
end
