...
function HomePhonographManager:StopPhonographMusic()
  bgmIns:StopJukeBox(musicFadeDuration, 0)
...
function HomePhonographManager:_PlayMusic(musicId, startTime, pastTime)
  ...
  if not bgmIns:IsDefaultBGM() then
    do
      local bgmCtrl = bgmIns.currentNewBgm
      if bgmCtrl then
  ...
  bgmIns:PlayJukeBox(path, pastTime, nil, nil, musicFadeDuration, musicFadeDuration, musicFadeDuration, 0)
