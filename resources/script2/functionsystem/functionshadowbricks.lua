...
function FunctionShadowBricks:CheckOptionInValid()
  if self.done or self.isLerping or not self.shadowPuzzleSystem or self.enterViewMoving then
    return true
  end
  return false
end
...
function FunctionShadowBricks:IsBrickDone(s)
  return s > 0 and nil ~= self.doneBrickSlotMap[s]
end
...
function FunctionShadowBricks:GetSlotByIndex(i)
  ...
  if i and i <= slotLength and i > 0 then
    return self.slots[i]
  end
end
