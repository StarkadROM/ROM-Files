...
function FunctionScenicSpot:AddCreatureScenicSpot(guid, ssID)
  ...
  if creature.data.props:GetPropByName("Hiding"):GetValue() > 0 and (not Game.Myself or not Game.Myself.data or Game.Myself.data.id ~= guid) then
    self.hideScenicSpots[guid] = ssID
    return
  end
...
function FunctionScenicSpot:GetNearestScenicSpot(originPos, camera)
  ...
  for k, v in pairs(self.scenicSpots) do
    ...
    if v.ID then
      ...
      if not (distance > 0) or not (minDistance > distance) or self:IsThisSceneSpotShouldBeRemoved(ss) then
      else
      ...
      for i = 1, #v do
        ...
        if not (distance > 0) or not (minDistance > distance) or self:IsThisSceneSpotShouldBeRemoved(single) then
        else
          minDistance = distance
