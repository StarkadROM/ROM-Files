...
function FunctionSkill:TryUseSkill(skillIDAndLevel, target, noSearch, auto, quickuse)
    ...
    if myself.data.userdata:Get(UDEnum.NORMAL_SKILL) == skillIDAndLevel then
      ...
      if skillType == SkillType.Function then
        ...
        if skillTargetType ~= nil then
          if quickuse then
            isUseSkill = myself:Client_QuickUseSkill(skillIDAndLevel, target, nil, nil, noSearch)
    ...
    if objects then
      for objID, obj in pairs(objects) do
        ...
        if dis >= LuaVector3.Distance(myPos, objPos) then
          ...
          local single = not OutlineIitems or OutlineIitems[id]
          local flag = true
