...
function FunctionPet:HandleStartSkill(skill)
  ...
  if skill and skill:GetSkillType() == "TouchPet" then
    ...
    if npet then
      ...
      local focusOffset
      if FunctionCameraEffect.Me():GetCurCameraMode() == FunctionCameraEffect.CameraMode.Free then
        focusOffset = FunctionCameraEffect.GetFreeCameraFocusOffset(npet.assetRole)
      end
...
function FunctionPet:ShowPetTip(playerData, closecallback, stick, side, offset)
  ...
  local s1 = (petLv > 0 and "Lv." .. petLv .. " " or "") .. playerData:GetName()
...
function FunctionPet:SetRewardItemCount(itemid, count, source)
  if itemid == Reward_ItemId and "table" == type(source) and #source > 0 and source[1] == ProtoCommon_pb.ESOURCE_PET then
