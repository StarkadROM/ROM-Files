...
function RaidPuzzleManager:RemovePushObject(npc)
  ...
  if not areaID or not curRow or not curCol then
    return
  end
...
function RaidPuzzleManager:TryPushObject(npc, deltaRow, deltaCol, startSpeed)
  if not npc or not npc.data or npc.data:Push_IsLockPush() or npc.data:Push_GetFeature_ForbidPush() or not npc:GetRoleComplete() or not FunctionUnLockFunc.Me():CheckCanOpen(9923) then
    self:FinishPushObj()
    return
  end
  local areaID, curRow, curCol = self:GetObjAreaInfo(npc.data.id)
  local areaData = self.raidAreaMap[areaID]
  if not areaData or not curRow or not curCol then
...
local CheckNpcIsTargetPushableObject = function(npc)
  ...
  local targetRow, targetCol = curCol or curRow or 1, 1
...
function RaidPuzzleManager:RotatePushableObject(npc)
  if not npc or not npc.data or npc.data:Push_IsLockRotate() then
    return
  end
...
function RaidPuzzleManager:OnRotatePushableObjectOver(npc)
  if not npc or not npc:Alive() or not RaidPuzzleManager.Me():IsWorking() then
    return
  end
...
function RaidPuzzleManager:ForbidAreaLightRefresh(areaID, isForbid)
  ...
  if lightMap then
    if not isForbid or not true then
      local result
    end
    for i = 1, #lightMap do
