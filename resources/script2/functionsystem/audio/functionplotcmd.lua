...
function FunctionPlotCmd:SetCurrentPlotAudio(plotAudio, fade, outDuration, inDuration, fadeInStart, keepBGM)
  if self.currentPlotAudio ~= plotAudio and self.currentPlotAudio and not keepBGM and fade then
    local currentPlotAudio = self.currentPlotAudio
    currentPlotAudio:FadeFromTo(currentPlotAudio:GetVolume(), 0, outDuration)
  end
  self.currentPlotAudio = plotAudio
  if self.currentPlotAudio and fade then
    self.currentPlotAudio:FadeFromTo(fadeInStart and fadeInStart or self.fadeInStart, 1, inDuration)
  end
end
...
function FunctionPlotCmd:ChangePlotAudio(seRelativePath, progress, playType, percent, playTimes, outDuration, inDuration, fallbackDuration, fallbackFadeStartVolumn, finishCallback, keepBGM)
  if AudioManager.Instance ~= nil then
    ...
    local audioSource = AudioUtil.PlayPlot_ByLanguangeSetting(seRelativePath, audioSourceType)
