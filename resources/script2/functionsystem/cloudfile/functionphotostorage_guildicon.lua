...
function FunctionPhotoStorage:GetGuildIconDownloadUrl(guildID, photoIndex, timestamp, extension, isThumb)
  ...
  local url = string.format("%s/%s%s/%s.%s", XDCDNInfo.GetFileServerURL(), self:GetPhotoUrlPath(m_photoType), guildID, photoIndex, extension)
  if not BranchMgr.IsTW() and not BranchMgr.IsJapan() and not BranchMgr.IsSEA() and not BranchMgr.IsNA() and not BranchMgr.IsEU() and isThumb then
    url = url .. "!Percent25"
  end
