...
function FunctionPhotoStorage:GetPersonalPhotoDownloadUrl(photoIndex, timestamp, extension, isThumb)
  local url = string.format("%s/%s%s/%s.%s", XDCDNInfo.GetFileServerURL(), self:GetPhotoUrlPath(m_photoType), Game.Myself.data.id, photoIndex, extension)
  if BranchMgr.IsChina() then
    if isThumb then
      url = url .. "!100"
    end
