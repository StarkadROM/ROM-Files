...
function FunctionPhotoStorage:GetSceneryPhotoDownloadUrl(photoType, roleID, photoIndex, timestamp, extension, isThumb)
  local url = string.format("%s/%s%s/%s.%s", XDCDNInfo.GetFileServerURL(), self:GetPhotoUrlPath(photoType, not roleID), roleID or FunctionLogin.Me():getLoginData().accid, photoIndex, extension)
...
function FunctionPhotoStorage:LoadSceneryPhotoFromLocal(photoType, roleID, photoIndex, timestamp, isThumb, funcOnFinish)
  ...
  if not isThumb or FunctionPhotoStorage.CheckSceneryPhotoPath(oldPath) then
  end
  return (self:LoadFromLocal(oldPath, funcOnFinish))
end
