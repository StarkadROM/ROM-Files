...
function UIUtil.PopUpRichConfirmYesNoView(title, content, confirmHandler, cancelHandler, source, confirmtext, canceltext, unique, lockreason)
  UIUtil.PopUpRichConfirmView(title, content, confirmtext, canceltext, confirmHandler, cancelHandler, source, false, false, unique, lockreason)
end
...
function UIUtil.IsScreenPosValid(posX, posY)
  if not UIUtil.screenWidth or not UIUtil.screenHeight or ApplicationInfo.IsRunOnEditor() then
    UIUtil.screenWidth = Screen.width
    UIUtil.screenHeight = Screen.height
  end
  return posX >= 0 and posX <= UIUtil.screenWidth and posY >= 0 and posY <= UIUtil.screenHeight
end
